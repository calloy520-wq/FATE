...
- **僅 NSFW/kanshou 模式**：同地性別配對提示、肉體狀態 JSON（2026-07 起整合為單一 physical_state 6鍵：姿勢與動作/胸部/顏面/肉棒/蜜穴/服裝狀態，原 visible_state+physical_state 8欄機制已退役；1-5每回合必填、不適用性別的器官代碼直接不輸出）、每位同行同伴的「身體記憶」技能標籤、敏感點、親密次數計數器、愛稱、🔥主動掌握模式段落（前端 `drive` 旗標開啟時注入——同伴依個性主動掌握節奏、攔下玩家的迴避意圖；僅鑑賞生效）
