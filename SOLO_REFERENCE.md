...
- `resolveFateBattle_(atk,def,opts)`：單次交手裁決。處理的 fx 標籤：
  ...
  - **🗡️ Avalon-Saber 理想鄉(2026-07·Mystic_Code injectMysticBuff_)**：御主持「全世界之鞘 Avalon」禮裝 ＋ 從者為【阿爾托莉雅】→ 注入 `avalon_saber`(**被動＝鞘之基本減傷 npDefMul 0.82**·同一般 avalon)＋`regen`(時回)。名字比對 `/阿爾托莉雅/`＋cls Saber。非阿爾托莉雅持 Avalon→僅一般 avalon。
    - **🛡️ 主動理想鄉·完全擋寶具**：`【理想鄉】on` 存從者 MEMORY(idealRealmOn_/setIdealRealm_·Core_Settings)。玩家用卡片 🛡️ 按鈕(get_tags `canIdealRealm`/`idealRealm`→`showIdealRealm`→`set_ideal_realm`/`actionSetIdealRealm`·Router_Economy)張開待命。Router_Battle 敵擊前攔截：`enemyFireNp && idealRealmOn_ && hasFx_(avalon_saber) && 御主純魔≥200` → **完全擋下(不跑 fateStrike·eDmg 0)＋扣 200 御主 MP＋自動關閉**。200 魔≈耗盡電池＝天然每場一次·攻防取捨。這是寶具預告的主動解法(非永久無敵)。
