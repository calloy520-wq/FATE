...
- `resolveFateBattle_(atk,def,opts)`：單次交手裁決。處理的 fx 標籤：
  ...
  - **🐙 summon_horror(螺湮城教本／青鬍子)＝【變身框架】(2026-07 重構·狀態機化)**：從「本場解放才在場」變成**跨戰鬥 12h 的變身態**——單一狀態源＝MEMORY`【海怪護盾】cur|max|expiry`。
    - **入場兩路**：①戰鬥中解放寶具(`actionFateBattle` useNp)②**戰前召喚 `summon_horror_beast`/`actionSummonHorror`**(不進戰鬥·付寶具 prana＋1AP·設肉身 12h)。前端卡片 `s.canSummonHorror`→🐙召喚海怪鈕→`summonHorror(name)`。共用入口 `summonHorror_(mem,gid)`(設 300/300/now+12h)。
    - **在場＝變身態(四效果全綁狀態)**：①**擋傷**`fateStrike_` 先扣海怪潰散才傷本體(gate on defC 現存肉身)；②**再生**每回合 `HORROR_REGEN`(+10)；③**追擊**horrorC(A/A巨獸)每回合並肩咬、抽御主 `HORROR_UPKEEP`(10·※原30)維持·付不出潰散；④**對城防**`npDefScale_` 改吃 `c.horrorUp`(rowToCombatant_ 讀 MEMORY 現存肉身)→**只變身時**本體享對城防禦規模(原 summon_horror fx 恆給·沒召也享·已修)。
    - **狀態源＝single truth**：`horrorPresent_(mem,gid)`(現存且未逾時)判在場；逾時殘影由 `clearExpiredHorror_`(戰前入口)＋regen 逾時分支＋維持費潰散 三處 `clearHorrorShield_` 清乾淨，讓 `horrorUp` 恆真實。常數 `HORROR_SHIELD_HP`300/`HOURS`12(※原8)/`REGEN`10/`UPKEEP`10。
