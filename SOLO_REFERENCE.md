...
5. 敵反擊：`enemyNpSpent` 一場限一次寶具。**🛡️ 寶具閘(2026-06)**：寶具是孤注一擲殺招、非見面招呼——敵唯有**自己被打殘**(`eHpRatio<0.5`)或**我方從者已殘可收尾**(`pHpRatio<0.45`)才解放真名；健康對健康一律普攻試探(免玩家一接觸就被無預警寶具秒殺)。觸發後再吃 `eNpUrge`(狂/暗0.22 else 0.10)+殘血加成的機率擲。
   ...
   - **🔋 敵御主每日回魔(2026-07)**：敵御主電池只被 `drainForNp_` 扣、從不隨時回自然恢復(那套只算玩家隊，見上「共用魔力池」)——若不補，長局裡放過一次寶具後就永久魔力見底、後續遭遇全部啞火，失去「寶具是孤注一擲」的張力。不用玩家那套逐時供需經濟(NPC 不必算到那麼細)，改用最簡單的**新的一天回滿**：`worldTick_`(Time_World)每次執行都呼叫 `refillMastersDaily_(sheets,gameId,day)`——MEMORY 記 `【回魔日】{day}`，見到記錄的日 < 當前日 → 該敵御主 MP 補滿並蓋新日期戳。無關「暗處養不起爆炸」(下方，那是純機率事件、不讀 MP，見注)。
...
- **「養不起爆炸」改成真查電池比例(2026-07 嚴謹化)**：`worldTick_`(休息時，`allowAttrition`)的「供魔不繼爆炸」原本**完全不讀 `COL.PC.MP`**(純看敵從者六圍總和的舊 bug，跟其御主電池剩多少魔力無關，即使御主當天已回滿也可能被判「養不起」)——已改成真查 `enemyMasterIdx_` 找其御主、算 `mpRatio=mp/maxMp`：**比例<25%(電池真的偏低)＋六圍總和≥230(全種子庫僅前14名頂尖強者過線，如阿爾托莉雅285/迦爾納260/伊斯坎達爾255，非原200門檻的26/35幾乎人人過)才有機會**，機率仍溫和(上限 18%)。迴路高的御主池子大，扣同一筆寶具魔力比例掉得慢，天然很少中；迴路低的小池子養強英靈才真的常撐不住——貼合「弱御主撐強從者」的原作張力。masterless(查無御主)不進這池子，那些交給上方 `SEAL_DOOM_HOURS` 的透支倒數處理，避免同一從者被兩套機制搶著判死。另一半「暗處廝殺」(7%隨機、與魔力無關、單純被他人所殺)不變。
