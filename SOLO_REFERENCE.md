...
- **「養不起爆炸」不是魔力機制、只是機率事件**：`worldTick_`(休息時，`allowAttrition`)裡的「暗處廝殺/供魔不繼爆炸」段落**完全不讀 `COL.PC.MP`**——`boom` 機率純看該敵從者六圍 rank 總和(`upkeep`≥260才有機會)，跟其御主電池剩多少魔力**無關**，只是拿「六圍越貴越可能養不起」當敘事包裝，長局用來隨機修剪過多昂貴敵從者(有 `WORLD_FLOOR_`=4 保底，不會清光)。別跟上面兩條真正查 `enemyCanAffordNp_`/`drainForNp_`/`refillMastersDaily_` 的電池機制搞混——那三個才是「敵御主到底有沒有魔力」的唯一真線。
