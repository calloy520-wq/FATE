...
- **「養不起爆炸」改成真查電池比例(2026-07 嚴謹化)**：`worldTick_`(休息時，`allowAttrition`)的「供魔不繼爆炸」原本**完全不讀 `COL.PC.MP`**(純看敵從者六圍總和的舊 bug，跟其御主電池剩多少魔力無關，即使御主當天已回滿也可能被判「養不起」)——已改成真查 `enemyMasterIdx_` 找其御主、算 `mpRatio=mp/maxMp`：**比例<25%(電池真的偏低)＋六圍總和≥200(這隻本來就貴)才有機會**，機率仍溫和(上限 18%)。迴路高的御主池子大，扣同一筆寶具魔力比例掉得慢，天然很少中；迴路低的小池子養強英靈才真的常撐不住——貼合「弱御主撐強從者」的原作張力。masterless(查無御主)不進這池子，那些交給上方 `SEAL_DOOM_HOURS` 的透支倒數處理，避免同一從者被兩套機制搶著判死。另一半「暗處廝殺」(7%隨機、與魔力無關、單純被他人所殺)不變。
