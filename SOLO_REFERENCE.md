...
- **🎛️ 實戰回饋三修(2026-07 玩家第一場實測)**：①**反噬去即死化**——極限檔定價已把血燒到見底(drainForNp_)、反噬再補刀＝第一發 65% 出局的雙重懲罰(玩家實測「太容易死」)。反噬改【純資源傷害·保底1不致死】，整組致死機件(戰果後結算/從者消散/causedDefeat 文案分流/前端燒斷橫幅)拆除；機率與傷害幅度不變(血魔雙空的資源壓力仍在)。②**超載檔位改一次點選**(玩家「開令咒+寶具一直問東問西」)——`openOverloadPicker`(Script.html·showHistoryOverlay 模式)一個選單三檔(底費/超載/極限·各標價格/倍率/反噬%·蓄勢令咒顯示加持徽章)，點選即出手、關掉即中止；`servantStrike` 增第8參 `overloadArg`(undefined→開選單·經 `pickOverloadAndStrike` 帶回)。規格外寶具全程 0 個 confirm()（多寶具＝寶具選單→檔位選單兩次點擊）；沒＋寶具維持單一確認。③**道場講評對症化**(玩家「伊莉雅每次都叫我繞去打御主」)——`handleDefeat` 從 res.report 推導**結構化敗因**(時限/夜襲/斬首失敗/正面交鋒·含是否已用寶具/反噬)前置給 `buildTigerDojoPrompt_`；prompt 拿掉「該偷襲御主而非硬拚寶具」等誘導範例(AI 每次照抄)，改「建議必須從【本局敗因】推導、嚴禁無關泛用套路」。
