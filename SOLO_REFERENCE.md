...
- **❖ 令咒可疊加寶具/主動技(2026-07 玩家定案·防繁瑣版)**：原作式「令咒疊寶具」孤注一擲——絕對必中(`atkWins=true` 凌駕擲骰＋無視必中槍)＋×1.5(Engine_Fate:733)。**疊加選擇全收在❖令咒鈕內、寶具/主動鈕零多問**(玩家否決了「每次放寶具都追問令咒」的版本)：❖鈕流程＝確認燃令咒→「疊加寶具？」是→**遞迴** `servantStrike(np=true,seal=true)` 走寶具流程(多寶具選單/超載檔位照問·寶具基礎確認因 useSeal 已確認意圖而跳過)；否→「疊加主動技？」(有真·施放技術才問)；再否＝必中普攻(原效果)。後端零改動(seal/np/skill 本就獨立旗標·對轟 pPow 已帶 seal·openingNp/openingSeal 同清)；戰報標題可同顯「❖令咒 💥寶具」；令咒對斬首(isMaster)依舊不適用。
