...
- `resolveFateBattle_(atk,def,opts)`：單次交手裁決。處理的 fx 標籤：
  ...
  - **🗡️ Avalon-Saber 理想鄉(2026-07·Mystic_Code injectMysticBuff_)**：御主持「全世界之鞘 Avalon」禮裝 ＋ 從者為【阿爾托莉雅】→ 注入 `avalon_saber`(**被動＝鞘之基本減傷 npDefMul 0.82**·同一般 avalon)＋`regen`(時回)。名字比對 `/阿爾托莉雅/`＋cls Saber。非阿爾托莉雅持 Avalon→僅一般 avalon。
    - **🛡️ 理想鄉·完全擋寶具(被動自動·2026-07 定案)**：**無開關**——Router_Battle 敵擊前攔截：`enemyFireNp && hasFx_(avalon_saber) && 御主純魔≥200` → **完全擋下(不跑 fateStrike·eDmg 0·戰報敘述)＋扣 200 御主 MP**。200 魔≈耗盡電池＝天然每場一次；★攻防取捨天然成立：留魔則自動擋、耗魔放自己寶具則擋不住(魔<200)，玩家用「花不花魔」隱性決定。get_tags `canIdealRealm`→卡片資訊標籤「🛡️ 理想鄉·自動護盾」(showIdealRealm 純說明·不可控)。這是寶具預告的解法(非永久無敵·非按鈕)。曾試「主動按鈕 toggle」版·因不夠直覺已改被動。
