...
| **show-don't-tell** | 敘事禁止直接寫出角色的 願望／個性 字面（萌點 2026-09 整組退休）。只能用神態動作演出（`servantCard_` 鐵則一二三 已強制）。 |
...
- `callGeminiAPI` payload 只有 model/messages/temperature/top_p/max_tokens＋可選 top_k/penalty（⚠ 2026-09 稽核：文件曾寫「帶 Gemini safety_settings」，代碼從沒有這個欄位）＋尾端附「台灣繁體中文（正體字）」語言鐵律（避免簡體滲透）。`max_tokens`：鑑賞 `actionPlay_` 明傳 2400（時間跳躍且無同伴時 700；`callGeminiAPI` 裡的 `isNsfwMode ? 1000` 預設從未生效，因為唯一的 NSFW 呼叫端一律明傳）/ solo fallback 2000 / `narrateWithState_` 預設 720。連線失敗時 fallback 回貼世界觀的柔性訊息（🌫️因果紊亂），原始錯誤只進 `Logger.log`。
...
- 🤝 **盟約倒數（2026-09 補接線）**：盟約有效期是 `day <= until`（`breakStaleAlliances_` 判 `day > allyUntil_` 才破），
  ...
  現在 `getLocalPeopleList` 多下傳一欄 `allyLeft`（後端算好，含今天＝`until - day + 1`），卡上顯示「休兵剩N日」。
  ⚠ 另一個破裂條件是**存活敵從者 ≤3 時強制全面瓦解**，那個不是倒數、規則說明裡已寫明。探針 `ally.js`。
...
PC(眾生):
  ...
  MEMORY12 INTENT13(棄用·原萌點) FACTION14 RANK15(職階) CONTRIB16 ALIGN17
...
MASTER(御主殿): ID0 NAME1 SEX2 APPEAR3 MAGIC4 CIRCUITS5 MELEE6 MAGIC_RANK7 HOME8 WISH9 PERSONA10 WAR11 SOURCE12 BACK13(身世) MOE14(棄用·原萌點) ALIGN15(陣營)
...
- **慾海擋牆**：`KANSHOU_BLOCKED_ACTIONS_` 白名單，`isKanshouCtx` 命中就在 dispatcher 層擋下戰鬥/經濟類 action（`fate_battle`/`use_seal`/`mana_supply`/`bond`/`rule_break_steal`/結盟系/陣地系/`set_*` 等）。刻意不擋 `update_fate`/`update_rel_tag`（solo/鑑賞共用 handler）。⚠ `move` **有擋**（鑑賞的移動走 `actionPlay_` 的 `moveTarget`，不經 `move`）。
...
### 命中/傷害核心
- **六圍降權（核心哲學）**：參數是角色速寫非戰力表。命中 `= d20 + (rankTier(主屬)×0.8 + rankTier(筋力)×0.2)×K_STAT + rand(-3~3) + outMod`；迴避 `= d20 + (rankTier(敏)×0.5 + rankTier(耐)×0.5)×K_STAT + rand(-3~3)`（⚠ 2026-09 稽核更正：舊寫法 ×2.5／0.65／0.35 已不是代碼現況，見 Engine_Fate.gs:515-516）；傷害 flat `rankVal×0.6`。階差壓到~12，d20 重新主導。
- **D&D 傷害骰**：`rollDice_(n,sides)`＋`rankTier_(r)`（E1→EX6）。武器骰每擊 `round(rankVal(主屬性)*0.6)+rankTier d8+命中分差*1.2−耐久/2`；暴擊(擲20)多骰 `rankTier d8+12`。
...
- **多寶具英靈 `servantNpOptions_`**（中央表）：斯卡哈L/吉爾(王財·Ea)/恩奇都/伊斯坎達爾/美杜莎R/EMIYA（⚠ 迦爾納／蒼白騎兵已隨陣容清理移除）。首項＝主寶具（敵預設）。玩家選定隨 fate_battle 夾帶。`npProfile_(c)` 解出 {scale,fx,r}；`npEffectiveRank_(c)=npProfile_(c).r`（單一真實來源·各選項專屬階級）。`resolveFateBattle_` 簽名效果（gae_bolg必中/ea執行殺/ubw/zabaniya/summon_horror/petrify/scaleMult）一律吃 npProfile。
...
- **出力檔** `outputTier_(pct)`→{hit,dmgMul,drainMul,np,label} **三檔**：100%(+3/×1.3/×2.0/可放寶具)、60%(巡航)、20%(省著走)。`snapOutput_` 把舊存檔的 40／80 吸附到 20／60（往下、安全）。`servantOutput_/setServantOutput_`。寶具僅 100% 可解放。
...
### 🎚️ 戰鬥數字住在哪（2026-09 重構後·玩家問「需要完全統一整理嗎」）

**答案是不要完全統一。** 戰鬥效果本來就兩類，硬併會讓表長出條件 DSL：

| 類別 | 住哪 | 誰在驗 |
|---|---|---|
| 平坦加減乘（17 條）：怪力／魔力放出／卡里斯瑪／狂化／神代／風王／自我改造／千里眼／陣地… | `SKILL_FX_`／`DEF_FX_` | `check_fx.py` 第一道（逐階級比值） |
| **有條件**（18 格）：只對 Archer／只在奇襲／只在第 1 回合／要比雙方階級／要看對方神格 | **`FX_TUNING_`**（數字）＋ `resolveFateBattle_`（條件） | `check_fx.py` 第二道（算式比值、散文比可推導性） |

改之前那 18 格的數字只活在 298 行函式的 if 裡，而前端把它們講給玩家聽——**沒有任何機器在對答案**。
現在函式一律 `fxTune_(fx, field, fallback)` 取值。**條件邏輯沒有搬**，那是刻意的。

**等價怎麼證**：`scratchpad/size/fxbase.js` 把 VM 的 `Math.random` 換成固定種子 LCG，
跑 25×25 英靈 × 6 種 opts × 3 種狀態＝ **10800 場**，每場攤成一行；改前改後 **md5 必須相同**。
（這輪實測：`3e8d1c51…` 前後一致，平均傷害 151.16、攻方勝場 8024/10800 也一字不差。）

**順手解掉的雙存**：`rankMul_` 寫死 `/30`、`rankTier_` 寫死 60/50/40/30/20，兩個都是從
`RANK_VALUE`（在 `Core_Settings.gs`）推出來的卻各寫各的。改成 `RANK_VALUE['C']` 與 `rankTierSteps_()`，
0~80 全部階級值逐一比對過、新舊完全一致。⚠ `rankTierSteps_()` 刻意延後求值——跨檔常數在頂層直接算會踩載入順序。
...
- **再簡化（2026-09·玩家「玩家只要一路按 就可以直接開始」）**：
  ① **戰爭選擇屏整個拿掉**——`newGameFlow()` 直接 `chooseWorld('canon','5th')` 進創角。第四次與混亂**沒砍、只是不擋在路上**，
  收進 `step-name` 的「▸ 自己設定」摺疊區裡的 `#s-war`（`setWarFromSelect_()` 改同一組 `warMode`/`currentWar` 變數，不切畫面）。
  ② **召喚頁先問「怎麼召」**——原本四條路(挑名冊／隨機／一句話生成／工房)一次攤開，玩家原話「這裡最亂」。
  改成三選一的門 `#sum-gate`：`🎲 交給聖杯決定` ／ `📜 自己挑一位`(展開 `#sum-pick`) ／ `🖋️ 自己造一位`(展開 `#sum-create`)，
  由 `sumMode_(mode)` 切換，每區留一顆「← 換一種方式」回門口。
  ⚠ 門口的隨機**先清掉 `selectedSummonClass`**：玩家可能先點過「自己挑」瀏覽職階再退回來按隨機，不清會被悄悄鎖死在那個職階
  （同款坑 `summonByDesc` 踩過，玩家原話「難怪我自創一堆 Saber」）。
  ③ **隨機不搶這一局在場的英靈**（玩家「選5 玩家不會搶5 選4 玩家不會搶4」）：`actionSummonServant` 的隨機分支
  依 `warName` 拿 `FATE_5TH_ROSTER`/`FATE_4TH_ROSTER` 的 `hero`（那是**英靈殿 ID**、不是 NAME）當黑名單，chaos 不排。
  **自選／自創不受限**——那是玩家自己要的，撞上敵陣由既有的 `playerServantName` 機制把敵方那組換掉。
  實測池子：第五次 10 位、第四次 11 位（跨場次抽得到，正典本來就跨過戰爭）。探針 `summon_pool.js`（含退化測試）。
- **🧮 技能說明的算式也是「一個數存兩處」**（2026-09 新掃描器 `check_fx.py`）：
  `Script.html` 的技能說明會把傷害數字**當場算給玩家看**（`+${Math.round(8 * m) + 14}`），
  真正結算的卻是 `Engine_Fate.gs` 的 `SKILL_FX_`／`DEF_FX_`。改了後端沒改說明＝玩家看到假數字、零錯誤訊息。
  `check_mirror` 只認 `KC_*` 常數，看不到藏在樣板字串裡的算式，這塊一直沒防線（CODE_NOTES 記著
  「str_up/burst 那批漏改」，咬過一次）。現在 14 個 fx 兩邊都有公式的逐個比對。
  ⚠ **比數值不比字面**：引擎記 `1-0.18r` 倍率、說明寫「−18%」是同一件事（divine_core），
  第一版只比字面當場整排誤報——誤報的掃描器會被直接無視，比沒有更糟。
...
- **🔢 一個數只准存一處（2026-09 稽核再抓到三例）**：`check_mirror` 只盯 `KC_*` 常數，
  **UI 文字裡寫死的數字它看不到**——這是共同回憶 10/8 那個 bug 的同一條縫。這輪補的：
  ...
  ③ 工房預算 340 存三處（後端函式內、`FORGE_FLOOR_`、Index.html 靜態文字）→ 收成檔案級
     `FORGE_BUDGET`，`FORGE_FLOOR_ = FORGE_BUDGET`，靜態文字改由 ~~`openForge()`~~ 填（2026-09-24 舊工房已移除，新工房是 `warForgeOpen`）。
  ⚠ 找法：把 UI 文字裡的「數字＋單位」抓出來，跟後端數字常數的值對照，再逐條看是不是同一個數。

- **✍️ 玩家看得到的字怎麼寫**（2026-09 玩家「我覺得很多說明用詞都太AI了…你不只是對我 而是在對所有玩家」）：
  掃出來的四個壞習慣，改的時候照這四條看：
  ① **破折號當解釋接頭**（「X——其實是Y」）：這是最明顯的 AI 腔。拆成兩句話，或用逗號。
  ② **「＝」當「就是」**（「留空＝隨機給一份」）：寫成人話「留空就隨機給你一份」。
  ③ **①②③ 當標題**：玩家不是在讀規格書，直接用標題或圖示。
  ④ **規格書語氣**（「純敘事風味用途」「不影響戰鬥數值計算」「未發動則該擊無此加成」）：
     改成「多半只是風味」「沒發動的那一擊就沒有這個加成」。
  ⚠ **刻意不改的**：狀態卡的 `—— 處事個性 ——` 那種裝飾分隔線（不是句子）、
  警告框的 ⚠（那是 UI 圖示不是文氣）、以及**寶具／技能辭條**的「名稱：詩意描述——機制」
  （那是辭典體例，25 條一起改風險大於收益；真要動就整批一次來，別改一半）。
  ⚠ **送 AI 的提示詞不在此列**——那是寫給模型看的，破折號與 ★ 都是刻意的。
  📐 這一輪實改：Index 38 句、Script_Kanshou 27 處、Script 44 處、Onboarding 9 處、後端玩家訊息 37 處。
  📐 **第二輪掃尾（2026-09 全面稽核）**：第一輪漏掉的解釋型破折號 37 處（Script 18、Kanshou 16、Onboarding 3）。
  現在非辭條、非裝飾線的玩家文字裡**一處不剩**；剩下的 26 處全在寶具／技能辭條那一區（刻意保留的辭典體例）。
  查法：抓「非空白字元 ＋ ──」，排除以 `//`／`<!--` 開頭的註解行、`—— X ——` 裝飾線，以及 1460~1590 的辭條區。
...
- **Seed_Rivals.gs**（開局鋪敵）：
  ...
  - `masterToNpcRow_`：御主殿列→敵御主眾生列。TRAIT←外貌(mAppear)、PREF←persona、BACK←身世、INTENT←永遠空字串(萌點已退休)、MEMORY=【願望】|【魔術】|【體術】|【魔術階位】|【迴路】。
  - `heroToNpcRow_`：英靈殿列→敵從者眾生列（god_hand 讀 `lives` 覆寫【試煉】）。
  - 正史真名擋名（禁玩家搶角）＝`actionManualNpc` 內聯的 `_canonServantHit`／`_canonMasterHit`（直接掃 SEED_SERVANTS／SEED_MASTERS；沒有 canonHeroNames_ 這支函式）。`FATE_4TH_ROSTER`/`FATE_5TH_ROSTER` 各自 `loc` 欄決定敵出生點（非讀 COL.MASTER.HOME）。
...
- AP：每日12，移動2AP、戰鬥/偵查/補魔/結盟/共處/整備=1AP（禮裝全被動、不耗 AP）、休息每hr補2。
...
- `masterCard_(row)`：御主演出依據卡（名/性別/性格/特徵/願望/體術/魔術）。**御主有聲**：可依性格給台詞，但**不替御主拍板戰略抉擇**（出戰/結盟/移動/補魔由玩家按鍵）、不逼問、不快轉。
- `enemyMasterCard_(row)`：敵御主精簡演出卡（性格取前4/特徵取前3/身世/體術/魔術，不塞六圍/寶具）。NPC 不受「不可替玩家決定」限制。只在敵御主本人同地在場時注入（`enemyMasterIdx_`＋位置比對）；`actionFateBattle` aiPrompt 加**關係錨**（「此敵御主正是 defC 的契約御主」）＋**戰局實況錨點**（用算好的 HP比例/傷害交換組白話戰況給 AI，讓敵御主反應對得上場面）。正典人物優先調用原作認知、卡片僅錨點。
...
- `raiseBond_`(升既有·帶 preData)／`bumpBond_`(無則建)／`bondFavor_`(讀好感傾向)；讀好感值直接 `row[COL.PC.BOND]`（沒有 getBond_）。`extractWish_`、`buildDreamPrompt_`（敗北虛假之夢·`cause==='timeout'`＝時限夢）、`buildVictoryDreamPrompt_`（勝利真夢·不露破綻）。
...
## 10.5 性格四格的標籤（2026-09 語意修正）

`PREF_LABELS_` 舊值是 `日常表象／真實內裡／喜歡的事物／討厭的事物`。**「表象」的語意是「裝出來的」**——
但種子資料那一格是真的（SABER 就是一絲不苟，不是裝的）；22 筆逐筆看過，沒有一個是假面。
標籤在說謊，資料沒有。現值：**`平常／熟了之後／喜歡／討厭`**（順帶比舊的短）。

⚠ **這組標籤有三張表拿【字面】當鍵，改一張要三張一起改**：
`PREF_LABELS_`（順序與字面）、`QUAD_REDUNDANT_`（疊字剝除）、`QUAD_LOOSE_LABEL_`（缺格時的鬆散標籤）。
2026-09 改的時候就漏了 `QUAD_REDUNDANT_`——查不到 → 疊字不剝 → 卡上冒出「喜歡：喜歡甜食」，零錯誤訊息。
`cards.js` 現在釘死這三張表對得上（含退化測試）。
...
- **行動 handler**（多數走共用骨架 `runSimpleAction_`）：`servantStrike`（出戰/寶具/令咒/刺殺御主）、`manaSupply`、`spiritRepair`、`bond`/`openBondMenu`、`rest`/`openRestMenu`、`scout`、`ruleBreakSteal`、`secondWind`、`setWorkshop`/`scavenge`、`proposeAlliance`/`breakAlliance`/`allyBond`、`prepMeal`。寶具旁 `🔥＋`/`🩸＋＋` 超載檔位鈕（`npTierArmed`·零 confirm·發射後歸零）、❖令咒蓄勢 toggle。
...
- **召喚/創角**（搬到 `Script_Onboarding.html`·只開局跑一次·共享同頁全域作用域）：`accountLogin`/`chooseWorld`（2026-09 起型態×場次合成一屏三顆鈕）/`openCanonMasterPick`/`pickCanonMaster`、`rollFate`/`selectFateRoll`/`renderFateRolls`（魔術天賦測定·擲 melee＋獨立擲 magicRank）、`checkNameOk_`（不再自成一屏，由 `createPC` 送出前呼叫）/`createPC`/`backfillMasterAi`、`doSummon`/`summonByHero`/`startGame`。
...
- **撞見敵人的配套「後續選擇」**（局面決定開放哪些按鈕·`FACTION_ENCOUNTER_CHOICES_` 資料表）：抵達時 `resolveFactionEncounter_` 回 `choices:{ambush,incite,slip}`，actionMove 寫【趁隙】loc@type 窗口進御主 MEMORY，`buildTagsPayload_` 回 `encounterWindow` → 前端 `renderEncounterBubbles`（複用鑑賞 `.btn-option` 泡泡·GAS 自產不靠 AI）畫進 `options-container`；抵達時 travelTo 吃 `_state.tags`、之後每次 `refreshFateTags` 依 `window._encWin` 重畫，窗口清掉即自動消失。
  - 🥷 **趁隙偷襲**（`faction_ambush`→`actionFactionAmbush`→`playerAmbushOnEnemy_`）：敵分心（frenzy/standoff/parley/pact）時搶一記奇襲，複用 `resolveFateBattle_` 的 `ambush` 先機（鏡射 `enemyAmbushOnServant_` 反向），`ambushMul = 1.3 + clamp(bondFavor×0.5, −0.25, +0.5)`（1.05~1.8，中性 ×1.3；近期被偷襲過【提防】再 ×0.6）、處理敵死亡(DEAD_/`markMasterLostServant_`/`aliveEnemyServants_` 勝利)。耗 1AP、用掉清窗口。
...
| 東西 | 現在 | 當初瘦身後 |
|---|---|---|
| `miniSystem`（每回合固定成本） | **641 字** | 586 字 |
| `servantCard_`（我方·阿爾托莉雅為例） | **300 字** | 平均 249 字 |
| `servantCard_(…, {foe:true})`（敵方） | **212 字** | — |
| `masterCard_` | **175 字** | — |
| 一般交鋒的戰報 `aiPrompt` | **1281 字** | — |

⚠ `miniSystem` 從 586 長回 641，是這半年陸續加規則的累積（不是壞事，但**下次再加規則前先看這個數字**）。
量法：`scratchpad/size/` 的探針直接抓 `Router_Narrative.gs` 的樣板字串長度＋跑一場真的交鋒取 `aiPrompt.length`。
...
- 🫶 **態度欄改成會動的（2026-09）**：玩家「每次叫 API 都是新的呼叫…如果她其實已經跟玩家打好關係、
  但實際資料沒有變動，她會就一直抱持原廠設定」。我方從者的那一欄改印「此刻對你」＝`bondStance_(BOND, 種子 toMaster)`
  （45/55/70/85 四階，<45 退回種子底色）；敵從者維持「對自己御主的態度」不吃玩家好感。位置也後移到口吻之後降權重。
  **鑑賞不受影響**（`servantCard_` 只有 solo 在用）。
- 🕊️ **交涉取代單一的「示好」（2026-09）**：敵御主面板從〔刺殺／結盟／示好〕變成〔刺殺／結盟／💬閒聊／📜交換情報／🕊️讓開一步〕。
  三種交涉走同一支 `actionParley`＋同一張 `PARLEY_ACTS_`，差別只有門檻/升幅/效果；每種各自每日一次（`【交涉日】` 一格裝三個）。
  `intel` 需對方好感 ≥0.15、當場掀開最多 2 名未偵查敵人的 `SEEN`；`yield` 擲 `parleyYieldChance_`，成功＝對方與其從者移離這一格。
  量測：好感 95 談成 70%、好感 0 談成 32%。~~`court_enemy`~~ 路由已換成 `parley`。
- 🫶 **敵方卡也會動了（2026-09 凍結資料稽核）**：`foeStanceNote_` 補上「此刻對你」——求愛/挑撥/結盟都在改
  敵方的 `COL.PC.BOND`，但兩張敵方卡原本從不講，AI 只好一直照原廠敵意演。結盟優先、中性不送。
- 🧠 **卡片是內化用、不是人物簡介**：標頭改「核心特質·內化用」，`performanceNote_` 明講不要把設定唸出來、要推演行為。
- 🗡️ **`servantCard_(row, {foe:true})`**：敵方卡不送 小動作（「熟了才看得到的一面」，
...
- ~~**萌點（`COL.PC.INTENT`）**~~：2026-09 萌點整組退休，這條連同 `clampMoe_`／`MOE_STORE_MAX_` 一併移除。
...
#### 來源優先序（`npKindOf_`，`Engine_Fate.gs`；`npLingers_` 已刪·零呼叫者）
...
2026-09 玩家問「我不想要御主有血條呢……妳覺得可行嗎」。量過血量的牽連範圍：

- `COL.PC.HP` 在御主身上有 **111 個讀寫點**
- 它撐著一整層機制：**御主戰死＝這局結束**（`Account.gs` 明文「御主血歸 0…這局已結束」）、
  敵從者**斬首御主**（大成功直接斬殺、護衛從者跟著消滅）、以身相代、燃燒生命力補魔（有「血量底線 1，不可被榨死」）、
  過載反噬、強撐、令咒反噬致死

提了三個方案：①只拿掉畫面上的數字（改顯示「毫髮無傷／掛了點彩／重傷」，機制全留）
②御主完全無敵（等於移除整個聖杯戰爭的輸掉方式）③不會死但會失能。
...
把旁白改成第二人稱之後，我自己製造了**新的撞號**：提示詞裡「你」同時指兩個人。

```
miniSystem 開頭   你是《命運停駐之夜》的說書人     ← 你＝AI
miniSystem 鐵律1  「你」＝玩家（御主）本人          ← 你＝玩家
miniSystem 鐵律4  數值系統已經算完，你只寫字        ← 你＝AI
```

這跟原本「旁白的我 vs 角色台詞的我」是**同一個形狀**，只是換了個字。
根治法是：**「你」只留給玩家，說書人一律不用第二人稱稱呼**。

| 檔 | 原本 | 現在 |
|---|---|---|
| `Router_Narrative.gs` | `你是《命運停駐之夜》的說書人。` | `《命運停駐之夜》說書人守則。` |
| 同上 鐵律1 | `「你」＝玩家（御主）本人，只寫你看得見…` | `敘事文字裡的「你」【只能】指玩家（御主）本人，只寫玩家看得見…` |
| 同上 鐵律4 | `你只寫字、不複述數字` | `說書人只寫字、不複述數字` |
| 同上 鐵律8 | `Fate 正典角色依你自身認知演` | `…照原作認知演` |
| `Router_Bond.gs` ×2 | `★你擅長書寫細膩動人的情慾` | `★這一段放手寫得細膩動人` |
| `Router_Bond.gs` 示好 | `GAS 已算好數值，你只演反應` | `數值系統已經算完，只演反應` |
| `Gallery.gs` 律令8 | `關係標籤由玩家定，你不改` | `…不可改動` |
| `Gallery.gs` ×4 | `你只需…／你只演…／系統給你的…` | 改成祈使句或「本回合」 |
| `Gallery.gs` 世界概況 | 一段裡「你」同時指玩家／被問到的人／說書人 | 三者各自點名 |
...
## 🧪 探針補洞（2026-09 全面稽核·玩家「全面檢查吧 一直做 不要停」）

**起因**：清點下來 69 條路由有 27 條從來沒被探針碰過。沒被跑過的路不是「應該沒問題」，
是「壞了也不會有人知道」——這一輪就是照著那張清單一條一條補，補到哪就在這裡記一筆。
（探針本體在 `scratchpad/size/`，不進 repo；這份清單是它們唯一的索引。）

- **`ally2.js`（14 條）** — 盟約三部曲（提議／共處／撕毀）＋只給 `npcId` 不給名字的查找，
  ＋鑑賞的改名／改性別／改住處名。**這支抓到本輪第一個真 bug**（共處的日限戳記，見 `actionAllyBond` 那節）。
...
- **`dl_np.js`（11 條）** — 真名應對的兩個收場，**這支抓到本輪第二、三個真 bug**：
  ① 第 14 日深夜按下應對會跨過時限卻不判敗（時限判定原本 regex 回傳的 clock 字串，而這支不回 clock）；
  ② 被那一發打到從者全滅時只標 `DEAD_`、沒判敗也沒夢境，玩家卡在「沒有從者的殘局」。
  ③ 對衝把【最後一名】敵從者打消滅時只設 `foeDead`、沒判勝——而勝利判定只存在於 `fateStrike_`，
  敵從者全滅後玩家再也打不了仗，於是永遠等不到收場、只能被時限反過來判敗。
  順便釘住雙從者只折損一員時**不可以**判敗。
  三處終局判定現在共用 `markDefeatIfWiped_`／`markVictoryIfCleared_` 兩支對稱的 helper。
- **`clock.js`（14 條）** — 時鐘／AP／時限：開局 AP 滿且那兩格是留空的、休息補 AP 不超上限、
  AP=0 要擋且不可以偷偷結算或扣成負數、跨日回滿、第 14 日之後判敗並給收場敘事。
  ⚠ 開局 `AP`／`DAY` 兩格是**空的**（`clockFromRow_` 讀到空字串才回預設），直接 `parseInt` 會拿到 NaN——要問 `getClock_`。
- **`cards.js`（9 條）** — 角色卡不可以把「佔位字」當成角色設定餵給 AI。**這支抓到本輪第五個真 bug**：
  種子的 `words` 全都只有 2~3 段，第 4 格一律被補成佔位字「厭惡之事」，而渲染出口先剝疊字「厭惡」再比對佔位清單，
  「之事」比不中 → **25 位從者的卡上全都寫著「討厭的事物：之事」**。25 位 × 每一則戰鬥/相處提示詞。
  含退化確認（還原修法會命中全部 25 位）。
- **`solo_all.js`（既有·加掛垃圾字掃描）** — 24 顆按鍵跑完之後，把**真的送出去的那份提示詞**掃一遍
...
- **`battery.js`（7 條）** — 「御主不上戰場」之後，御主唯一會掉血的路：魔力不夠又要放寶具＝燒命當電池。
  三種情況都量過：魔力夠→打完一滴不掉；只差一點→真的燒血（實測差 20 魔＝燒 40 血，剛好 2:1）；血只剩 3→擋下或走真名應對，
  血永遠 ≥1、不會死在自己的供魔上。
  ⚠ 寫這支踩到三個坑，**都不是 code 的問題、是探針自己**：① MP 直接歸零驗不到燒血（血只能 2:1 換魔，
  湊不到全額會整個被擋下）② 上一場打完要重新挑在世敵人並搬到同地 ③ 敵人可能先預告真名，回的是
  `needNpResponse` 控制訊號、這一發根本沒打。第三點會讓它**間歇性紅燈**——已改成重試最多 12 次，連跑 5 遍穩定 9/9。
  **會間歇性叫的探針跟不會叫的一樣糟**：你會開始習慣忽略它。
- **`twice.js`（11 條）** — 同一顆按鈕按兩次（網路一慢玩家一定會這樣做）：建角／召喚／佈設陣地／
  進鑑賞／提議結盟各按兩次，第二次要嘛被擋、要嘛回到同一個存檔，不可以「靜靜地把同一件事做兩遍」。
  現況全過（多一個御主、多一名從者、白花一次 AP、多一個鑑賞分身，都不會發生）。
  ⚠ 寫這支時踩到兩個**探針自己的**坑：solo 的帳號綁定不在 MEMORY 上（那是鑑賞的做法，要從「帳號」表的 PC 欄反查）；
  玩家御主那一列的 `FACTION` 是「無」不是「御主」（「敵御主」才是敵方那批）。兩次都是先看到紅燈才發現是自己數錯。
- **`ai_evil.js`（19 條）** — 「AI 只說書、GAS 掌數值」那條鐵則的機器驗證：讓說書人回 15 種爛東西
  （null／空字串／不是 JSON／半截 JSON／沒有 narration／narration 是數字或物件／夾帶 HTML 注入／2 萬字／
  `rel_changes` 想給 +9999／想改不在場的人／想改玩家自己／不是陣列／想自己指定關係標籤／`world_note` 灌 50 筆），
  兩軌都跑一遍。結果：零例外，好感一次最多動 ±5（`rel_changes` 硬夾），不在場或不同局的人動不到，
  玩家自己動不到，關係標籤仍由 GAS 依好感升降，solo 的數值一格都不受影響。
- **`own.js`（64 條路由 × 2 種別人的 pcId）** — 帳號分流的機器驗證：拿別人的 solo／鑑賞 pcId
  對每一條路由各打一次，兩條判準——**不可以回 `success:true`**、**不可以動到任何一列資料**（整表前後比對）。
  現況全過。`enter_kanshou` 會改到資料是預期的：它替呼叫者開自己的後日談存檔、根本不看傳進來的 pcId
  （實測新列標著【帳號】乙、甲那一列一個字沒動），登記在 `SELF_CREATE_OK`。
...
🏃 **一次跑完**：`scratchpad/size/runall.sh`——只跑**有斷言**的那 22 支，印一行摘要，全過才 exit 0。
改完代碼想快速回歸就跑它（目前 22 支／約 300 條斷言）。
⚠ 沒列進去的 `.js` 多半是「量東西／印報表」用的（`create_probe`／`move_probe`／`np_class`…），
它們印的 ❌ 是內容不是失敗（AI 被攔截、規則擋下），**不要拿來當紅燈**。

⚠ **順手學到的**：跑一輪「所有探針」很便宜（幾分鐘），而且會抓到**探針自己過期**——
這輪就有三支因為我改了簽名/刪了常數而靜靜壞掉。改完代碼把全套探針掃一遍，比只跑 check.sh 多看見一層。

## 🫶 「此刻對你」＝種子底色 ＋ 好感位移（2026-09）

`servantCard_` 的這一格原本是 `BOND_STANCE_` **整句取代**種子的 `toMaster`。量出來的後果：
好感 ≥45 之後，25 位從者的態度句變成**同一句話**——吉爾伽美什「打從心底信任你」、
已喪失言語的赫拉克勒斯「露出只給你看的那一面」、黑櫻的佔有慾被換成信任。
**角色的區別度剛好在關係開始好看的那一刻整個消失**，而且零錯誤訊息。

現在 `bondStance_(bond, seedStance)` 是疊加：種子句永遠留著，好感只在後面補一句
**位移句**（`BOND_STANCE_` 四階，每一句都寫成「對前一句的修正」，所以接在任何種子句後面都成立）。

```
好感 20  視為雜種，命令聽不聽全看自己高不高興
好感 45  視為雜種，…——不過對著你的時候，這份底色鬆了一些
好感 70  視為雜種，…——不過對你早已破了例，而且不打算收回
好感 90  視為雜種，…——不過早就名存實亡，只是誰都沒說破
```

⚠ **機器擋**：`check_seed.py` 第⑨條盯的是**結構**——階梯的值必須先落進變數、最後的 `return`
必須同時帶著種子參數（會跟著 `var seed = String(seedStance…)` 這種別名走）與那個變數。
有人改回 `return BOND_STANCE_[i].s` 就會叫。退化測試已注入確認。
探針 `seed3.js` 另補一條功能面的：同一好感、不同角色，態度句不可以相同。

## 🚫 萌點整組退休（2026-09）

玩家：「萌點不該由我們定義⋯我們應該只要給性格 萌不萌 是玩家的事情」。

拔掉的東西：種子的 `moe`／`dailyMoe`／`SEED_MASTERS.moe`、三張卡（`servantCard_`／
`masterCard_`／`enemyMasterCard_`）的萌點欄、鑑賞【在場人物】與【玩家資料】兩行的
`萌點(僅供內化)`、創角的 `npc_intent`（solo 御主／鑑賞御主／AI 從者／工房四處）、
`translateMoeToDaily_`、`clampMoe_`／`MOE_STORE_MAX_`／`MOE_BAN_`、逆天改命的
`intent` 路由與前端 `intent-box`／`fate-btn-intent`／`cf-moe`。

⚠ **欄位不刪**（COL 是位置索引）：`PC.INTENT`／`HERO.DAILY_MOE`／`MASTER.MOE` 三格永遠寫
空字串，已登記進 `check_seed.py` 的 `DEAD_COL_ALLOW`。`buildPlayerStatusString` 第 25 格
也照樣佔位送空字串，別把它抽掉（前端是照索引讀的）。
`CODEX_PERSONA_VER` 升 **v78**：舊局已經存進去的萌點，在升級路徑裡一併洗掉。

⚠ `check_seed.py` 第⑩條擋復發：任何**代碼行**出現 `萌點`／`npc_intent`／`clampMoe_`…，
或去**讀**那三格欄位，就會叫。註解與墓碑（同行寫著「棄用／退休／已移除」）放行。

## ✂️ 戰鬥提示詞瘦身：只給事實，讓 AI 去擴寫（2026-09）

玩家指著「御主接住了從者」那句：

> 「你這提示詞 太冗長了…請簡潔 有力 讓AI去括寫 只要告訴AI實際的事實就好…
> 　拜託不要講解太多 說得越多 我的畫面就會固化…讓他依照簡短的事實 去演出這一切」

一次 `fate_battle` 實測：**system 706→558 字、user 1842→1436 字**（合計 −21%）。
砍的全是**演出指導**，GAS 裁定的事實一個都沒動。

```
御主【在那一擊落定之後】衝上前接住、扶穩、把人拉開——演出這一拍：手掌撐住的力道、
對上的那一眼、一句短促的話。★御主是【在那一擊落定之後】才上前的：全程沒有挨到任何
攻擊、也沒有受傷；接住之後從者立刻重新站定，交鋒繼續由從者打。      〔120 字〕
  ↓
御主在那一擊落定之後上前接住、扶穩。★御主毫髮無傷，從者隨即重新站定。   〔33 字〕
```
...
| 舊 | 新 |
|---|---|
| 御主守在戰線側後方讀著戰況、適時下令，該退則果斷拉開距離——【不近身、不出手】＋兩條各自 40 字的補充規則 | 御主在戰線側後方讀戰況、下令。★御主在戰圈之外、毫髮無傷：能用的只有指令、魔力、令咒與卡上列出的魔術；體術只用在站位、閃開波及、接住從者 |
| ★戰後讓『X』依性格給一句主觀反應（破綻、對方是否現底牌、自身傷勢、對敵手評價皆可）… | ★收尾留一句『X』的反應，依其性格，每回合換個角度 |
| ⚠️靈基驟然高鳴——預兆正急速匯聚、殺意如實質般壓來…★收在這股「山雨欲來」的窒息壓迫 | ⚠️「X」的寶具正在匯聚、【尚未發動】。★收在這個當口 |
| ★【技能不喊名】直感、心眼、騎乘…（劍先到、一步搶先、魔力自劍身爆散），旁白不點名… | ★【喊得出名字的只有兩樣】寶具真名解放與令咒；其餘技能一律化成動作與畫面 |
| 演出這一擋的神聖、靜謐與絕對 | （刪；前面已經寫了發生什麼事） |

判準（下次再加句子時照這條走）：
- **留**：GAS 算出來的事實（誰、對誰、幾回合、傷勢、誰還活著、誰在場）、硬約束（御主毫髮無傷、字數、JSON 格式）。
- **砍**：分鏡指導（「演出這一拍：A、B、C」）、形容詞菜單（「神聖、靜謐與絕對」）、
  同一件事的第二次說明、以及**任何舉例**——例子就是把畫面先畫給模型看。

⚠ 順手修掉一個我自己前一輪造成的重複字：`miniSystem` 鐵律 1 變成「旁白**旁白**一律用…」。
⚠ 有九條探針在 grep 舊措辭，全部改成對新句子斷言（意思不變，只換字串）。

## 🏠 開局落點：玩家不生在敵人頭上（2026-09）

🐛 舊版 `spawnName` 寫死「偏好新都」（`validMapNames.find(n => /新都/.test(n))`），
而正典 5th 的 **伊莉雅絲菲爾＋赫拉克勒斯** 與 **間桐慎二＋吉爾伽美什（第 3 天到）** 就駐在冬木·新都——
**開局直接生在狂戰士頭上**。玩家「每次 solo 都會撞到伊利亞」有一半是這個，不只是陣容固定。

改成 `masterSpawnLoc_(validMapNames, war)`（`Seed_Rivals.gs`，跟 roster 放在同一檔）：
從 `MASTER_SPAWN_PREF_`（衛宮宅／冬木·商店街／穗群原學園／冬木·中央公園／未遠川河畔）挑第一個
**這場戰爭的陣容沒佔用**的點。偏好清單只是順位，安全來自 `taken` 那層——陣容改了落點自己會讓開。
實測 4th／5th 都落在 **衛宮宅**（兩場陣容都沒佔）。

混亂模式的地點是**召喚時當場洗的**，創角時算不出來，所以反過來做：`seedRivalsForGame_` 的 `locPool`
先把玩家已經站著的那一格濾掉（敵人讓開玩家，而不是玩家讓開敵人）。

探針 `spawn.js`（10 條，含注入退化：把舊的「偏好新都」塞回去會當場叫）：
正典 4th／5th ＋ 混亂連跑 5 次，都驗「開局那一格沒有任何敵人」。

## ⚔️ 創角頁「哪一場戰爭？」（2026-09）

混亂模式本來收在「▸ 自己設定」裡的 `#s-war` 下拉選單第三項——要先點開收合區、再拉選單，等於玩家看不到。
（2026-09 早先為了「一路按就可以直接開始」把整屏拿掉時，順手收進去的。）
改成名字/性別底下一排常駐按鈕：🏆 第五次（Fate/SN）｜⚔️ 第四次（Fate/Zero）｜🎲 混亂（每局都不一樣）。
值仍存 `#s-war`（改成 hidden），`setWarFromSelect_` 的讀法一字不動；`pickWar(v)` 只負責寫值＋上色。
外貌／身世／願望／禮裝還是收在「自己設定」裡。

⚠ **初次上色不能算「換了場次」**：`setWarFromSelect_` 會在換場次時清掉「扮演正典御主」並跳提示，
而 `showOnly_('step-name')` 進畫面就要把目前那場亮起來——所以 `pickWar` 比對舊值，只有真的變了才往下呼叫。
不然從「扮演正典御主」選完進創角頁，會立刻看到「換了場次，剛才選的正典御主取消了」。

⚠ **順手補了一個僥倖過關的掃描**：`check_wait` 是往上 30 行找等待指示，`chooseWorld` 的正典御主名單預取
本來靠鄰居的標記落在視窗內才過的——我插兩行就把它推出去、當場變紅。那支本來就是背景預取
（跟已登記的 `setWarFromSelect_` 是同一件事的另一個入口），正式登記進 `BACKGROUND` 並寫明理由。
`check_ui` 也把 `pickWar`／`pickOrigin` 納進入口清單，並加一格「選了混亂，`#s-war` 要真的是 chaos」
（已注入退化確認會叫）。

## 🎲 敵人陣容哪裡來（2026-09 玩家問「每次 solo 都會撞到伊利亞，真的有隨機嗎」）

**正典（4th／5th）的陣容是寫死的**，不是隨機壞了：`FATE_5TH_ROSTER`（`Seed_Rivals.gs`）＝原作八組，
伊莉雅絲菲爾-5th＋赫拉克勒斯固定在冬木·新都，每一局都一樣——正史就該長這樣。
**隨機只存在於 `war === 'chaos'`（🎲 混亂模式）**：御主池與從者池各自 Fisher-Yates 洗牌後配對，
地點也洗，前 3 組保證第 1 天就在、其餘 50% 機率延後到第 2~5 天登場。
⚠ 混亂模式收在創角頁「▸ 自己設定」的 `#s-war` 裡（`Index.html` 的 `<option value="chaos">`），
跟來源三分類同一個毛病——收起來玩家就看不到。

## 🎛️ 分模型（2026-09）

一顆主力模型不再打天下，三種用途各自指名（都可用指令碼屬性覆寫，常數在 `Core_Settings.gs`）：
...
- **補魔怎麼換過去**：前端解鎖分支的 `narrateExtra` 由 `{longForm:true}` 改成 `{longForm:true, lewd:true}` →
  `narrate_only` 讀 `userData.lewd` → `narrateWithState_` 的 `opts.lewd` 決定 `model`。
  ⚠ **模型常數要在函式裡讀**：第一版寫在 `narrate_only` 的 `const narrationText = …` 那行，
  而那行沒有縮排、`check_loadorder` 把它當成頂層宣告就叫了（載入當下 `LEWD_MODEL` 可能還是 undefined）。
  搬進 `narrateWithState_` 內、順手把那行的縮排補上。
- **尺度**：三支共用 `LEWD_EXPLICIT_`（`Router_Persona.gs`，單一真實來源）。2026-09 玩家「補魔我要很色很色」後加重成：
  「這一段放到最色，寫滿寫透：器官用本名，體液、聲音、氣味、溫度全部照實寫；衣服怎麼被解開、手先碰到哪裡、
  姿勢怎麼換、進出的節奏怎麼變，一個環節都攤開來寫；身體的反應寫具體——發抖、收緊、痙攣、失神，
  越到後面越失控。鏡頭貼著身體走，慢慢寫。」
  一起調的三個規格（**改篇幅就要連著改 token**，不然生到一半被截斷）：
  篇幅 500~600 → **800~1000 字**（三支）｜`longForm` 的 `max_tokens` 2000 → **3200**｜
  `lewd` 路徑的 `temperature` 0.85 → **1.0**（句子才不會一直落回同一組詞）。
  ⚠ 寫的是【筆法與涵蓋範圍】，不是動作菜單——玩家的老規矩「例子會固化畫面」在這裡一樣算數。

## 🎭 召喚頁「這是誰？」（2026-09）

玩家：「選英靈自創那邊要確實的把 fate／原創／動漫分類的明顯一點」。
來源三分類本來收在「▸ 指定職階與角色來源」的收合選單裡、預設完全原創——**玩家根本沒看到這排就按了生成**，
於是描述欄打一個既有作品的角色名，系統卻照「完全原創」那套去生。
改成描述欄底下一排三顆常駐按鈕（選中的發亮），值存在 hidden input `s-origin`（`summonByDesc` 讀法不變）：
✨ 我自己的（技能寶具自取）／🏆 Fate 角色（用正史的名字）／📺 別部作品（用原著招式名）。職階仍留在收合區（預設不限）。

## 🚫 三筆種子移出英靈殿（2026-09）

`美遊-Saber`／`小黑-Archer`／`伊莉雅-Caster` 從 `SEED_SERVANTS` 移除（原作設定是孩子，而補魔與鑑賞是露骨橋段）。
`CODEX_PERSONA_VER` v79→**v80**：升版號才會讓 `upgradeCodexPersonas_` 把既有英靈殿裡那三列一併刪掉
（它會刪「種子沒有、`SOURCE` 是 seed」的列）。

⚠ 移一筆種子要連著清乾淨的四處（這次每一處都真的踩到）：
`KANSHOU_LOCATION_TAGS_` 的老地方名單（水族館整條沒了）、`KANSHOU_HERO_HOME_` 的住處、
短名別名表、以及前端 `KC_SLEEP_HINTS_` 的住處徽章（`check_mirror` 當場抓到「前端多了 埃德費爾特宅邸」）。
**地點本身留著**——那是一處去得了的地方，跟誰住無關。
探針 `seed3.js` 有三條點名這三位（巧遇保底池／水族館的老地方名單／伊莉雅兩種靈基互斥），一併改掉；
同一位英靈的兩種靈基互斥仍由斯卡哈 Lancer/Assassin 那組守著。

## 🧬 AI 生成從者整修（2026-09）

玩家實測：自訂生成一位「如月尤菲」，卡片上**只有 2 個固有技能**，寶具卻是**克勞德的「超究武神霸斬」**。

**寶具為什麼會借別人的**：`originGuide_` 的三分類（fate／anime／original）**只管技能名，寶具名一個字都沒管**——
`★np：寶具名＋一句威能簡述` 對「這名字該從哪來」完全放生。玩家在描述欄打了「如月尤菲」，
模型認出那是 FF7 的 Yuffie Kisaragi（忍法·影分身其實還對），然後從同一部作品抓了最有名的那招，而那是克勞德的。
**它沒認錯作品，它認錯人**。修法：三分類各補一條 `np` 規則，並講明「同一部作品裡別人的招式屬於別人」。
沒填描述、只指定真名那條路（寫死的字串，吃不到三分類）也補上同一句。

| 來源 | 技能名 | 寶具名（新增） |
|---|---|---|
| fate | 沿用 Fate 原著技能名 | 沿用**這個角色本人**在 Fate 原著的寶具真名 |
| anime | 取該角色原著招牌招式 | 取**這個角色本人**的最強招式／絕技名 |
| original | 自取獨特招式名 | 自取，與既有作品的招式名自成一格 |

**技能為什麼只有 2 個**：真正的錨不是「固有技能 2~3 個」那句話，是**下面 JSON 範例只示範了 2 筆**——
模型照抄範例的形狀。範例還寫死 `"n":"自取的招式名"`，在 fate／anime 兩支跟上面的指示直接打架。
修法：範例改成 3 筆、技能名改中性佔位，指示 2~3→3~4，後端 `sanitizeSkills_(…, 3)`→`4`。
（對照：種子 17 位的固有技能 2~6 個、平均 3.6；種子還會把寶具本身也列成技能，AI 生成的不會，所以卡片看起來更薄。）

⚠ 探針 `creation.js` 中段把 `callGeminiAPI` 換成自己的樁（不記錄送進去的字），⑦ 這段要驗提示詞，
得先把 `probe.js` 的攔截樁裝回來——不裝的話攔截下來的東西永遠是空的，斷言會整排紅但原因跟提示詞無關。

## 🎭 御主卡不再被每回合複述（2026-09）

玩家實測：戰鬥每一回合都從「你挺直高挑的身形，神父般嚴肅端莊的面容…世俗妄想…聖經章節的祈禱姿態」開場。
對照御主卡就知道 AI 在**逐格唸卡**：挺直高挑＝命格特徵、神父般嚴肅端莊＝個性第1格、世俗妄想＝第2格、
晨禱與聖經章節＝第3格。這是**紅線②（禁直述個性字面）在實戰中被違反**。

根源：`servantCard_` 有 `performanceNote_` 那句「上面的卡是【內化用的素材】」框住，**`masterCard_` 沒有**，
結尾還寫著「有神態與台詞」——等於邀 AI 每回合描寫御主的外觀。
（同一個形狀 2026-07 在萌點上踩過一次：「這張卡幾乎每次敘述都帶上，AI 手上唯一的素材就這句，
連續幾回合就反覆同一個動作」，見 CODE_NOTES `masterCard_`。萌點退休後，外貌＋個性接手變成新的跳針來源。）

修法（`Router_Persona.gs` `masterCard_` 尾句）：
...
**怎麼死依陣營走**（2026-09 玩家「不一定要殺玩家，可以反過來一直榨乾玩家」）：結果一律是御主敗北，
死法查 `SEAL_BACKLASH_`（`Router_Bond.gs`，上面的先命中先算、最後一列是保底；加一列就多一種死法）：

| 陣營含 | 結局 | 事實句 |
|---|---|---|
| 惡 | 榨乾 | 沒有停手——反過來把御主的魔力與體力一起抽乾，御主在一波接一波的高潮裡斷了氣 |
| 混沌 | 榨乾 | 索性順著這股失控要到底——一路榨到御主的魔力見底、心跳跟著停下 |
| （其餘） | 了結 | 積壓的恨意引爆，殺了御主 |

`report.backlash` 帶著結局下傳，前端據此選老虎道場的講評鍵（`seal_backlash`／新增的 `seal_drained`）。
機制沒動：HP 歸 0、假夢、`sealBacklash` 旗標都照舊。

**三支的分工（玩家定案）**——`令咒＝雙方都被強化，跟羈絆無關；60 只分抗拒還是迎合`：

| | 誰在主導 | 提示詞給的事實 |
|---|---|---|
| ① 令咒·羈絆 <60 | 令咒壓著 | 內心一路抗拒，身體卻不受控制地迎合下去，御主就這樣得逞 |
| ② 令咒·羈絆 ≥60 | 令咒推著、她自願 | 迎上來，在令咒的加持裡比平常更放得開、更享受 |
| ③ 💧 補魔（無令咒） | 從者掌控 | 節奏由從者掌控（英靈強韌、何時攀頂自己決定）；御主剛燒過迴路，最後是癱軟的那一方；從者在魔力上飽足，肉體這一側還留著沒盡興的餘裕 |

①②共用 `sealManaFx`：「令咒同時強化了兩邊：敏感度被推到遠超常態，御主的性能力也被拉高、承接得住。高潮由御主的動作引發，來得又多又失控。」
⚠ 舊版把「抗拒翻成索求」寫進這段共用事實，於是②一邊說「本來就願意」一邊叫 AI 演抗拒——自己打架。抗拒只屬於①。
...
玩家：「把SOLO 也整理」。把 solo **玩家看得到的字**全撈出來讀了一遍（確認框／alert／tooltip／
面板小字／教學／創角流程／七支 Router＋Account 的 `message:`），改了 354 行。

**solo 跟鑑賞的差別**：令咒／靈基／御主／絕對命令這些 Fate 詞是這一軌的味道，**不是 AI 感，要留**。
AI 感是這四種：

| 形狀 | 舊 | 新 |
|---|---|---|
| **確認框寫成說明書**（條列後果＋括號補充） | 💧 補魔：御主硬擠魔術迴路，將共用魔力池一口氣回滿。⚠ 代價【永久】：…（迴路↓會讓魔力池上限縮小、回魔變慢、禮裝變弱）。這是燒迴路換契約的孤注一擲…（想省代價：靠靈脈/陣地/休息自然回魔。注意：池見底會被動燃血、缺口÷2 由御主血量獨自承擔。）確定補魔？ | 💧 補魔：硬擠迴路，把魔力池灌回滿。代價是永久的：血上限 −N、魔術迴路 −M 條。補嗎？ |
| **tooltip 寫成段落** | 與盟友共處／共濟魔力：增進羈絆。每位盟友每日一次、耗 1AP。此地若有未結盟的敵從者，共處時門戶大開、可能被趁隙突襲。 | 跟盟友相處，增進羈絆。每天一次、1AP |
| **「AI」跟系統詞漏到玩家眼前**（AI 敘述會依此描寫／連線受阻／未果／查無／參數／御主列／殘列） | 連線失敗，整備未果。／查無御主／御主列已不存在（可能剛被清理） | 連不上。／找不到你的角色／你的角色不見了，重新登入看看 |
| **文言腔的錯誤訊息** | 御主的魔力儲備已然充盈，毋須補魔（免付燒蝕之代價）。／輿圖之上查無此地，無路可達。／行動點用完了，從者也需要喘口氣。先歇息，恢復了再戰。 | 魔力是滿的，不用補。／地圖上沒有這個地方。／行動力用完了。 |

幾個決定：
- **數字只留在玩家要據此做決定的地方**（補魔的永久代價、令咒還剩幾道、海怪每小時抽幾魔、破戒的 35%）。
  說明性的公式（「2 點血換 1 點魔，血剩 1 就停」「缺口÷2」）全拿掉——那是規則書，不是按鈕。
- **「行動力不夠，沒辦法XX。先休息，恢復了再來。」二十處統一成「行動力不夠。」**（`needRest` 旗標會讓前端自己亮休息鈕，字裡不用再教）
- **「XX失敗。」／「連線失敗，XX未果。」統一成「沒XX成。」／「連不上。」**
- 敵從者在場的三種警告（睡覺／補魔／療傷／相伴）從四行縮成一句：「這裡有敵從者「X」，補魔時很容易被偷襲。還是要補？」
- 教學留著，但拿掉「姿態只影響敘事怎麼寫你，不改變勝負機率」這種在解釋實作的句子。

### 第二輪：事實講完就停（2026-09）

玩家把教學裡兩句挑出來：「那是代價，不是挨打」「休越久，外面的世界也走越多」——
「這 2 句都是多餘的 AI 感，說結論就好」。這是第五種形狀，前一輪的四種都沒抓到它：
**句子本身口語，但事實講完之後又掛了一截評語／提醒／安慰／建議**。

| 尾巴的種類 | 舊 | 新 |
|---|---|---|
| 評語 | 魔力不夠又要放寶具，會燒你的命去墊。那是代價，不是挨打。 | …會燒你的命去墊。 |
| 提醒 | 每休息 1 小時回 2 點行動力，也回血回魔。休越久，外面的世界也走越多。 | 每休息 1 小時回 2 點行動力，也回血回魔。 |
| 建議 | 你太虛弱了，再燒會死。休息，或用令咒脫離。／魔力在掉。找靈脈、設陣地、把出力調低，或補魔。 | 你太虛弱了，再燒會死。／魔力在掉。 |
| 安慰 | 離開冬木？進度會留著。／連不上，沒移動。再試一次。 | 離開冬木？／連不上，沒移動。 |
| 教學裡的忠告 | 留給真的危急的時候／不用手動關／別讓它扣到底／按下去前都會告訴你代價 | 整句拿掉 |

寫法：**寫完那個 GAS 判定出來的事實，句號，停。** 下一步要做什麼，按鈕本身會說
（`needRest`→休息鈕、`canForceSeal`→燃令咒鈕、`needMana`→補魔鈕）；後果玩家看到數字自己會算。
「再試一次」這種也算——連不上就是連不上，玩家知道要再按。
`行動力不夠。`／`少了東西。`／`沒建成。`／`出錯了。` 這些短到像沒寫完，但這就是全部的事實。

⚠ 後端訊息裡**不能寫死性別代名詞**——`check_pronoun` 當場抓到我寫的「她會叫你」「先給她一個真名」
（御主與從者的性別都是資料），改成「對方會叫你」「先取一個真名」。
⚠ 探針靠這些子字串判斷結果，改寫時保住了：`用盡`／`今天已經`／`還沒熟到`／`早就摸清`／`同一個地方`／
`沒有這種交涉`／`只能有一種姿態`／`已經存在`／`最多|上限`。
