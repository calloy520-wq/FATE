...
| 🏃 脫離 | 一律可用 | 追擊擦傷＋**真名仍在弦上**＋耗 1 AP |

⚠ **脫離的代價是玩家實測抓出來補的**：第一版是「0 傷、0 代價」，玩家一句
「無損脫離……大家一定選這個吧？需要一點代價」就把問題點破了——
**零傷又零代價的選項不是選擇，是正確答案**。補了三層，而且三層都是自然後果、不是懲罰稅：
1. **他的真名並沒有放出去** → `clearNpTelegraph_` 不執行，仍蓄在弦上，下次碰面照樣要面對（結構性的代價，不是扣血）
2. **轉身就走會露出破綻** → 被順勢咬一記普攻（實測 7~8/10 命中、傷害 7~13）
3. **這一拍就是這回合的動作** → 耗 1 AP
