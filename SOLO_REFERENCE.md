...
- **📜 本場戰記（里程碑回顧）**：獨立「戰記」表(自動建)，schema `[game_id, 日, 時, 內容]`。`logWarEvent_(gameId, text)`(Router)只記 solo 局(g_)、附遊戲內 day/hour。接線點：召喚開戰(actionSummonServant)、玩家令咒(actionFateBattle useSeal／actionUseSeal repair/mana/escape)、敵令咒脫離(fateStrike_)、從者擊破(雙方)、令咒透支倒數＋暗處養不起/廝殺(worldTick_)、斬首擊殺御主、結盟/破盟(actionProposeAlliance/actionBreakAlliance/breakStaleAlliances_)、奪杯/敗北。`actionWarChronicle`(action `war_chronicle`)依玩家當前 game_id 撈回。前端 `openWarChronicle`(Script)→ 抽屜「📜 本場戰記」(Index.html action-drawer)，依日分段呈現。用 game_id 分流，新局自然只顯示本場。
