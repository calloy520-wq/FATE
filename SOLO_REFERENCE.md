...
**追加·風景照(玩家「那我不能拍貓貓狗狗嗎?」)**：拍照不再要求有同伴在場。拍攝對象三態：①輸入框先打字再按📷(`photoIntent`·清洗+60字截斷)且沒點名同伴→**風景/生活照**(AI自由入鏡街貓/狗兒/鳥雀/光影，玩家指定的東西當主角；同伴在場可亂入鏡頭邊角)；②沒打字有同伴→拍同伴(原行為，intent若點名同伴也走這)；③沒打字沒同伴→拍風景(原本是「拍照落空」擋下，已移除該擋)。風景照人物欄記「風景」(相簿自動長出「風景」篩選分頁)、緞帶固定草綠`#7a9a6a`。前端`kanshouTakePhoto`改讀輸入框值當intent並清空，`send()`加第21尾參`photoIntent`。
