...
| set_overload | actionSetOverload | 🔥**灌魔超載檔位**(玩家設定＋/EX規格外寶具本戰要灌多兇)：讀該從者 SIX JSON 的`寶具`階算 `rankCap=npOverloadCap_`(≤1.0 則拒·非規格外寶具無旋鈕)，存 MEMORY`【超載】<mul>`(1.0–2.0)。未設＝拉滿。詳見 §10「灌魔超載」。前端寶具膠囊下旋鈕 `setNpOverload`(樂觀更新)。免費即時。 |
...
- **🛡️ 防禦 fx 格式表 `DEF_FX_`＋防禦規模表 `DEF_SCALE_`(2026-07 資料驅動·對稱攻擊側)**：`Engine_Fate.gs`。
  ...
  - **`DEF_SCALE_`＝防禦規模表**(`summon_horror/wall_def→對城`、`territory→對軍`，優先序＝陣列順序)：`npDefScale_(c)` 掃表回防禦規模，餵 `NP_SCALE_MATRIX[攻][防]`(對稱 `npAtkScale_`)。
...
- **🔥 灌魔加乘·灌魔超載(2026-07·規格外寶具才有旋鈕)**：`npOverloadCap_(寶具階)`(Engine_Fate·資料驅動數 '+'／EX)→無＋=1.0(定額·不可調)、＋=1.5、＋＋/EX=2.0，對應原作「A++對城劍隨輸入魔力提高威力」，跨名冊通用(Excalibur/Enuma/Ea)非某人專利。**玩家手動檔位(2026-07 定案·非純 auto)**：`【超載】<mul>` 存從者 MEMORY(`getOverloadTier_/setOverloadTier_/clearOverloadTier_`·Core_Settings·夾 1.0–2.0)，前端寶具膠囊下 `npOverloadSelectorHtml` 給「基礎/×1.5/×2.0」鈕(`setNpOverload`→`set_overload`·樂觀更新)，**未設＝拉滿至階級上限**(想省魔力才調降·有些玩家不愛灌)。`Router_Battle` NP 段 **pour**：僅【全開100%】的＋/EX寶具，`chosenCap = min(玩家檔位, rankCap)`，把御主餘裕魔力(MP+焚血+過充額度)超載灌到 chosenCap → 需額外「底費×(cap-1)/(rankCap-1)」比例的魔力·不足按比例，倍率 `atkC.npOverloadMul` 壓在 combatant 上(不改簽名·比照 output/npChoice)、`resolveFateBattle_` 寶具解放段 `wRelease` 才乘(對手反殺不吃)。戰報 `report.overload`(>1.25 前端橫幅🔥×N)。**敵方 NP 不超載**(暫·敵幾乎無餘魔·可續作)。get_tags 給 `overloadCap`(階級可調上限·>1.0 才顯旋鈕)＋`overloadTier`(玩家當前設定)。
...
【試煉】N(god_hand命數) 【寶具選】N(多寶具英靈解放哪個·set_np_choice) 【寶具預告】1(敵蓄勢·getNpTelegraph_) 【過充】N(補魔存的無償超載額度·一次性·getOvercharge_/set/clear) 【換裝】文字(玩家自訂從者服裝·換衣不換人·getOutfit_/set/clear) 【超載】<mul>(玩家設定的灌魔超載檔位1.0/1.5/2.0·set_overload·未設＝拉滿至階級上限·getOverloadTier_/set/clear)　※【路線】route／【史】firedPins 已隨正典插針退役·無用遺留
...
- **從者能力卡四類分區(2026-07 `buildSvCard`)**：`abilitySection(icon,label,pills,col)` 統一渲染四段——**💥寶具**(`npPillsHtml`·多寶具常駐可點選定`pickNp`＋規格外寶具附`npOverloadSelectorHtml`🔥超載旋鈕`setNpOverload`)／**⚡主動**(可開關耗魔技·`showActiveSkillToggle`·`ACTIVE_FX_JS`={burst,str_up,projection})／**✨特殊·可選**(要自己挑的·**查 `SELECTABLE_FX` 註冊表·資料驅動**：魔境的智慧／皇帝特權`openMageRealmPicker`＋原初符文·盧恩`openRunePicker`；加第三種＝表加一列{ready,suffix,onclick}＋寫 picker·分桶迴圈不動)／**🛡️被動**(常駐技＋特性·點看說明；含恩奇都變容 fx `shapeshift`＋synergy 的亮/暗全盛標籤·不可切)。⚡主動同理查 `ACTIVE_FX_JS` 表。右上 `?` 鈕→`showAbilityHelp()`四類教學。
- **令咒→寶具(2026-07)**：`renderWarActions` 算 `myActiveHasNp`(從者有寶具)；❖令咒鈕→`servantStrike(name,hasNp,true)` **燃令咒·全力強放選定寶具**(必中＋×1.5＋就算魔力見底照放·bypass 出力/魔力閘)；💥寶具鈕＝正常解放(要出力100%＋魔力夠)；**出戰＝普通攻擊·不放寶具**。放哪把一律由膠囊 `pickNp` 決定。
