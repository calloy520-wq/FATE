...
- `resolveFateBattle_(atk,def,opts)`：單次交手裁決。處理的 fx 標籤：
  ...
  - **🗡️ Avalon-Saber 理想鄉(2026-07·Mystic_Code injectMysticBuff_)**：御主持「全世界之鞘 Avalon」禮裝 ＋ 從者為【阿爾托莉雅】→ 注入 `avalon_saber`(MC_COMBAT_ npDefMul 0.20·承受寶具傷近乎歸零·硬接敵真名)＋`regen`(時回)。非阿爾托莉雅持 Avalon→僅一般 avalon(0.82)。名字比對 `/阿爾托莉雅/`＋cls Saber。這是寶具預告的天生解法。
