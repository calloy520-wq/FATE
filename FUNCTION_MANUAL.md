...
- `KANSHOU_NOTED_TAG_`／`KANSHOU_NOTED_SEP_`／`KANSHOU_NOTED_CAP_`／`KANSHOU_NOTED_LEN_`／`KANSHOU_KNOWN_TIERS_`／`kanshouKnownTier_(metCount)`／`kanshouKnownOfYou_(memory)`（2026-09 新增，`gas/Gallery.gs`）— 「她眼中的你」知情度。`kanshouKnownTier_` 查 `KANSHOU_FAMILIAR_TIERS_`（**與相處基調共用同一條軸、不另立門檻**）回初識/混熟/老交情；`kanshouKnownOfYou_` 回 `{tier, rule, noted[]}`，`rule` 來自 `KANSHOU_KNOWN_TIERS_`（**只講「她知道多少」，態度是 `KANSHOU_RAPPORT_TONE_` 的事，兩張表不重疊**）、`noted` 是 `【眼中的你】` 標記以 `／` 切開的清單（**分隔符不可用 `｜`/`【】`**，`makeTextTag_` 會剝掉）。掛在 `partyDetailsArr` 每人自己那行（`pKnownStr`）；各段界線由 `_knownRuleStr_` 集中印一次，**只印這回合在場者真的落到的那幾段**（`_knownTiersSeen_`）。
