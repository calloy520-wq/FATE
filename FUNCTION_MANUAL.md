...
- `KANSHOU_MET_COUNT_TAG_`／`KANSHOU_FAMILIAR_TIERS_`／`KANSHOU_RAPPORT_BOND_TIERS_`／`KANSHOU_RAPPORT_TONE_`／`kanshouRapportTone_(bond, metCount)`（2026-07 新增，`gas/Gallery.gs`）— 相處基調 2D 表。`【相處】N` 每個對話回合對每位在場者 +1（`kanshouTimeJumped_` 時不加）；查 好感4段 × 熟悉度3段 → 一句具體演法，查無回空字串（刻意留白＝那一格不給指令）。掛在 `partyDetailsArr` 每人自己那行，取代舊的 1D `pTierToneStr`。
