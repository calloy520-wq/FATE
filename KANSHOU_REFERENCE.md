...
- **她也能主動邀約**（`kanshouPromiseOffer_`，2026-07 三度改版·玩家「AI亂寫萌點…也想要當好感卡39之類的時候GAS主動發出邀約」，順勢把這塊也從AI手上拔掉）：舊版靠 AI 自己判斷要不要開口約(`promise_proposal`欄)，玩家實測常常不開口或亂約。**改GAS依好感數值直接判定，不是機率、卡關就一定邀**：每回合掃在場同伴，`kanshouRelChatCeiling_(bond)` 剛好卡在天花板(bond===39/59/79，代表純聊天已經爬不上去了)且該同伴**沒有**尚未赴的【約定】(`kanshouGetPromise_`) → 就跳邀請泡泡；一次只邀一位(同回合跟玩家發起的「同去」互斥，避免兩個提議泡泡打架)。地點/時段由 **`kanshouPickDate_(heroName, day, bond)`** 依當前好感梯度挑（見下方「約會地點好感分級」），同一天同一人固定挑到同一個地方（hash動day+heroName，不會每回合亂跳）。落地完全沿用舊有管線：回傳 `promiseProposal` → 前端跳邀請泡泡（`kanshouAcceptPromise`）→ 玩家按「邀請她」帶 `opts.promiseAccept` **直接落地【約定】**(GAS已裁定該邀、必定成立，不必真的走`kanshouPromiseMeet_`那套有拒絕機率的player-initiated流程)。婉拒＝`kanshouDeclinePromise`（軟性婉拒、下回合條件仍成立會再跳）。與玩家發起共用同一套赴約結算。**⚖️ 玩家裁定：她約完就走也照樣成立**——約定契約只差玩家點頭，她在不在場不影響寫入；差別只在敘事（在場演她的反應／已離場演玩家記下這個約）。別把「玩家發起需對方在場」的規則錯套到這裡。
  ...
  **🐛→✅ 2026-07 四度改版·乾脆整個框成玩家自己的念頭**（玩家釐清設計意圖：「當回合AI敘述完全沒鋪墊邀約，突然跳出『她想約你』很怪；應該是玩家自己意識到好感度到頂了、要用約會突破」）：上一版的「她主動開口」修法只是消歧義的過渡，這次直接定案——泡泡文案／按鈕／送給AI的合成訊息全部改成「玩家自己起心動念、開口邀她」的口吻，跟`kanshouInviteCohabit`(玩家自己隨時可發起的同居邀請)同一種口吻，不再是「她開口、你同意」。**GAS 判定時機/地點/必定成功完全不變**，只是包裝方向整個反過來——泡泡：「感覺和『她』可以更靠近一步了——要不要約她…」／按鈕「邀請她」；`kanshouAcceptPromise`合成訊息：「（我鼓起勇氣，開口約『她』改天見面——她點頭答應了…）」。`kanshouCohabitOffer_`/`kanshouAcceptCohabit`同款修法（泡泡「要不要邀請她搬來一起住？」／訊息「我鼓起勇氣，邀『她』搬來一起住——她答應了」）。
