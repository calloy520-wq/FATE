...
- **🎀 小道具**（2026-07 新增·根治「幫她戴貓耳朵過幾輪就忘記」的機制保證版）：`KANSHOU_PROPS_`(資料驅動·**2026-07移除內建「跳蛋」後現為空陣列**，僅保留擴充掛勾、全面改走下方玩家自訂道具)＋`KANSHOU_PROP_LEVELS_`(關閉/微弱/中等/強勁)＋**`KANSHOU_PROP_LEVEL_FX_`(2026-07新增：三階→具體行為指令查表，運作中的道具另拉★行)**，存該同伴列 MEMORY `【小道具】id1:強度1,id2:強度2,...`（`kanshouGetProps_`/`SetProps_`/`ToggleProps_`，**多件可同時裝備**，逗號分隔比照【性格鎖】同款寫法——2026-07 玩家「其他道具怎麼辦，一次只能一種？」後從單槽改多槽）。**玩家UI手動裝備/移除/調強度，GAS直接寫，不靠AI自己判斷該不該記**——跟`outfit_change`(現appearance_extras)那套「AI自己判斷有沒有變化」的路徑刻意分開，這條是機制保證。
  ...
  - **🌀 催眠指令(ignoreBond)獨立入口(2026-07 二度改版·「催眠的和新道具要確實分開成兩種」)**：一開始把「無視好感」做成一般自訂道具表單裡的一個勾選框，玩家後來覺得混在一起不夠清楚，**改成完全獨立的按鈕/面板/action**：
    ...
    - 存進`【自訂道具】name:hasIntensity:part:ignoreBond`(4欄，第4欄向下相容舊3欄格式)。**三階＝作用點不同，不是強度不同**（2026-07 二度定案，玩家收集催眠題材討論後重新推導；先有一版「三階都套同一句更強烈」被否，再有一版「越強越恍惚→強勁＝判若兩人」也被否）。舊版的病灶：把三條軸綁在同一根拉桿上一起拉，最高階＝三軸全滿＝判若兩人，同時犯兩個錯——①人格被關掉（紅線②的反面、二創圈罵最兇的「キャラ崩壊」）②她沒反應就沒有互動性，**最高階反而最無聊**。

      三條軸：**軸1 意識**(她知不知道自己不對勁)＝永遠清醒，不隨階數下降；**軸2 行為**(她能不能拒絕)＝只有這條隨階數上升；**軸3 常識**(這件事在她眼裡算不算異常)＝最高階動的是這條，不是人格。現行三階：
      - **微弱**＝軸2 動一點點。持續的傾向而非可數事件（玩家：「只漏一次感覺好怪…我要的是輕微、基本沒有要影響」），她照常做自己、不覺得有異，只是身體比平常慢半拍地不配合她的意思。
      - **中等**＝軸2 拉滿＋軸1 清醒＝**明知被操縱卻無法反抗**（玩家指名的核心爽點）。她知道自己不對勁、也知道是玩家搞的，嘴上照樣質問抗拒罵人，身體每次還是照做——意志完整、就是擋不住，落差本身就是戲。
      - **強勁**＝**常識改寫**（術語上這才是「常識改變」；舊版那種判若兩人在分類上屬於「洗腦」）。個性/態度/敵意全部原封不動，只拔掉『這件事很奇怪』這個判斷，她一臉理所當然照暗示生活、被追問還會理直氣壯反駁，**旁人仍照常人反應**（單體催眠＋路人正常反應才有對照組；大家常識都被改反而沒有落差）。

      ⚠ `p.name` 是玩家自由輸入的暗示句，措辭必須通用、不可假設是命令句。
      ⚠ **連動修正**：`PROMPT_REL`【角色一致性】原本的催眠例外是「★催眠暗示生效中例外，可判若兩人。」——那正是被否掉的舊設計，已改成「人格照樣不崩：被繞過的只有拒絕得了與覺得這很奇怪，個性/態度/敵意原封不動照跑，不可寫成判若兩人」。改三階時這句必須同步，否則兩處互相矛盾。啟動中(有強度且非關閉)的ignoreBond道具，`pPropStr`依這三階注入對應那一句——且**同步在USER prompt的【親密尺度五階】規則本體加註例外**(`佩戴中的『催眠暗示』類道具例外...不受此天花板限制`)，避免兩條指示互相矛盾(親密尺度五階本來就自稱「最高優先·凌駕...」，沒註明例外的話AI可能誤判優先權蓋掉這個效果)。面板顯示：等級鈕旁/道具名稱旁加`🌀無視好感`小標籤(`ignoreBondHint`)提示玩家哪些道具是這一類。**⚠ 2026-07 補「其他人不該知道」**（玩家「我對a催眠b不能知道啊，雖然ai知道但他不能敘述出來」）：提示詞把同地點所有在場者的卡片一次全塞給AI，AI技術上「看得到」A身上的催眠狀態，原本沒有東西明講「B不該對此有反應」——本來是靠已拔掉的醋意暗流機制去戳它演出來，機制拔了不代表AI不會自己聯想到。已在`_ignoreBondLines`結尾補一句：「★這是只有她自己感覺得到的私密效果，除非外顯到旁人一看就懂，否則在場其他人不知情、不該對此有反應或評論」——只在該同伴身上真的有ignoreBond效果生效時才附加，其餘回合零成本。**⚠ 2026-07 補代詞代入規則**（玩家問「我打『你現在想狠狠肏我』，AI看起來是他要肏我還是我想操她？」）：暗示內容是玩家自由輸入的文字，格式五花八門(第一/第二/第三人稱都可能出現)，提示詞原本沒明講「你/我」這類代詞該怎麼代入場景裡的角色，靠AI自己從上下文猜有風險。已在`_ignoreBondLines`結尾再加一句：「★暗示內容裡若出現「你/妳」「我」等代詞，你/妳＝她本人、我＝玩家，依此代入解讀，不要弄反」——把代換規則寫死。前端`kanshouOpenHypnosis`面板的輸入框上方同步加一行提示：「💡 用「妳」指她本人、「我」指你自己來寫...別寫反了」，讓玩家下指令時就用對的視角。**⚠ 2026-07「催眠太強」曾加過升階閘門、同月又整個拿掉**（玩家「有點太繁雜，改回去隨意切換強度、不限制慢慢提升」）：一度限制`ignoreBond`類只能一階一階往上升(跳兩階回`success:false`)、前端同步把跳階鈕標`disabled`。實玩後的結論是**節奏感沒賺到、繁瑣感先來了**——要推到想要的強度得多按兩次，中途每一階還各算一次狀態變更。**閘門已移除，強度隨意切換**；`KANSHOU_PROP_LEVELS_` 因此純粹是三段強度，不再是一把有方向的尺。

      **🗑 「悄悄解除」上線同日就整個移除**（玩家：「AI 能演這麼細緻嗎？不告訴她是不是不要了，解除就是演吧」）。
      拆開看就知道它演不出來：**強勁階與「已解除·她不知情」的外顯行為完全一樣**（照樣順從、照樣不覺得奇怪），差別只在「這是不是她自己的意思」，而提示詞還明講她不知道——那段期間敘事上沒有任何東西可以不同。更糟的是小模型被要求「知道但不准表現出來」時，最常見的失敗是**漏出來**（寫成「她隱約覺得哪裡不太一樣」），反而把橋段搞砸。
      而那個定番的爽點在**宣告的那一刻**，不在偷偷解除的那段期間；照催眠學那條原理（解除＝信念），**宣告本身就是解除**——「其實從剛才那一刻起妳就沒被影響了」這句話是真是假沒人驗證得了，包括她。所以那筆帳記了也沒人看得到。已把宣告直接寫進 **⏹解除** 那一步的動作敘述，一顆按鈕、零額外狀態。
      向下相容：`KANSHOU_HYPNO_RELEASED_='已解除'` 常數保留，只用來**認出舊存檔殘值**——`_ignoreBondLines` 與 `_kanshouHypnosisActive_` 的 filter 都排除它（否則會掉進 else 分支被當成「強勁」演），面板顯示成「未施展」。
      ⚠ **單一入口**：同伴抽屜的快調面板（`_kcRenderDrawer_`/`kcQuickSetProp`）已把 ignoreBond 過濾掉——那條路沒有「⏹解除＝當面告訴她」那段敘述，留著等於開一道能靜靜把暗示關掉、什麼都不演的後門。

      **現行操作只剩一條判準：跟她說話的會演，動手腳的不會。**
      · **觸發 AI**：🌀施展新的一句／⏹解除（＝當面宣告並撤下）
      · **樂觀更新、靜默**：調強度（`kcHypnoQuietSet_`，比照 `kcQuickSetProp`：立即改本地快取＋重繪、背景送出、失敗還原並提醒）
      靜默不必擔心 AI 不知道：`_ignoreBondLines` 每回合重建，下一句話就照新強度演。升階閘門也已移除，強度隨意切換。

      🧹 **拿同一把尺掃過全設計的結果**（2026-07 玩家「照這個思路去看看有沒有多餘的設計」）：
      - **砍掉**：`★【性別配對】` 非女女的那一句。原本會輸出「${在場所有人的名字}：依各自實際性別自然互動」——
        **既沒給事實（連誰是什麼性別都沒講）也沒給約束**，在場 5 人時還把 5 個名字全列出來講這句廢話。
        只留真正帶限制的女女那條（禁插入式、以手指/舌頭/器物替代）；沒有女女配對時整個區塊不輸出。
      - **驗過沒問題、別再懷疑它們**：
        · `★【動筆前最後確認】` 看似跟【在場驗證鐵律】重複，其實是刻意放在整份 prompt **最後一行**的 recency 錨點；
          完整規則只在【在場驗證鐵律】講一次（canonical），程式碼註解本來就寫明了。
        · 好感欄後面掛的四個尾綴（`pMemStr`/`pAtCeilingStr`/`pChillStr`/`pTierToneStr`）各自條件不同、內容不重疊，
          且 `pTierToneStr` 刻意只在低階出現（中高階不加，免得畫蛇添足）。
        · 六張內容表（橋段16／地點橋段5／節慶6／同居5／地點標籤8／撲空文案5）**每一條都有觸發路徑**，沒有死內容。
        · `KANSHOU_LOCATION_ACTIVITY_` 用 hash(名字+日+地點) 決定性挑變體：零持久化、同一天同一人同地恆定，是真的紋理。
      - **留給玩家判斷的兩個**（不是 bug，是取捨）：
        · **小道具快調抽屜**（`_kcRenderDrawer_`/`kcQuickSetProp`，畫面右緣常駐的 ◀ 標籤）：跟同伴卡的「🎀 小道具」面板
          功能重疊，而內建道具表已空、催眠也已排除，所以它只在「玩家自訂了強度可調的道具且該同伴在場」時才有內容，
          其餘時候顯示「在場沒有可調整強度的道具」。**兩道門**這件事本身在這個專案已經咬過人（催眠那次）。
        · **「她主動」的觸發率**：每回合只擲一次（取在場最高好感、再除以 `1+未赴約數`，且一天最多一次）。
          好感 100＝每回合 1.4%＝平均 71 回合一次（約一個完整遊戲日）；手上有 2 個未赴的約時再除以 3。
      - **一個數字**：好感天花板相關的規則分散在 4 個區塊、共約 504 字，佔整份 user prompt 的 1/10。
        每一塊都是實際踩到漏洞後補的（命令句繞過、多人共用同階、人格線性軟化…），合併有回歸風險，暫不動，但記著這筆帳。

      🧭 **這一輪留下的判準（值得帶去看別的設計）**：一個狀態如果**沒有任何一回合能產生可觀察的差別**，它就不是「細緻」而是「不存在」——只是多花提示詞、多一個按鈕、多一條要維護的分支。問法很簡單：**「開著它跟關著它，這一回合的敘事會不一樣嗎？」** 答不出來就砍。

      **🔎 2026-07「小道具+催眠整體檢查」再抓到 5 個**（都已修）：
      1. **催眠被當成穿戴在身上的物件**。`pPropStr` 原本把催眠跟實體道具混在同一份「佩戴道具:…——這是既定事實，narration須自然反映其存在」清單裡；身上沒有任何實體道具時也照樣輸出「佩戴道具:妳現在很想靠近我(強勁)」，AI 容易寫成她戴著寫有那句話的東西。關閉時更荒謬：整段只剩「（強度關閉≠取下，仍配戴在身上）」在講一句已解除的暗示。**已把兩份清單分家**：`_wornStr` 只列 `!ignoreBond`，`_hypStr` 只走★行，兩邊都空就整段不輸出。**⚠ 同月追加（玩家「佩戴道具<<<這開頭也太怪」）**：前綴本身也把「所有道具都穿戴在身上」寫死了，而道具名稱是玩家自由輸入的（繩子/椅子/遙控器都可能），`佩戴道具:繩子` 讀起來就是錯的。**前綴改成跟 UI 同一個詞「小道具:」**（按鈕/面板都叫「🎀 小道具」，單一詞彙），是否穿戴交給選填的 `戴在X` 去講；關閉那句同步改成「強度關閉≠拿走，東西還在她身上/手邊，只是暫時沒在作用」。
      2. **「關閉」的催眠佔著裝備名額不放**（`KANSHOU_PROP_EQUIP_CAP_`＝5）。攢滿 5 句關閉的暗示之後就再也施展不了第 6 句，玩家完全看不出為什麼。既然關閉對 AI 已經完全隱形，它跟「沒施展」本來就是同一件事：**⏹解除 改送 `level=''` 直接從她身上撤下**，名額當場放回來。
      3. **催眠面板只列「她身上正掛著的」**，解除後那句暗示就從畫面消失、要再用只能重打。**改成比照 `kanshouOpenProps` 列整份目錄**（`_kcCustomProps` 裡 `ignoreBond` 的），沒施展的顯示「未施展」，隨時可再施展在任何人身上。
      4. **好感未達門檻時，一般道具的裝備/強度鈕照樣長出來**，點下去才被後端打回票（本專案犯過好幾次的「永遠不會成功的按鈕仍長出來」）。門檻/上限由 `kanshou_companions` 下傳 **`propBond`/`propCap`**（`KANSHOU_PROP_EQUIP_BOND_`/`_CAP_`），前端不寫死數字。⚠ 只鎖「裝上去/調強度」，**移除一律不鎖**——後端移除路徑本來就不看好感。
      5. **`actionKanshouCastHypnosis` 兩次分開寫入**（先玩家列目錄、再目標列施展），中途失敗留下「目錄建了但沒施展」的半套資料——而 `actionKanshouAddCustomProp` 早就為同一個理由改成「全程只改記憶體、確定後單次整表寫回」。**同一種病一起治**，順便 2 次 round-trip 併成 1 次。
      另外：🗑刪除 對催眠的確認文案改成明講「不會有任何解除的敘述」，避免玩家拿刪除當解除用（那條路是靜默的）。

      **🎀 小道具三階也補上具體行為指令**（2026-07 玩家「有時候小道具都沒有生效的感覺」）。實測攤開提示詞：她那一列 260 字、切成 8 段，「小道具:」出現在第 185 字（71% 處）；整份提示詞 28 處 ★ 強指令，小道具那句**不是 ★**；三階共用同一句純形容詞「其中正在運作的道具依強度影響她的反應」。**這正是本專案已經記過兩次的同一種病**（篇幅指示只有低端給數字／催眠三階都套「更強烈」）：小模型對形容詞疊加的區分度很弱，沒有具體行為指令的那一端會被直接無視。催眠已照此修過且有感，小道具還停在舊寫法。
      新增查表常數 **`KANSHOU_PROP_LEVEL_FX_`**（微弱／中等／強勁 → 一句具體演法；資料驅動，加階往表加一列即可），運作中的道具**另外拉 ★ 行**、不再只是屬性列裡一個括號。細節：
      - **同一階多件合併成一行**（`★【運作中·繩子、跳蛋(微弱)】`）——否則同一句指令會原封不動重複好幾遍，浪費字數且小模型對重複段落容易只認一次。
      - **`effect` 留空時補一句**「依名稱合理推定其作用並整段一致地套用」——原本 AI 只有道具名稱可以猜，常常整段當它不存在。
      - **無強度的純裝飾道具（`戴著`）不長 ★ 行**，維持原本「narration須自然反映其存在」即可，不必每回合給行為指令。
      - 屬性列那句「其中正在運作的道具依強度影響她的反應」已移除（★ 行講得更具體，留著只是重複）。
      ⚠ 連帶修 `check_prompt.py` 的**覆蓋漏洞**：★ 區塊標題上限原本寫死 `{2,20}`，標題較長的區塊（插了兩個 `${}` 的）會被**靜靜跳過**，兩道不變式對它形同不存在。放寬到 `{2,40}` 後掃描數從 80 → 83——先前有 3 個區塊從來沒被檢查過。**掃描器的覆蓋漏洞比沒有掃描器更危險**（會誤以為檢查過了），這條記進教訓區。
    - **⚠ 2026-07 補「催眠跟其他好感規則會不會衝突」**（玩家追問）：查出兩個真衝突＋一個較次要的：①🛑【角色一致性】原本寫死「靈魂態度仍守設定...嚴禁退化成發情機器」，完全沒有催眠例外，跟當時「強勁」階明講的「判若兩人」互相矛盾（⚠ 2026-07 三階重新推導後，這條例外的方向已整個反轉成「人格照樣不崩」，見上一則）；②`driveStr`①「主動程度嚴依好感分階」是獨立寫的一句，親密尺度五階的例外沒有回頭覆蓋到它，AI 得自己跨段推理優先權；③【篇幅指定】只看好感，催眠把低好感角色推向高強度場面時字數還是卡在最低檔，容易覺得被砍短。三處都補上「有ignoreBond效果生效時」的例外/覆寫，且**全部條件化**（新增`_kanshouHypnosisActive_`旗標，`partyRows`裡任一人有生效中的ignoreBond道具才為true）——沒人在玩催眠的回合，這三段提示詞完全不變、零額外字數。篇幅那條額外把目標字數在生效時直接鎖定最高檔(500字)，不再看好感臉色。
...
- **持久欄位的「欄名」跟「範例」都會窄化 AI 的理解，兩處都要顧**（2026-07 玩家實測「幫她戴貓耳朵，過幾輪就忘記」）：`appearance_extras`(原名`outfit_change`)本身是每回合都重新餵回的持久欄位(存`【換裝】`)，理論上不該被對話歷史的6則滑動視窗限制住——但舊欄名字面就是「換裝」、範例又只給「絲綢襯衫」這種正經換裝，AI 容易把玩家臨時加的配飾/道具(貓耳朵之類)當成那句台詞的趣味描述、覺得不算「持久狀態」而不寫進來；沒寫進持久欄，這個設定就只活在對話歷史裡，滑出視窗後就真的看不到了。已改名`appearance_extras`(外觀附加物，不再暗示只認衣服)＋範例加「貓耳頭飾」，兩處一起下手。**排查「AI 忘記某個設定」時，先確認有沒有對應的持久欄位在接住這類事實——通常有，缺的只是欄名/範例沒涵蓋到這種情境、AI 沒把它跟該欄位對上號，不是機制本身漏接。**
  - **🔄 2026-07 後續修正**（玩家「小道具已經有專門機制了，外觀服裝也幫我專注在外觀服裝吧」）：既然持久小道具系統(含玩家自訂)上線後已經是配飾/道具類事實的機制保證正解，`appearance_extras`範例拿掉「貓耳頭飾」、改回**只專注服裝本身**——避免兩套機制搶著記同一件事。這條教訓本身依然成立(欄名/範例會窄化理解)，只是配飾類換了個更適合的專屬管道去接住，不必再靠這欄硬撐兼管。
