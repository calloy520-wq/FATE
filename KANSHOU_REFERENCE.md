...
- **🎀 小道具**（2026-07 新增·根治「幫她戴貓耳朵過幾輪就忘記」的機制保證版）：`KANSHOU_PROPS_`(資料驅動·**2026-07移除內建「跳蛋」後現為空陣列**，僅保留擴充掛勾、全面改走下方玩家自訂道具)＋`KANSHOU_PROP_LEVELS_`(關閉/微弱/中等/強勁)＋**`KANSHOU_PROP_LEVEL_FX_`(2026-07新增：三階→具體行為指令查表，運作中的道具另拉★行)**，存該同伴列 MEMORY `【小道具】id1:強度1,id2:強度2,...`（`kanshouGetProps_`/`SetProps_`/`ToggleProps_`，**多件可同時裝備**，逗號分隔比照【性格鎖】同款寫法——2026-07 玩家「其他道具怎麼辦，一次只能一種？」後從單槽改多槽）。**玩家UI手動裝備/移除/調強度，GAS直接寫，不靠AI自己判斷該不該記**——跟`outfit_change`(現appearance_extras)那套「AI自己判斷有沒有變化」的路徑刻意分開，這條是機制保證。
  ...
  - **🌀 催眠指令(ignoreBond)獨立入口(2026-07 二度改版·「催眠的和新道具要確實分開成兩種」)**：一開始把「無視好感」做成一般自訂道具表單裡的一個勾選框，玩家後來覺得混在一起不夠清楚，**改成完全獨立的按鈕/面板/action**：
    ...
    - 存進`【自訂道具】name:hasIntensity:part:ignoreBond`(4欄，第4欄向下相容舊3欄格式)。**三階＝作用點不同，不是強度不同**（2026-07 二度定案，玩家收集催眠題材討論後重新推導；先有一版「三階都套同一句更強烈」被否，再有一版「越強越恍惚→強勁＝判若兩人」也被否）。舊版的病灶：把三條軸綁在同一根拉桿上一起拉，最高階＝三軸全滿＝判若兩人，同時犯兩個錯——①人格被關掉（紅線②的反面、二創圈罵最兇的「キャラ崩壊」）②她沒反應就沒有互動性，**最高階反而最無聊**。
      ...
      **🤫 第五態「已解除」（2026-07，玩家：「中途解除催眠然後宣告沒有催眠，看著對方也失去反抗的想法」）**：後端常數 `KANSHOU_HYPNO_RELEASED_='已解除'`／前端 `KC_HYPNO_RELEASED_`。**刻意不併進 `KANSHOU_PROP_LEVELS_`**——那個陣列是三段強度，而這個狀態語意上不是更強/更弱而是「已經沒效了、但她不知道」，併進去只會讓「第幾階」變得沒有意義。任何階都能隨時切進來，從這裡也能再施加回任一階。一般道具吃不到這個值（`def.ignoreBond` 才放行）。
      提示詞：`★【…·已失效·她不知情】` **仍然繞過好感天花板**——機制上「解除」等於她相信自己被解除了，而她並不相信。同時把「其中正在運作的道具依強度影響她的反應」那句排除掉（已失效還說正在運作是自相矛盾）。
      前端兩顆按鈕是同一條路的前後半段：`🤫 悄悄解除（不告訴她）` → `已解除`；處在 `已解除` 時該鈕變成 `💬 告訴她真相` → 撤下。

      **⚠ 2026-07 二度簡化（玩家「有點太繁雜」）——判準只剩一條：跟她說話的會演，動手腳的不會。**
      · **會觸發 AI**：🌀施展新的一句／⏹解除（＝當面告訴她，不再分「單純撤下」與「告訴她」兩種）／💬告訴她真相
      · **樂觀更新、靜默**：調強度／🤫悄悄解除（`kcHypnoQuietSet_`，比照 `kcQuickSetProp` 立即改本地快取＋重繪、背景送出、失敗還原並提醒）
      悄悄解除靜默**不是偷懶**：她本來就不該察覺，演出來反而破功——機制語意自己替樂觀更新背書。靜默切換不必擔心 AI 不知道：`_ignoreBondLines` 每回合重建，下一回合的敘事自然照新狀態演。
      （舊版）每次切換都送一段敘述（`kanshouHypnoSet` 組字串 → `kanshouSetProp(...,castMsg)` → `send()`）：調深/調弱/當面關掉/悄悄解除/告訴她真相各一句。理由跟🌀施展同一條——玩家明講「這裡需要一次呼叫AI才有催眠感覺」，靜默寫入等下一輪就沒有那個當下。
      ⚠ **單一入口**：同伴抽屜的快調面板（`_kcRenderDrawer_`/`kcQuickSetProp`）**已把 ignoreBond 過濾掉**。那條路沒有解除兩態、也沒有「⏹解除＝當面告訴她」那段敘述——留著等於開第二道門，而那道門會靜靜地把暗示關掉、什麼都不演。催眠一律走同伴卡的「🌀 催眠指令」面板。
