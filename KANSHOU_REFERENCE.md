...
**① 敘事的 `<br><br>` 變成畫面上看得見的四個字。** 2026-07-24 補 self-XSS 那次（`fa56630`）把 AI 敘事本文整段 `escapeHtml`，但提示詞（`nsfwBaseRules` 第 2 條與 solo `miniSystem`）明寫「換行一律用 `<br><br>` 分段」——escape 之後分段標籤就成了字面文字，**兩軌同時中招整整四天**。根源解：`Script.html` 新增共用的 `aiHtml_(text)`（escape 全部 → 只把 `<br>` 放回來 → 再轉真換行），`narrate()`／鑑賞 `send()`／老虎道場三處統一走它。**放行清單就只有 `<br>`**，`<br onload=…>` 這種帶屬性的假標籤不放行。回歸探針 `scratchpad/sim/brfix.js`（9 條，含 4 條注入案例）。
