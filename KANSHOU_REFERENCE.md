...
- **觸發＝第四層·最低優先**：排在節慶→住處睡眠→地點橋段【之後】，只補前三層沒佔到的時段空檔，不搶膝枕/共浴/下廚。舞台限 `kanshouPlayerHomeLocs_`（`region:'home'` 五處＋我的房間，**與「她的住處」`kanshouHomeLocs_` 是兩回事**），候選經 `_reIsCohabitTrigger_` 限**同居中**的她。
- 內容刻意寫**平凡的共同生活**（一起弄晚飯、睡前閒談、半夜宵夜）而非情慾升級——缺的是「一起生活」的實感，親密場面既有橋段已涵蓋。同居必然 bond≥80（跌破會被 `kanshouSyncRelTier_` 自動解除同居），故單一分支即可。
