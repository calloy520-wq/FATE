...
- **`KANSHOU_RAPPORT_TONE_[好感段][熟悉段]`** → 一句**具體演法**（不是形容詞標籤——那是本專案犯過三次的錯）。
  **刻意留白**：12 格只填 8 格，查無＝不輸出這個欄位、讓 AI 自由發揮。填滿只會稀釋真正有戲的那幾格。
  最有戲的一格是 **陌生×老交情**：「熟歸熟，她對你就是沒有那個意思」——這個狀態在改之前**機制上不可能存在**。
