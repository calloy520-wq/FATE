...
- **同居**：好感≥`KANSHOU_COHABIT_BOND_ = 90` 可邀（`kanshouInviteCohabit`，玩家自己隨時主動發起，需她在場被問），就寢/夜襲在 `KANSHOU_COHABIT_ROOM_ = '和室'`；`【同居】1` 標記。⚠ **2026-07 稽核修·同居會隨好感跌破90自動解除**：舊版`【同居】`只有兩處會寫成1、全檔案沒有任何地方清回0——好感若同居後因爽約/冒犯一路跌到接近「點頭之交」，標記仍在，AI仍照樣把她骰進和室、仍觸發夜襲/賴床，敘事跟數值直接矛盾。已在`kanshouSyncRelTier_`(跟REL_TAG梯度同步同一個函式、呼叫時機也一致)裡補上：BOND低於`KANSHOU_COHABIT_BOND_`就清掉`【同居】`。
...
- **💌 約會地點好感分級**（`minBond`欄位，2026-07 三度改版新增，原供已移除的`kanshouPickDate_`選點用）：`KANSHOU_LOCATIONS_` 每筆可選填 `minBond`（省略＝0），沿用 `KANSHOU_REL_TIER_` 同一份好感梯度門檻——0（日常初識，河邊小徑/咖啡廳/老道場等既有地點）／40+（熟識的朋友：摩天輪、水族館）／60+（親近的人：夜景展望台、隱藏溫泉）／80+（戀人·可以很色：情侶溫泉套房、深夜賓館）。**⚠ 2026-07 八度改版：`kanshouPickDate_`已移除，`minBond`欄位現無任何程式碼讀取**，純屬保留分級資料、不影響現行流程——`promiseMeet`/`proposeMove` 的地點驗證從來就不看 `minBond`，玩家自己走地圖過去或帶她同去完全不受此限。`dateOnly:true`（情侶溫泉套房／深夜賓館）＝這類明顯是約會限定的私密地點不進 `kanshouRollDailyLocation_` 的日常閒晃保底池，這個用途仍在使用中；`noEncounter:true` 一併擋掉陌生人巧遇。
...
- **🕐 時段開放地圖（`bands`，2026-07 六度改版新增，玩家「清晨走進深夜賓館，櫃檯空無一人像恐怖片開場」）**：`KANSHOU_LOCATIONS_` 每筆可選填 `bands`（`timeBand_` 5段：清晨/午後/黃昏/夜/深夜 的子集），省略＝不受限、全天候開放（向後相容，其餘地點不受影響）。目前設限：書店二樓(清晨/午後/黃昏)、水族館(清晨/午後)、摩天輪(清晨/午後/黃昏/夜，只排除深夜)、深夜賓館／夜景展望台／情侶溫泉套房(黃昏/夜/深夜)——依地點名字/現實常識訂門檻，其餘地點一律不設限，想擴大範圍只需往表加 `bands` 一列，不必動判定邏輯。
  ...
  - **必爽約陷阱同步修掉**：`kanshouPickDate_`（GAS 自動選約會地點/時段）與玩家 `promiseMeet`（相約）都已改成只挑/只准跟該地點 `bands` 相容的約會時段（`KANSHOU_APPT_BANDS_` 午後/黃昏/夜）——避免「約好了、赴約時卻被地點未開放擋在門外」的必爽約陷阱（同一類 bug 之前在拜訪私宅那邊修過一次，這次換 `bands` 踩到同個坑，修法完全比照）。
