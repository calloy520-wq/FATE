...
for i, l in enumerate(lines, 1):
    ...
    # 🐛→✅ 2026-09：舊版只認【緊接在 ★ 後面】的【…】，於是寫成「★演出這段抵達【…】」
    #   「★上面是御主的【內化用素材】」這 25 條一律不算區塊——它們只是碰巧被鄰居那 420 字的尾巴
    #   吞進去才被檢查到，位置一變就沒人看（跟 check_wait 的固定視窗同一種脆弱）。
    #   現在 ★ 與【之間允許一小段文字，這些指示才成為自己的區塊、真的被規則①②看見。
...
SOFT_BAN = re.compile(r'(?<![要能])(不可|不要|不得|不必|不能|禁止|勿(?!論)|絕不|別讓|別再|別把|別只|別當|別靠|別急|別跳|別改|別拿|別演)')
SOFT_SKIP = ('情不自禁', '不禁')
...
NEG_EXAMPLE = re.compile(r'(?:不可|不要|不得|不必|不能|別|勿|禁)[^\n。；]{0,40}?(?:「([^」\n]{2,24})」|[（(](?:例|如)[:：]?([^）)\n]{2,30})[）)])')
