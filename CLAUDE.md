...
- **🌹 慾海 kanshou（鑑賞後日談）**：不需先打贏，直接從英靈殿召喚同伴進入。**NSFW**。持久化 個性／特徵／關係／外顯／肉體 ＋ 歷史因果。**無戰鬥、無經濟、無房東房客世界觀、無地點**（2026-09 玩家「只要扯到移動都會很怪！直接都砍了吧」——地圖/移動/住處/作息整組退休，只剩一格 AI 每回合自己寫的背景），其餘全靠 AI 即興。建在 `nsfwBaseRules`（紅線①）＋鑑賞自己的 `actionPlay_`（跟 solo 只共用 `callGeminiAPI`）。**說書人風格 15 段交給玩家**（☰「⚙ 說書人設定」·`KANSHOU_STYLE_MODULES_`·預設一格不改＝提示詞逐字不變，見 `KANSHOU_REFERENCE.md` §說書人風格）。

**⚔️ solo 已從零重建（2026-09-24）**：選單「⚔️ 聖杯戰爭」＝`War_Engine.gs`（純規則）＋`War_Router.gs`＋`Script_War.html`，一天兩個決定、按鈕越少越好、沒有魔力／AP／地圖／好感。玩家要的核心：**「按鈕越簡單越好」「畫面簡單但要有策略感」**。改規則先跑 scratchpad `war/sim.js` 量勝率，理由與數字見 CODE_NOTES.md『WAR_』。舊版 solo 留著「舊版」入口，確認後整組拆。🛠️ 英靈工房只剩一個＝`War_Forge.gs`（一張表單寫一整列英靈殿，新戰爭與鑑賞通用，見 CODE_NOTES.md『WAR_FORGE_』）。
...
函式改名時 `CODE_NOTES.md` 的錨點要跟著改（錨點對不上＝那段歷史等於消失）——2026-09 起 `check_docs` 會擋：錨點名字全樹 0 次出現就叫。⚠ **叫了先查是不是改名**，改名就把錨點改成新名字，別直接砍——當場就有一個 `actionCourtEnemy`→`actionParley` 差點被我砍掉。另外那 1009 條 `<sub>檔:行</sub>` 的行號實測 88% 是錯的（會漂），已整批砍掉只留檔名。
