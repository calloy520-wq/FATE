...
**⚔️ solo 已從零重建（2026-09-24）**：選單「⚔️ 聖杯戰爭」＝`War_Engine.gs`（純規則）＋`War_Router.gs`＋`Script_War.html`，一天兩個決定、按鈕越少越好、沒有魔力／AP／地圖／好感。玩家要的核心：**「按鈕越簡單越好」「畫面簡單但要有策略感」**。改規則先跑 scratchpad `war/sim.js` 量勝率，理由與數字見 CODE_NOTES.md『WAR_』。舊版 solo 留著「舊版」入口，確認後整組拆。
