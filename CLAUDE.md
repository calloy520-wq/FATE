...
- **🌹 慾海 kanshou（鑑賞後日談）**：奪杯後與封存從者的約會，**NSFW**。**一個更單純的世界**（2026-06 玩家定案，2026-07 追加經濟層重啟見下）：持久化 **個性／特徵／關係／外顯狀態／肉體 ＋ 歷史紀錄·因果 ＋ 金錢**；無戰鬥，一張約會大地圖，其餘全靠 AI 即興。仍建在 `nsfwBaseRules`（紅線①）＋ `buildDefaultSystemPrompt` 上、共用 `actionPlay` 引擎。
...
🔵 **2026-07 玩家推翻部分經濟決定，僅 kanshou 恢復真經濟層**（2026-07-13「真的要賺錢 然後要付住宿費用?每個禮拜要付錢?我要去打工賺錢?!」，確認要「真的有系統記錄、會被扣款/賺取」、範圍「僅鑑賞」）：Phase 1（已實作，見 SOLO_REFERENCE.md §76）——`COL.PC.MONEY`/`RENT_WEEK`（尾端新欄，solo 恆空）、開局起始金錢 3000、💼打工固定薪資 800/次(耗時4h)、每週房租 1500(可透支成負值，無懲罰機制，純軟性)。Phase 2（已實作，見 SOLO_REFERENCE.md §77）——🛍️商店：資料驅動品項表(`KANSHOU_SHOP_ITEMS_`)，裝飾品買了存玩家 MEMORY【家居裝飾】清單(narration可參考)，禮物買了直接送在場的人(2026-07§91後不再有「同行」概念，改依LOC同地點判定)、好感依固定值增加(GAS掌數值)。**這條路線僅限 kanshou，solo 依然全程無花錢入口不變**。Phase 3 起(家事整潔度/任務系統)尚未實作，之後陸續加。
