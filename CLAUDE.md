...
- **驗證**：改完必跑 `bash check.sh`。除語法外還跑十七支不變式掃描，都是把**重複踩過的漏洞形狀**改成機器擋：
  ...
  `check_seed.py`（**種子寫了卻沒人吃**——2026-09 瘦身當場抓到四種形狀，共通點是零錯誤訊息：
  ①【幽靈 fx】美狄亞掛著「金羊毛 Argon Coin EX」，引擎裡一個字串都沒有、`ALLOWED_FX_` 沒有、
  前端說明表也沒有——玩家看得到、按了什麼都不會發生；②【死欄位】御主殿的「居所」「屆次」全樹零讀取
