...
- **寶具有充能、沒有魔力**：放完要等 `NP_COOLDOWN` 個早晨；補魔把充能往前推 `SUPPLY_CD` 夜；令咒可以無視充能硬放。魔力整組拿掉（玩家同意）。
- **戰爭不等你**：敵人每夜照職階個性（`WAR_CLASS_.aggr`）找人打、互相吞併，打倒別人的會變強；知道你據點的會在你固守時夜襲。
- **最後一夜＝柳洞寺決戰**：時間到不是莫名其妙判輸，是剩下的從者一個接一個上來、不能撤退、打到有人倒下。
  而且【不是你親手收下的魂】會灌給活著的敵人（`FINAL_ABSORB`）——這條是專門殺「一直固守躲到最後」的：
  沒有它，固守在第五次戰爭的勝率是 35%（正是玩家抱怨的「休息就贏了」）；有了它掉到 4%。
- **數值還在、換成看得懂的話**：六圍照原作讀進來，攻擊取筋力與魔力高者；畫面只給「勝算大／勢均力敵／勝算小／凶險」與傷勢字。
  `STAT_SPREAD` 把階級差距打六折：原作強弱還在，但抽到誰不至於開局就定勝負（沒打折時 Saber 81%、小次郎 18%）。
- **職階特性一條、說得出來**：`WAR_CLASS_.trait` 直接顯示在從者卡上（Saber 看得到寶具預兆、Rider 撤退、Caster 據點魔術陣、Assassin 出擊第一擊…）。
...
| 亂按 | 2% | 7% |
| 一直固守＋休養 | 4% | 9% |
| 狂放寶具（天天補魔、令咒硬放） | 11% | 16% |
| 有想法（先打聽、挑勝算、試探後再放寶具、傷重就撤） | 53% | 72% |

每位從者（有想法的打法）落在 22%（吉爾·德·萊斯）～69%（蘭斯洛特）。一局約 50 個決定、平均打 11～12 場。
