...
### `KANSHOU_NOTED_TAG_` / `KANSHOU_KNOWN_TIERS_` / `kanshouKnownOfYou_`

══ 📝 她眼中的你（2026-09 玩家「跟外面 ai 不同，這裡的 ai 明確知道所有設定，第一次遇到玩家就把玩家看透了」）══

玩家卡（原【玩家資料】）上的 `性格:[內裡]`／`萌點(僅供內化)`／`經歷`，過去是每個在場角色【無條件全知】。查了一遍才發現整套鑑賞從來沒有任何一條規則說過「她還不認識你」——`grep "還不認識\|不會一眼看穿\|知情"` 是 0 筆。

根源是**一份資料兩種用途被混在同一行**：①寫玩家自己的內心與感受（★【你也是這座城裡的一個人】要用，才剛補厚）②在場角色對玩家的認識。①該全知、②不該，但它們共用同一行字，所以只能一起全知。修法是把兩種用途拆開標示，不是把資料拿掉（拿掉就把玩家打回攝影機，那是上一輪剛修好的東西）。

**為什麼分成兩層、而且刻意讓它們由不同的東西保證**（玩家追問「能確實紀錄嗎…？」）：
「她對你多熟」這條線 **GAS 自己算得出來**（`【相處】` 次數查 `KANSHOU_FAMILIAR_TIERS_`，跟相處基調共用同一條軸），所以擋不擋得住全知**完全不取決於 AI 肯不肯配合**；「她記下你哪些特點」才交給 AI 吐 `noticed`，走的是共同回憶那條已經跑了幾個月的 append/去重/上限管線。**先用擋得住的東西擋住，再用盡力的東西讓它長出來**——順序反過來（只靠 AI 自律不要說中）就會是那種平常看起來有效、關鍵時刻靜默失效的設計。

`KANSHOU_KNOWN_TIERS_` **只講「她知道多少」，不講態度**——態度是 `KANSHOU_RAPPORT_TONE_` 的事。兩張表同軸不同事，寫的時候一直有把「她還不熟所以會冷淡」寫進來的衝動，那是把兩張表混成一張，混了以後任何一邊要改都得先把另一邊解開。

`【眼中的你】` 存**她自己那列**的 MEMORY（不是玩家列）：每個人各記各的，同一個玩家在不同人眼中本來就該不一樣；存玩家列會變成「全城共用一份筆記」，那就是換個寫法的全知。分隔符用 `／` 不用 `｜`——`makeTextTag_` 會把 `｜【】` 從值裡剝掉（它就是為了防 AI 偽造欄位才這樣做的），用 `｜` 當分隔符會被自己的防線吃光。
