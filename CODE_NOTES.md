...
### `verifyPcOwnership_`　<sub>Account.gs</sub>
...
### `makeTextTag_`　<sub>Core_Settings.gs</sub>
...
### `parseTraitsHelper`　<sub>Core_Settings.gs</sub>
...
### `looksToTraitParts_`　<sub>Core_Settings.gs</sub>
...
### `toTaiwanTrad_`　<sub>Engine_Combat.gs</sub>

2026-09-23 LANCER×ARCHER 實測：同一份提示詞、同一條【語言鐵律】，gemini flash-lite 一整輪三步全簡體，
另一組也每步夾幾個簡體字與舊字形（個／邊／著／裡／點的簡寫）。提示詞兩道語言鐵律管不住偶發漂移，重試又是整顆模型的錢與秒數。
根源解在出口：`callGeminiAPI` 回傳前逐字換，字表只收「繁體絕不會出現」的字（跟 `check_simp.py` 同一張準則表），
多對一的字（后/發/乾/裡/麵/隻/台…）刻意不收——寧可漏一個簡體字，也不要把「皇后」換成「皇後」。
字表簡體端在 .gs 裡寫成 `\u` 逃逸，否則 `check_simp` 掃 repo 會把字表本身當混入的簡體字。
著／裡／為／麼／眾／線／說 的舊字形與日本新字體（氣／戰／發／驛…）不是簡體字，原本 check_simp 不收，這次一併加進它的字表——轉換器字表必須 ⊆ 掃描器字表（`trad.js` 釘住），兩邊才是同一張準則表。

### `callGeminiAPI`　<sub>Engine_Combat.gs</sub>

**思考預設關（2026-09-23）**：金閃閃×SABER 換成 `google/gemini-3.5-flash` 跑，十步全部「模型中途停了：length」、
...
玩家要「正史隨機感」：抽到的從者不該是敵陣那位。第一版我照字面理解成「4th＋5th 全部排除」，
先量了一次才發現不能這樣做——英靈殿可召喚 20 位，兩場正典去重佔掉 14 位，**隨機池只剩 6 位**，
其中 4 位是 Prisma Illya 的外傳客串、斯卡哈還佔兩格，沒有 Rider 也沒有 Berserker。
「隨機」會變成「每次都抽到魔法少女」，跟要的效果相反。把數字攤給玩家後定案：**只排這一局在場的那組**
（選第五次就排第五次那 9 位）。池子回到 10~11 位，而且多出來的是迪盧木多／伊斯坎達爾／蘭斯洛特這些
正史英靈——在第五次召喚出第四次的英靈，原作本來就發生過（Saber、吉爾伽美什都跨過戰爭）。

⚠ **`roster.hero` 是英靈殿的 ID 不是 NAME**（`阿爾托莉雅-Saber` vs `阿爾托莉雅·潘德拉貢`）。
我第一次就是拿 NAME 去比，14 個一個都沒對上——而且這種錯不會噴任何錯誤，只會「排除功能安靜地沒作用」。
`seedRivalsForGame_` 的 `findHero` 就是用 ID 查的，照它。

⚠ **鋪敵方陣容發生在召喚【之後】**（`actionSummonServant` 尾端才呼叫 `seedRivalsForGame_`，
因為它要拿玩家選走的那位當參數），所以這裡讀不到「場上已經有誰」，只能查場次表。
這也是為什麼黑名單用 `warName` 而不是掃眾生列。
...
玩家自己打的那句話，跟按鍵路徑（相約／告白／牽手／移動…那些 GAS 寫的摘要）共用同一個 `【玩家意圖】：` 前綴。
「意圖」對按鍵是實話（那真的是 GAS 歸納出來的「他想做什麼」），貼在玩家原話上卻是假的：
那句話不是意圖，是他真的做出來、說出口的東西。模型照著標籤理解，就把它當成已經發生的前提跳過去，
直接寫對方的反應——玩家看到的就是「我打的字沒有被演出來」。
根源解是讓標籤說實話（`【玩家原話】：`），而不是再寫一條規則去對抗錯誤的標籤；
`enact` 那條鐵律是配套的第二半（叫 AI 把那一步撐成完整的一拍），兩件事缺一都不會有效。
⚠ 歷史暫存存的是**沒有標籤的原始 `userMsg`**，所以改標籤只動提示詞、不動存檔；
`kanshouRecentDigest_` 裡那條 `replace(/^【玩家意圖】：/,'')` 是給改版前的舊列用的，留著。
...
`KANSHOU_STARTER_IDS_` 後來種了四個人進去，於是新玩家第一眼看到的就是一句假話，零錯誤訊息。
2026-09 整段重寫（玩家：「整個也很 ai 人機感覺…簡單的介紹就好」）：只講每顆鈕是幹嘛的，
語氣句（「想做什麼都可以」「日子還長」）全砍。寫這段的原則是**少寫一點**——它是說明書不是開場白。

### 怪癖退出【兩軌】的卡片　<sub>Gallery.gs · Router_Persona.gs · Seed_Codex.gs</sub>

🗑️ 2026-09 玩家：「**其實我一直想砍怪癖…這應該要讓 AI 去演阿。像是 SABER 應該就是直接美食和
獅子玩偶，這樣就好，幹嘛要規定他表演啥**」。

回頭看，**這一格從設計上就是舞台指示欄**——原版 22 筆裡有一半本來就是【觸發＋動作】：
「吃到好東西時會安靜下來」「看到別人握刀的手勢不對就想糾正」「被道謝時會愣一下」
「被碰到時會慢半拍才反應」「飯點一到就自己出現在別人家」。
每回合都送出去，模型就每回合找機會演一次。大河之所以最出戲，正是因為她那兩個動作最大聲。

⚠ **一個要記下來的自我修正**：同一天我才在批評 Gemini「把怪癖改成 if-then 腳本」，
而它有一半是照抄原版的形狀——**我在維護一個本來就生病的欄位**。玩家看出來了，我沒有。

做法：
- **鑑賞卡不再送怪癖**（`pQuirks` 讀取與插值移除），鑑賞創角也不再叫 AI 產生這一格
  （提示詞那行與 JSON 範本欄位一起拿掉）。
- **值得留的【東西】搬到它們本來該在的欄位**——玩家自己指的方向：
  獅子玩偶→SABER 的「喜歡」、眼罩→美杜莎的「外貌」、電子產品→凜的「討厭」。
  其餘 19 筆的怪癖檢查過，沒有獨有的名詞會跟著消失（蹭飯已在喜歡、修車廠已在身分、
  右臂纏繃帶已在外貌）。
- **solo 的 `servantCard_` 同步砍掉**（玩家：「SOLO 也砍吧」）——那一行 `｜${quirks}` 移除。
- ⚠ **資料層不動**：`PERSONA_QUIRK_TAG_` 與種子的 `quirks` 欄照舊（寧棄用不刪）。
  `masterPersonaLean_`（Router_Bond.gs）仍讀這一格，但那是**內部關鍵字分類器、不是提示詞**，
  不會叫 AI 表演任何動作，所以留著；它另有 PREF／願望／logic／BACK 四個輸入，缺這格也降級得動。
- 👓 順帶修正：美杜莎的**眼罩是 Rider 戰鬥版的**，日常版戴眼鏡。眼鏡搬進 `dailyLook`
  （臉上的固定特徵），從 `dailyOutfit` 拿掉——衣裝是會換的東西，同一件事不存兩處。
...
**第一輪它給的東西不能用**，三個方向都是反的，記在這裡當教訓：
- **文言化**（庖廚／衣袂／朱眸／頷首／剖白心跡）——卡片是 AI 拿來「變成那個人」的依據，
  遞文言文過去回來就是文言腔，而這一軌的設定是【輕小說筆觸＋現代冬木市的日常】。
- **把怪癖從【狀態】改成【if-then 腳本】**（「走路沒有聲音」→「行走間衣袂少有聲響」、
  「甩馬尾別過臉」→「說中心思時撫弄髮梢別過頭」）。加上觸發條件之後，模型會**主動去製造
  那個條件**好讓自己能演那個動作。它自己第二輪承認了這點：「模型會為了合理化 B，
  在沒有 A 的情況下主動在旁白裡強行無中生有湊出 A」。
- **氣質從【感官素材】變成【形容詞＋感】**（「紅茶的熱氣與寶石的冷光」→「明麗敏銳的端正氣質」）。
  左邊 AI 可以直接寫進場景，右邊它只能複述。
- 順帶把身體描述系統性淡化（豐盈→柔韌），而這是一個 NSFW 為主軸的模式。

**第二輪它只動全稱副詞，那批是對的**：整張卡每回合都送，`總／總是／每次／一直` 在哪一格
都會變成「每回合演一次」。全樹共 6 處，它一個沒漏。採用 3 條、改寫 3 條
（它的「腳步倒會」「格外得多」中文彆扭；「好像常換人」把百貌哈桑的梗弄丟了）。

⚠ **它的理由講錯了**：它以為那些字會觸發掃描器。實際上 `check_seed` ⑪ **只掃氣質格**
（`dailyLook` 的第二段，`parts[1]`），其餘五格是靠原則自律、機器不管。對的事、錯的理由。

⚠ **掃描器當場擋下它一條**：`logic` 「事做完先走」→「事做完默默退開」被 `check_seed` 判
「行為準則沒有取捨」。這一格的規格是【把兩件都想要的東西擺在一起、說出放掉哪一個】，
「先走」是一個選擇，「默默退開」是單向形容詞。而它擔心的事（AI 誤會要脫離隊伍）
在 `npc_exit` 砍掉之後已經不存在——AI 動不了同行名單了。退回原文。

### `stripStanding_`　<sub>Gallery.gs</sub>

🐛→✅ 2026-09 全份提示詞重讀時抓到：**「常駐舞台指示」會從【資料】那條路溜進來**。
`check_standing.py` 只掃【我們自己寫的】提示詞，掃不到 AI 回寫的欄位。實測形狀：
AI 往 `noticed` 寫「總是帶著溫熱的煎茶」→ 存進她列 →每回合餵回去變成
「她注意到我**總是**帶著溫熱的煎茶」＝**AI 自己給自己下了一道常駐指令**，
跟種子裡退休掉的「背脊永遠打得筆直」同形，只是走資料那條路、機器看不到。
在唯一的寫入口剝掉 `永遠|總是|老是|一律|每次|每天|從不|不停`。
⚠ 剝完仍是一句通順的觀察（「帶著溫熱的煎茶」），這一格本來就 ≤14 字。

### 2026-09 全份重讀：人稱與懸空指涉　<sub>Gallery.gs</sub>

玩家「妳全體再看看一次吧」，從頭逐行讀完整份提示詞，抓到四件事：

- **玩家在同一份提示詞裡有四種叫法**：`我`（旁白）／`妳`（NPC 當面叫）／
  `你`（一路跟著你走・她注意到你・你走進來的時候・只有你一個人）／`玩家`（玩家問起就…）。
  旁白已經是第一人稱，指玩家一律改成「我」。⚠ 對【AI】說的那些「你」
  （只給你看／給你判斷光線）是對的，不動。
- **兩個懸空指涉**：①`見下方【此刻】`指的其實是【他們此刻】（★【此刻】是日期時間那塊）；
  ②`【已經確立的事】`這個名字**全樹不存在**（真名是「這個世界已經確立的事」），
  而且世界帳本空的時候那塊整個不送——改成自己把話說完，不指向任何區塊。
  跟 narration 欄那句「長度見【篇幅】」同一個形狀（篇幅選「隨意」時那行根本不送）。
- **三個永遠為空的插值**（`backgroundCrowdStr`／`PROMPT_REL`／`npcDialoguePrompt`）
  每回合各生一個空行。宣告與插值一起清掉，樣板結尾再加一道 `replace(/\n{3,}/g,'\n\n')`
  ——往後新增的條件式區塊自動適用，不必逐個顧。

### 【他們此刻】的抬頭　<sub>Gallery.gs</sub>

🗑️ 2026-09 砍掉抬頭裡的「(穿著是此刻的衣服，**長相體態不隨之改變**)」。
玩家問「會多餘嗎？」，而我**查不到它的來歷**——CODE_NOTES 沒記錄、git 也沒留下理由。
它防的形狀猜得到（換裝時 AI 順手把髮色體型一起改了，因為外貌住 system、衣服住 user），
但**「猜得到」跟「發生過」是兩回事**，不該用猜的理由留一句每回合都送的話。

⚠ **這是一次有條件的移除**：如果實玩看到她的外貌跟著換裝漂移，就把它加回來，
**並且這次把觸發的實例寫在這裡**。沒漂就是省下來的 21 字。

### `KANSHOU_MIN_PER_TURN_`　<sub>Gallery.gs</sub>

每回合推進幾分鐘（`kanshouHourPerAction_` 拿它除 60 當時鐘步長）。

🐛→✅ 2026-09 玩家問：「『這一幕就寫這十分鐘』有隨設定變動嗎？」——**沒有**。
真值在這顆常數，提示詞卻把「十分鐘」**寫死兩遍**（★【此刻】與★【夜已深】）。
改這顆常數，時鐘會照新值走、AI 卻還在收舊數字，**零錯誤訊息**——跟 `DOJO_CAUSE_`
把 `${FATE_DEADLINE_DAYS_}` 寫死在頂層樣板那個坑同一個形狀，只是這次在函式裡、
不會被 `check_loadorder` 抓到。現在兩處都代入常數。

⚠ `style.js` 驗這條**必須用原始 user**：`base.p` 走過那支濾除函式，而 ★【此刻】
正好在 VOLATILE 濾除清單裡——用 `base.p` 兩條斷言都驗不到（第一版就是這樣，
注入退化也不會叫）。

### 2026-09 提示詞三刀（玩家逐條看完整份提示詞之後）　<sub>Gallery.gs</sub>

玩家要求把鑑賞整份提示詞倒出來看，當場點出三件事，三件都成立：

**①「離場者？現在沒了吧」→ 其實還在，但砍掉。** `npc_exit` 是活的，它讓 AI 說某人告辭，
GAS 就把她移出同行名單**並且把她的 LOC 改到別的地點**。問題是：誰跟著我走、誰留在哪裡，
是玩家用面板那幾顆鈕決定的事（玩家原話「我 A 解散他，他會一直在 A」）——
**兩把鑰匙開同一道門，其中一把在 AI 手上**。整組移除（範本一行＋handler 1438 字）。
要她離開，敘事照樣寫得出來，只是位置不會被動。

**②「感覺很多解釋都重複了」→ 數出七處。** 「第一人稱『我』＝玩家」講 3 次（開頭／narration／options）、
「照卡上的名字寫」2 次、physical_state 與 appearance_extras 的玩家版/NPC版幾乎逐字重寫一遍、
「沒有就填什麼」有**五種說法**（否則「無」／其餘「無」／平常「無」／沒有就留空／沒換就留空）。
NPC 版改成「同上·這個人的」，五種說法收成一種。
⚠ 順手抓到一個**懸空指涉**：narration 欄寫「長度見【篇幅】」，但篇幅選「隨意」時
★【篇幅】那一行**整個不送**——它指向一個不存在的區塊。改成只寫「劇情」。
system 1558→1438 字、user 749→727 字。

**③「一直以來的底色？太硬了吧，每次都傳，所以根本不能改變？」** 玩家問到最深的一層。
先釐清：**那句話沒有凍住她，她本來就是凍的**——`PREF`/`TRAIT`/怪癖/行為準則四格
**只在召喚那一刻寫入，整局零寫入**。會長大的是關係（共同回憶／她眼中的你／關係稱呼／相處次數），
個性不長。而設計本來是有分層的（性格第二格＝「熟了才看得到的那一面」＋相處三階），
但 2026-09 砍掉「解釋欄位是什麼」的句子之後（因為 AI 會把欄位名唸出來），分層就失效了——
**AI 不知道第二格是深層，第一回合就全端出來**。兩難：講清楚結構→它把結構唸出來；不講→深淺一次爆光。
玩家定案：**「不要分表面跟內在了，直接統稱個性，AI 自己想辦法」**。
生成提示詞與面板標籤一律改成「個性兩句(各講一件不同的事)」，四格儲存不動
（COL 是位置索引，欄位本身不刪）。`formatPref` 本來就沒印層次標籤，卡片端零改動。

### `PROMPT_BODY` / ~~`genderHintStr`~~（2026-09-23 拿掉）　<sub>Gallery.gs</sub>

🐛→✅ 2026-09 玩家：「**但是我也是女的欸！！！我拿什麼頂她阿**」——玩家角色是女性，
敘事照樣寫成「用性器對準…挺腰撞了進去」「肉壁絞緊我的分身」「將熱燙的愛液灌入」。
★【身體】那一行明明就在提示詞裡。兩個根因疊在一起：

- **寫成能力清單**：舊值「做得到的是手指、舌頭與器物」讀起來像**建議**，模型自己補一根上去也不違背它。
  改成講身體本身：「我跟○○都是女性的身體，**彼此一樣**。要進入對方，靠的是手指、舌頭，或找得到的器物。」
  ⚠⚠ **我的第一版寫成「兩腿之間沒有陰莖」，玩家當場擋下**：「是不是太刻意了，說不定他看到陰莖
  又只記得這個詞了」。他是對的——那個寫法**鑽過了 `check_prompt` 的黑名單卻違反它的精神**
  （黑名單收的是 `不可/不要/禁止…`，「沒有」不在表上），而 CLAUDE.md 裡玩家自己的原話就是
  「**說越多它會越想歪**」「他有時候只會記憶重點、忘記前面的禁」。**點名不要的東西＝把它塞進模型腦裡，
  而且是每回合塞一次。** 正面寫法只講存在的事：兩邊一樣。
- **坐在自己量出來的死角**：原本掛在 `PROMPT_REL` 裡，落在 user 的**第 5 行、離結尾 18 行**——
  而本檔 🧊 排序原則那條寫著「實測過**事實寫在 20 行以前就會被 AI 當成沒發生**」。
  拆出 `PROMPT_BODY`，壓到玩家那句話的正前方（實測差 2 行）。
  身體狀態 `nsfwMemories` 一起搬——它每回合都在變，照排序原則本來就該在後面。

⚠ `style.js` 驗這條要用 **B（女玩家）**，★【身體】只在女女配對時生成；而且 style.js 裡那支建局 helper 抓的是
「表上第一個不是自己的列」，那可能屬於別人那一局（實測 B 就抓到 A 的人），要自己按 game_id 拉。
⚠ 位置那條只能驗【結構】（它跟玩家動作差幾行），驗「它存在」抓不到這個 bug——原本它就存在。

### `KANSHOU_HIST_WINDOW_`　<sub>Gallery.gs</sub>

逐字對話歷史的窗口（則數）。2026-09 從 4 砍成 2，玩家原話：「後面給太多資訊，AI 走不出來」。

量出來的數字說明了「走不出來」是字面意義的——穩定之後每回合送出去的東西：

| 區塊 | 字數 | 佔比 |
|---|---|---|
| system（規矩＋人物卡） | 1807 | 51% |
| user（此刻的狀態／地點／時間） | 694 | 20% |
| **玩家講的話**（歷史裡） | **16** | **0.5%** |
| **說書人自己上兩段寫的散文** | **1020** | **29%** |

**1 比 64。** 模型在問「這一回合該長什麼樣」時，context 裡聲音最大的答案就是它自己剛寫的那兩大塊，
於是第 3 回合起開始逐字抄自己（實測：最後一回合 260 字裡只有約 25 字是新的）。
砍成 2 之後它自己的散文 1020→510 字、佔比 29%→17%，整包 3537→3019 字。

⚠ 再往前的事情**不是消失**，走的是挑過的事實那三條路：世界帳本／共同回憶／她眼中的你。
⚠ 這個數字直接換敘事連貫感。原本 `_sceneCut ? 2 : 4` 的分支在兩邊都變成 2 之後整個退場。

📐 **同一次測量還挖到一個更大的東西（尚未處理）**：整份提示詞裡
**【角色】相關 698 字 ／【世界·場景】相關 114 字＝6.1 : 1**，而那 114 字大半還是規則——
真正描述「這個世界長什麼樣」的只有「我的房間・清晨・06:10」。模型手上有 698 字關於這些人、
10 個字關於這個世界，它要寫 500 字就只能翻來覆去寫她們（胸口、髮尾、碧眼、紅茶香）。
玩家的觀察：「以前的遊戲是一堆設定，AI 反而不太會強調角色」——因為角色只佔其中一小塊。
⚠ **我們一整天做減法，每砍一刀，角色卡在剩下的東西裡就更巨大一分**，這是自己走出來的死胡同。
兩條路：繼續減（角色會變薄）／加世界（世界帳本早就蓋好了，只是餵得很省）。後者同時餵飽
「AI 自由發揮」與「該記得的要記得」，且不必寫死任何劇本——只要讓它記住自己編出來的世界。

### `KANSHOU_STYLE_MODULES_` / `kanshouStyle_` / `buildDefaultSystemPrompt` 陣列組裝　<sub>Gallery.gs · 風格層</sub>

🗑️ **2026-09 演法指令整批砍除**（玩家：「是不是全砍了！我真的只要給資料，還是格式」
「設定那麼多我還不如直接去找 GROK 色色就好了」）。玩家貼回一整局實測，最後一回合的 narration
約 260 字裡**只有約 25 字是新的**，其餘是前一回合的逐字複製：三個段落各自連續 4~5 回合一字不差。
把整份提示詞倒出來逐條分類成【資料】【格式】【教它怎麼演】之後，第三類全部砍掉：
- **🚨【收尾】「最後一句留給被搭話的人，停在等玩家回應的那一刻」**——這是「她紅著臉等待著我的
  下一步」連五回合的直接來源。整個 `ending` 模組與呼叫點移除（固定段 8→7）。
- **`perform` 把八個欄位名逐一列出來**（平常看得到的性格／熟了才看得到的那一面／喜歡的／討厭的／
  怪癖／做選擇的方式…）——**欄位名變成模型的詞彙**，敘事裡冒出「她熟了之後才會顯露的佔有慾」
  「她的怪癖讓她在做選擇時總是傾向…」。卡上那些詞組本來就看得懂，不必教順序。連同後半段
  「讓它從稱呼、距離、眼神與舉動裡透出來」（演法）一起砍，只留【事實】：這些卡只給你看、當事人沒讀過。
- **`history` 的「只供語氣與細節連貫」**——字面就是在發抄襲許可，模型沿用到逐字一樣。
  改成「它讓你知道這一路走到哪裡了；這一回合要寫的是【接下來】的那一段——新的動作、新的話、新的反應」。
- **★台詞與人物互動【佔 narration 七成以上】**（配額式指令）、**🛑【角色一致性】**（沒帶資訊的廢話）、
  **★【誰在場】的「其餘的人在場、做自己的事就好」**（演法）。
- 同批修掉三個「格式自己造成的固化」：`dialogueFormatRule_` 原本寫「（就是括號前面那個名字本身）」
  ——規則自己在指涉括號，模型就把「櫻（女）」整個當名字冠上去；★【此刻】的時間戳被當成要寫出來的
  資訊（一段報一次時、日期重複五次），補一句點明那幾個數字是判斷光線/氣溫用的；
  ★【這座城裡還有誰】原本只說「不會開口」，模型就只守住嘴巴、照樣把不在場的人寫進背景並逐字複製，
  改成「不在這一幕的【畫面】裡」。
- **留下的**：★【性別配對】（玩家指名保留）、所有資料區塊、所有格式區塊。
  system 1594→1518 字、user 768→673 字。

⚠ `style.js` 裡那些「釘住某句話存在」的斷言，砍掉那句話之後要**翻面成墓碑斷言**（盯它別長回來），
不是刪掉——刪掉等於這個坑沒人守。這批翻了六條。

🧹 **2026-09 第二意見第三輪：三處「同一件事講兩遍／兩句互相打架／跨區塊指路」**。
把整份提示詞貼給另一個模型做第三輪對讀，它挑出來的不是字寫錯，是**結構上的重複與矛盾**——
這類東西每回合都送、零錯誤訊息，自己讀一百遍也看不出來（因為每一句分開看都是對的）：
- **`voice` 開頭的「在場的人各自用自己的名字稱呼」**與 `dialogueFormatRule_` 的
  「每句前冠說話者的名字，用卡片開頭那個名字，同一個人跨回合都用同一個」講同一件事，
  而後者才是**管得住格式的那條**（它說死了用哪個名字、跨回合要一致）。前者移除。
- 🐛 **★【誰在場】「這個世界裡已經有名字的人可出現可開口」** ↔ ★【這座城裡還有誰】
  「他們此刻各自在自己的地方，【不在這一幕的畫面裡】」——**同一份提示詞裡兩句互相打架**，
  模型只會挑一句聽。前面那半句是我前一天自己改出來的（本意是「路人不必具名」，寫成了放行條款）。
- **【在我身邊的人】抬頭的「此刻穿什麼、和我走到哪一步，見下方【他們此刻】」**——跨區塊指路，
  而【他們此刻】住在 user slot、這句住在 system 快取段：每回合白付字數去指另一份文件，
  標題本身就自明。改成「(以下是他們是誰)」。

🐛→✅ **同一批抓到 `style.js` 自己在說謊**：墓碑斷言那一整叢用的 `_ALL_` 寫成
`base.s + '\n' + base.p`，而探針取樣函式回的是 `{s, u}`——`base.p` 是 `undefined`，
於是那叢斷言**長年只看得到 system prompt**，住在 user prompt 裡的墓碑（★【誰在場】那幾條）
一個都驗不到。發現的方式就是 CLAUDE.md 那條規矩：新斷言寫完**注入一次退化確認它會叫**，
四條裡有一條不叫，才順著查到 `_ALL_`。修成 `base.u` 後四條全叫。
**不會叫的斷言比沒有更糟，它給你「已經有防線」的錯覺**——這次是第二次踩到同一個形狀。

🔒 **2026-09 資訊邊界（第二意見：「全知視角汙染」）**。兩位以上同伴同場時，每個人的
「我們一起走過」與「她注意到我…」躺在**同一份 prompt** 裡，模型的注意力會同時掃過兩份，
於是「你跟 A 說的事」會被當成全場共有的背景知識，B 就變先知。先逐條對過代碼再改：
- **資料層本來就是私有的**（比第二意見描述的更嚴）：`memory`(共同回憶) 進 `MEMOIR` 欄、
  `noticed`(她眼中的你) 進 `KANSHOU_NOTED_TAG_`，**兩者都寫在那個人自己那一列**，
  寫回去之前還有一道地點比對——不在這個地點的人，AI 就算硬寫也寫不進去。
- **在場名單本來就是物理隔離的**：在場＝同 `COL.PC.LOC`，不同格的人卡片根本不組進 prompt。
- **洞在提示詞，而且只有半句**：`perform` 寫的是「在場的人並不知道自己被這樣寫著」
  ——防了「NPC 讀自己的卡」，**沒防「NPC 讀隊友的卡」**。補完同一句的另一半：
  「每張卡上的事是我跟那個人之間的事，其他人手上有的，僅限於自己在場時看得到聽得到的那些。」
  這是**資料作用域的界定**，不是演法指令；住 system 快取段（0.25 倍），每回合多十幾個字。
⚠ 第二意見給的原句是「嚴格禁止跨角色共享秘密…絕對無從得知，不可展現出先知般的知情反應」
——三個否定式，`check_prompt` 會直接擋，而且正犯玩家自己立的規矩（「【禁】寫就不要再給他
例句了，說越多它會越想歪」）。**好的診斷不等於好的句子，診斷採納、措辭重寫。**

🗺️ **同批收窄 ★【這座城裡還有誰】的報位授權**。原本寫「我問起就照認識的樣子回答，
**也可以說出對方此刻在哪**」——後半句讀起來像系統授權，兩個副作用：沒問就主動通報場外動向，
以及報得像 GPS（清晨 06:30 精確說出朋友在哪一格）。改成
「我主動問起時，依彼此交情與日常作息回應對方的去向。」
⚠ **名單上的地點本身要留**（`_list` 仍然是「凜在凜的家」這種逐字事實）——那是系統真值、
玩家真的能走過去，而當初開這個口子正是為了修「舊版只給名字不給地點，AI 只好自己編一個地方」。
改的是**怎麼說出口**，不是拿掉事實：現在她會演成「這時間的話，櫻應該已經去店裡了吧」。

玩家：「其實我應該直接在這裡打造一個類似酒館就好？！」→「符合自由 玩家自己決定增減」。
先把鑑賞 prompt 量了一遍再切：事實（GAS 裁定）與技術契約（JSON／分段／在場驗證）約 2500 字不交——交了玩家一改就壞整局；
「怎麼寫」的口味約 965 字、13 段，交。
- **預設留在 `.gs`、不搬進試算表**：十三支掃描器只看 `.gs`，搬走＝`check_prompt`／`check_pronoun` 對這 13 段失明。
...
- **`def` 不下傳前端、面板不顯示它**（2026-09 玩家：「把我的設定目前就是預設值 但是不要顯示內容給玩家看到」）：
  預設值就是調好的提示詞本體。攤在面板上，玩家會照著那個句式改、也會在敘事之外先讀到一次敘事的骨架——出戲。
  所以每列多一欄 `hint`（一句話講「這段管什麼」），`actionKanshouGetStyle` 回 `hint` 不回 `def`：
  連封包裡都沒有那串字，不是只有畫面藏起來。
  ⚠ 灰字一拿掉，舊介面（文字框 placeholder＝預設句 ＋ 啟用勾選）就無法讀了——玩家原本是靠灰字/黑字/勾沒勾
  去推自己現在是哪一種。所以同一刀改成三顆明講的鈕【預設｜自訂｜關閉】，`ksMode(m)` 是三態唯一判讀口。
  後端的狀態規則一條都沒動（缺列＝預設、空文字＝預設、`0`＝不送），三態只是把它照實畫出來。
  ⚠ `ksMode` 把「正在編輯」排在「已關閉」**前面**：三顆鈕是互斥狀態，關掉的那段按【自訂】卻沒反應就是死按鈕
  （第一版寫成 `!m.on ? 'off' : …`，check_ui 的分頁測試當場抓到）。存檔時 `on:true` 一併把它開回來。
- **面板分四頁**（2026-09 玩家：「那13段 這樣一長排 很難設定 排版分類一下」）：分類依【這段在管誰】
  （文筆／你／對方／世界），**刻意不用 `slot` 分組**——slot 是「進哪一段提示詞」的技術欄位，
  玩家讀不出意思，拿它當 UI 分類是把實作細節推給使用者。兩者從此各管各的：`cat` 只影響畫面，
  `slot` 只影響組裝，提示詞一律照 key 點名、不吃表的順序。
  ⚠ 分頁本質是**把狀態藏起來**（你改過的那段可能在別頁），所以每個分頁掛一個角標＝那一類有幾段不是預設。

### `sanitizeAiData_`　<sub>Gallery.gs</sub>
...
### `kanshouPcIdx_`　<sub>Gallery.gs</sub>
...
### `KANSHOU_DAILY_TRANSLATE_SYS_PREFIX_`　<sub>Gallery.gs</sub>
...
### `kanshouDailyTranslateCall_`　<sub>Gallery.gs</sub>
...
### `translateLookToDaily_`　<sub>Gallery.gs</sub>
...
### `KANSHOU_MET_COUNT_TAG_`　<sub>Gallery.gs</sub>
...
「越想要的東西越裝作不在意…這些是什麼」「感覺她也不是這種個性的人」「懷疑是當初為了設定而設定」。
三句都成立，而且證據就在我們自己的筆記裡：

1. **出身**：`CODE_NOTES` 原文寫著「DAILY_LOOK 改為四段[外貌本相][氣質舉止][自稱口氣][私密一面]，
   **對齊 PERSONA.traits/PREF 格式**」——它是為了對齊格式生出來的欄位，不是角色需要它。
2. **手感**：25 句裡 6 句共用同一個模板「越X越（裝作／用）Y」。那是填格子的手感。
3. **本質**：生成端的提示詞得**明文叫 AI「這一格禁止跟萌點重複」**才寫得出來
   （`dailyMoeHint`／`lookPrivateHint` 兩個參數的存在就是為了這件事）。
   要靠一條禁令才能跟另一欄區分開的欄位，本來就是同一欄。

所以整組退休，不是只把它藏起來：種子四段→三段、`TRAIT_SLOTS_` 3→2、生成端改產三短句、
兩支 translate\* 各少一個去重參數、`looksToTraitParts_` 不再補第三格。
⚠ `traitParts_` 保留相容：舊局存的三、四格讀到就地剝掉（`a.splice(2, ...)`），不必回頭改資料。
⚠ 這是「量測工具看不到的重複」的第二個例子（第一個見 `_spotlight_` 那節）：
`check_back_not_personality` 對萌點/私密一面只量到 <10% 字面重疊——**功能上的同類，字面不會像**。
...
### `kanshouRestBody_`　<sub>Core_Settings.gs / Gallery.gs · 2026-09</sub>

鑑賞的 `PHYSICAL`（肉體）只有一個寫入點：AI 的 `physical_state`，而且是
「有吐才覆寫」（`if (nCleanState)`）。沒有任何地方會清掉它——所以 AI 某回合寫了「腿還在發軟」，
那句話就會跟著那個人走過好幾天、好幾個地點，跟後來發生的一切打架。
這是玩家要查的「新的變動資料與原廠設定衝突」裡最乾淨的一個：**變動資料自己卡住不退**。

修法沿用同一段程式碼上面既有的先例——「🤝 睡覺自然放手：牽手不跨夜」——【一天結束】時一併歸位。
⚠ **同一天內刻意不清**：那一欄寫的就是此刻的身體，當天還在是對的。
⚠ 歸位要 `dirtyPcRows.add(idx)`：`actionPlay_` 只寫有標記的列，改了記憶體不標記＝靜靜丟掉。
⚠ 驗的時候會踩到**兩段式就寢**（`kanshouNightSceneNames_`）：身邊有羈絆 ≥60 的人時，
...
稽核 25 條抓到 4 條寫成了**劇情弧或條件分岔**：
「盡忠職守、初期保持距離，逐漸動搖」「唯對少數有趣之人起興致」「與共鳴其瘋狂的御主引為摯友；否則貌合神離」
「冷眼試探、出手無情，認可方鬆動」。

為什麼這是問題：① 那是把劇情走向先講給 AI 聽，它會照著演（show-don't-tell 的反面）；
② 關係的深淺**已經由好感／契約在管**，態度欄再寫一次「會慢慢變」就是兩個真實來源在打架。
改成四條單一核心立場（例：「以騎士之禮盡忠，公私分明地隔著一步距離」）。
`check_seed.py` 的 `TOMASTER_BAN_` 把「逐漸／初期／否則／一旦／；」這類措辭改成機器擋。

**但寫成單一立場只解了一半**——玩家接著指出真正的問題：「每次叫 API 都是新的呼叫，只會一直重複讀取
我們給的資料。如果她其實已經跟玩家打好關係、但實際資料沒有變動，她會就一直抱持原廠設定。」
說得對：卡片是靜態的，關係卻一直在動，兩者一分岔，角色就永遠出不來。
所以態度欄改成**兩條路**（`bondStance_`／`BOND_STANCE_`）：
- **我方從者**印「此刻對你」＝依 `COL.PC.BOND` 查階段表（45/55/70/85 四階），關係推進、卡片跟著動。
  好感 <45 退回種子 `toMaster`——剛締約時每個人的距離感本來就不一樣（EMIYA 嘴硬、美杜莎護主），
  那是角色特色；一旦真的建立起關係，就不該再由原廠設定說了算。
- **敵從者**維持「對自己御主的態度」：那句講的是他跟【他自己的御主】的關係，本來就跟你的好感無關。
⚠ **鑑賞完全不受影響**——`servantCard_` 只有 solo 在用，鑑賞走自己那行
`【在場人物】…｜關係:她是你的X(好感:N…)`，關係本來就是活的。玩家擔心的「鑑賞用變動的會卡住」不會發生。
⚠ 同時把態度欄**從卡片第一欄後移**到口吻之後：它原本是 AI 讀到的第一件事，權重過高。

### `kanshouSummonClash_` / `KANSHOU_SRC_TAG_`　<sub>Gallery.gs · 2026-09</sub>

三位客串英靈（斯卡哈-Assassin／伊莉雅-Caster／恩奇都-Lancer）長年掛在
`KANSHOU_SUMMON_BLOCKED_IDS_` 裡「暫時不開放」，玩家要求解封。查下去真正的問題不是他們，
是**撞名**：斯卡哈有 Lancer/Assassin 兩種靈基（真名同一個），伊莉雅有 Master/Caster 兩個版本。
鑑賞整層是**用名字認人**的（AI 指名互動、好感寫回對應列），兩個同名的人同時在場就會寫錯列。
封鎖只是繞過症狀。

根源解：① 建列時蓋【英靈源】戳記（來源種子 id），② 召喚查重比 id → 比真名 → 舊列才退回跨名比對。
於是兩種姿態各自都召喚得到，但**同一位英靈只能有一種在場**，訊息也講得清楚
（「已經存在於這個世界了」vs「同一位英靈只能有一種姿態在場」）。
⚠ 伊莉雅-Caster 的真名是「伊莉雅絲菲爾·馮·愛因茲貝倫（Caster install）」，跟 Master 版的真名比不出相同；
...
- **`golden_fleece` 從「純門面」接上數值**：它原本引擎零接線（說明卡自己寫著「無戰鬥數值」），
  玩家 2026-09「新增一個 +10 血量標籤給他吧…算是彩蛋 不想砍掉」。
  作法是開一張 `HP_BONUS_FX_`（fx → +HP）查表，`servantMaxHp_` 多收一個 skills 參數去加它——
  不是在建列點寫 if。加下一個效果＝往表加一列，三個建列點與 resync 自動吃。
  ⚠ 這個數字同時活在說明卡裡，所以 `check_fx.py` 補了第三道逐格對答案。
...
### `getNickname_`　<sub>Gallery.gs</sub>
...
### `sanitizeNickname_`　<sub>Gallery.gs</sub>
...
### `actionKanshouSummonHero`　<sub>Gallery.gs</sub>
...
### `actionEnterKanshou`　<sub>Gallery.gs</sub>
...
### `actionBackfillKanshouAi`　<sub>Gallery.gs</sub>
...
### `actionKanshouCompanions`　<sub>Gallery.gs</sub>
...
### `KANSHOU_STYLE_MODULES_` 的 `world` 那句　<sub>Gallery.gs</sub>

🧭 **2026-09-23 地點中立**：玩家「有說這裡是冬木市嗎？能不能不預設地點？一開始從冬木開始，後續去到哪都可以變動」。
舊句「和平的現代冬木市，大家都是住在這裡的普通市民」每回合都在說「你們在冬木」，跟【場景】可以寫到任何地方
互相打架。第一版改成「故事從冬木市開始，現在在哪跟著【場景】走」，玩家再糾正：「我要冬木開場而已，不是每次呼叫都塞給 AI」——
於是那句只剩「和平的現代日常，大家都是普通市民」，冬木搬進開場布景「冬木市，我的房間」出現一次，
（同日二稿：玩家要「帶點奈須味道但要日常」——魔術／技能／寶具都在、只拿來過日子，偏離日常就用微搞笑拉回；「點到為止」是刻意留的煞車，這條每回合都送、沒煞車就是回頭加料。三稿把例子從「修水管、挑晚餐」換成「會騎乘的人開車厲害、有直感的人聞得出好吃」——玩家「微搞笑不是真無厘頭亂掰，要有點小依據」：用法要從那個人真正的技能／寶具長出來。四稿加「都還在身上」「寫成那個人順手做了什麼」：LANCER×ARCHER 實測「都在」被讀成道具——LANCER 扛著長槍去河邊野餐，還把「A級的騎乘與敏捷」階級寫進旁白。五稿把兩個括號例子拿掉、改指「卡上那個人的本事」：例子變成了樣板，見『kanshouSkillLine_』。）
每回合送的是「★【現在地點】：X」。冬木的正典事實留在
...
多熟交給 AI 從歷史判斷。同一次讀全文還砍了：女×女 ★【身體】那段（改成固定模組 `gender` 一句「性別以卡上寫的為準」）、
尺度模組的預設值縮成「尺度跟著玩家走。」、世界觀那句由玩家定稿。教訓：**每回合的提示詞要當成一篇文章給玩家從頭讀**，
掃描器只擋得住形狀，擋不住「這句沒必要」。

### `worldNoteDropEcho_`　<sub>Gallery.gs</sub>

🐛→✅ **2026-09-23 加料從後門回來**：SABER×RIDER 對照第 3、4 步，AI 把亮起的底細原封抄成 world_note——
「RIDER：討厭鏡子和量身高」「SABER的飲食原則：無法接受馬虎的飯菜」。帳本的亮法是釘選／在場人物／提到，
名字含 SABER 的條目只要她在場就每回合餵，觸發條目做的隔離等於白做。濾網放在回寫前：跟這回合的 ★底細
共用兩字詞的丟掉、kind 人物且名字含同伴名的丟掉（同伴有自己那一列，帳本記的是常民）。

### `KANSHOU_STYLE_MODULES_` 的 `agency` 那句　<sub>Gallery.gs</sub>

🖋️ **2026-09-23 玩家的輸入化成動作與台詞融進去**：自動檢查抓到改後四步裡四步「複述玩家原話」——模型把
「RIDER，妳最近都在看什麼書？借我一本。」原句放進引號當開頭。玩家：「把玩家的輸入寫成台詞、動作自然融入劇情互動呢？」
舊句「narration 從這一步演起」被讀成「先把這一步照抄一遍」。改成「把它化成「我」的動作與台詞融進場景（口氣可以改、
句子可以拆、可以接在動作裡），再從它落地的那一刻往下演」；user 尾端那行同步加註「化成我的動作與台詞融進去」。
自動檢查會分開標「複述玩家原話（N 處）」，下一輪看那個數字有沒有掉。
**沒掉，反而更糟**：Flash Lite 把「化成台詞」讀成字面，原句一字不改地放進去，還學了尾端那對『』寫出
「SABER：『這是……要給我的嗎？』」這種格式。第二版換一個角度：玩家打的字本來就顯示在畫面上，
敘事再抄一次是多餘的——「那一步玩家自己已經看見了，這一段從在場的人對它的反應寫起」；
尾端那行的『』也拿掉（模型會模仿看到的任何括號）。

### `kanshouOutfitLine_`　<sub>Gallery.gs</sub>

👕 **2026-09-23 裝扮句每回合都在被描寫**：定稿版對照裡第 1 步「修長的雙腿裹在過膝黑襪裡」、第 4 步「紅衣黑裙與過膝黑襪」。
「穿著紅衣黑裙過膝黑襪」每回合都送，跟喜好以前的狀況同一個形狀。但裝扮跟喜好不同：換了衣服模型得知道、脫到一半
更得知道（玩家「當初也是想要有脫衣服的狀態」）。折衷＝**狀態照存、句子看情況送**：這件衣服還沒講過（第一回合、換裝、
AI 的 appearance_extras 寫回）才送；玩家提到衣物詞或肉體狀態不是如常時也送；其他回合歷史裡有。講過的存 MEMORY【裝扮已述】，
跟【換裝】對得上就不再送。⚠ 同一句改動會讓 `style.js` 的「逐字回到基線」誤紅——比對前剝掉「穿著X。」那一句，
別把整條 live 卡加進 VOLATILE（那會連【玩家原話】那行一起吃掉）。

### `kanshouSeason_`　<sub>Gallery.gs</sub>

🐛→✅ **2026-09-23 ★【此刻】的數字被整串念進敘事**：兩輪 A/B 十步裡四步開頭是「2005年12月20日清晨六點四十分」。
那行本來是「給你判斷光線、氣溫用的」，但模型看到數字就當成要寫的事實。玩家問「是不是就不要了」——
不能整行拿掉：清晨與深夜寫出來的場景不一樣，夜未眠／睡覺那些鈕也靠時段在講話。改成只給兩個詞
「冬天的清晨」，年月日時分留在 HUD 給玩家看。`style.js` 釘住「這一行沒有數字」。
**第二輪（同日）**：改成字之後，SABER×RIDER 對照五步裡四步開頭是「冬天的清晨帶著幾分薄霧…」「冬日清晨的冷空氣…」——
模型把那行當開場公式。跟裝扮句同一個解法：講過就不再送（【此刻已述】），時段換了才送一次。節奏那句留著，它從沒被念出來過。
**第三輪**：只送一次之後，模型的時鐘開始飄——玩家第 3 步說「下午去新都」，敘事就寫「午後的陽光」，
第 4 步「西斜的夕陽」，第 5 步「迎接夜晚」，而遊戲時鐘還停在清晨六點五十。所以玩家這句話含時間詞
（`KANSHOU_TIME_KEYS_`）就再錨一次「仍是冬天的清晨」。三種送法：沒講過→送；時段換了→送；提到時間→「仍是」。

### `loreEntriesFromSkills_`（~~kanshouSkillLine_~~ 已改成觸發條目）　<sub>Gallery.gs</sub>

2026-09-23 玩家問「AI 有知道自己的特性嗎？」——沒有：鑑賞卡只有名字／性別／性格／外貌／準則，技能寶具一字未送，
模型拿真名自己撈記憶，EMIYA 被寫成「憑藉著敏銳的直感」（他是心眼）、吉爾伽美什「用『騎乘』的本事」開車（他沒有騎乘）。
後者是世界線那兩個括號例子（會騎乘的人開車、有直感的人聞得出好吃）變成樣板的證據：金閃閃×SABER 一輪裡
SABER 每步都『直感』、金閃閃拿到不存在的騎乘。例子拿掉、資料上卡：種子本來就有技能與寶具，只給名字不給階級
（「A 級騎乘」那種話就沒東西可抄），世界線改指「卡上那個人的本事」。單一真實來源：種子改了卡跟著改。

**同日再改：本事不上卡、走觸發條目。** 上了卡之後 flash 每步都掏：火鍋沒人提能力就有王財漣漪、「魔力放出——不，我是說」、
風王鐵鎚三處。玩家：「不能用世界書嗎……能力 本事 特性 寶具？」——對，這正是觸發條目的形狀：每回合都看得到就會想用，
提到才給就只在提到時用。技能靠 `fx` 查 `LORE_FX_KEYS_` 拿日常關鍵字（騎乘→車／開車／機車，王之財寶→寶庫／有沒有／工具，
直感→好吃／哪家／預感），加一個 fx 就是往表加一列；寶具認名字＋「寶具」＋`LORE_NP_KEYS_`。內容只有 `本事「X」`／`寶具「X」`，
怎麼用交給世界線那句。單字 key 一律登記進 `KANSHOU_LORE_KEY_ALLOW1_`（探針釘住）。

### `kanshouLoreStr_`　<sub>Gallery.gs</sub>

📖 **2026-09-23 觸發條目**：玩家「AI 常常拿這些出來表現，除非玩家有提到才可以去用」。兩輪種子 A/B
量到的形狀就是這個——「喜歡寶石、討厭電子產品」每回合都在 system 卡上，模型每回合都找機會讓凜摸寶石。
角色扮演圈的解法是 SillyTavern 的 World Info：常駐資料與關鍵字觸發資料分兩層。我們照它的 `character_book`
規格（`{keys, content}`）在 GAS 裡實作，只掃**玩家這一步的訊息**——AI 自己寫的字不算，否則它端一杯紅茶
出來就會觸發紅茶條目，下一回合再端一杯，變成自我餵養。要連敘事一起掃就開 `KANSHOU_LORE_SCAN_AI_`。
亮起的段落放 user prompt，system 逐字不變。沒有 `book` 的列從 PREF 第三、四格自己長條目（原創英靈／
改命過的），所以玩家在改命面板改的喜好照樣會生效。

**同一天第二輪（玩家「還可以塞進去的都塞進去…提到時候他要有正確反應」）**：共同回憶（釘選常駐、其餘提到才亮）、
世界帳本（鑑賞軌改成釘選／在場人物／提到才餵，solo 戰記照舊——局勢層級的事不等玩家提）都併進同一套亮法。
沒有 keys 的自由文字用「共用兩字詞」（`loreOverlap_`）判提到了沒；功能字與泛用詞（喜歡／我們／今天…）要剔掉，
否則「我喜歡妳」會把每一條「喜歡 X」都點亮。**種子經歷整格退休**：玩家看到那 22 句「借住在士郎家、在商店街打工」
說「這算是經歷嗎…都是多餘的」——它們是我們編的，模型從名字就知道她是誰；`RETIRED_DAILY_BACK_` 只留來洗既有列。
⚠ 引擎裡那張功能字表要用 `\u5979\u4ed6` 寫她他、把「不」放到最後——否則代名詞與否定式兩支掃描器會把一張字表當成提示詞。

### `worldFeed_`　<sub>Gallery.gs</sub>

📍 **`AT`＝這條長在哪個地方。** 玩家原話：「我如果在 A 村莊有農場跟小雞，我移動到 B 城鎮進行劇情後，
我下次回到 A 村莊，我的農場、我的雞都應該要在。」

一般條目走相關性計分、搶 `KANSHOU_WORLD_FEED_MAX_` 那 6 個名額；有 `AT` 的走另一條路：

- 站在那個地方時**必定餵回**，不看分數也不占名額（自己的上限是 `KANSHOU_WORLD_AT_MAX_`——
  一個地方堆了 20 條會把整段提示詞吃掉）。
- 人在別處時**不餵回**：農場不會跟著你走到 B 城鎮。但玩家自己講到名字時要放行，
  不然 AI 會演成沒這回事。
- **免疫淘汰**（連同 `OWN`，你自己開的店）：很久沒去，回去的時候它也得還在。這是「世界會留下痕跡」
  真正兌現的地方，靠「最近有沒有被提到」排序的淘汰法剛好跟它相反。
- `at` 指向查無此地的名字就當沒填——不然那條永遠餵不回來，是最安靜的那種壞掉。
- 餵回時名字不在內容裡就補上：只餵「村口那塊地被你圈起來種菜」的話，AI 講不出那是「我的農場」。

### `actionKanshouParty`　<sub>Gallery.gs</sub>

🫂 同行名單的唯一入口（op = `add`／`drop`／`clear`）。2026-09 玩家定案：「我可以指定 AI（類似牽手同行，他必須回應我）跟我一起，其他 AI 也可以出現，但就是很薄的背景板」。

**為什麼不再看 LOC**：舊版「在場」＝跟玩家同地點，於是整個鑑賞得養一套位置模擬（日常落點骰、老地方加權、住處、同去提議、移動帶人…）才能讓「想見的人剛好在」。玩家講白了這件事的本質——「應該要把他們當成一串資料，不用糾結他們在哪個位置，如果這回合有出來互動再進行資料維護」。所以判準改成玩家自己指定的一份 id 名單，LOC 退化成「這一幕在哪」的衍生值。

**只認 id 不認名字**：「鑑賞眾生」是全帳號共用的一張表，不同玩家的同伴同名是常態，靠名字比對會撈到別人的列。

**`add` 順手把對方 LOC 落到玩家所在場景**：不然加進來的人要等到下一次 `actionPlay` 的同步迴圈才就位，中間任何讀 LOC 的地方（地圖人數、住處）會先看到舊值。

### `KANSHOU_PARTY_TAG_`　<sub>Gallery.gs</sub>

🫂 同行名單存在【玩家】那一列的 MEMORY（`【同行】id,id,id`），不是各自存在同伴列上——「誰跟著我」是玩家的狀態，一個數只存一處；查名單也就不必掃全表。

**舊存檔遷移只種一次**：`actionPlay` 第一次看到玩家列沒有這個標記時，把此刻同場的人收進名單（上限內），同行制上線那一刻人不會憑空消失。判準刻意是「寫過沒有」（`makeTextTag_` 的 `has`）而不是「值空不空」——玩家自己按清空是**空值**，屬於寫過，不會被重新種回去。這兩件事看起來一樣、意思相反。
...
### `actionKanshouSetSex`　<sub>Gallery.gs</sub>
...
### `dialogueFormatRule_`　<sub>Gallery.gs</sub>
...
### `buildDefaultSystemPrompt`　<sub>Gallery.gs</sub>
...
### `rel_changes`　<sub>Gallery.gs</sub>
...
### `kanshouClockInfo_`　<sub>Gallery.gs</sub>
...
### `KANSHOU_NIGHT_SCENE_TAG_`　<sub>Gallery.gs</sub>
...
2026-09-23 補 LANCER／ARCHER（庫丘林、EMIYA）：日常裡御主就是這樣叫的，卡上寫「庫·丘林」「無銘」玩家反而接不上。
...
### `saveGameHistoryBatch`　<sub>History_Sync.gs</sub>
...
### `escapeHtml_`　<sub>History_Sync.gs</sub>
...
### `readRecentPlayerRows_`　<sub>History_Sync.gs</sub>
...
### `STATE_PRE_DATA_`　<sub>Router_Action.gs</sub>
...
### `handleGameAction`　<sub>Router_Action.gs</sub>
...
### `OWNERSHIP_CHECK_EXEMPT_`　<sub>Router_Action.gs</sub>
...
### `STATE_AFTER_ACTIONS`　<sub>Router_Action.gs</sub>
...
### `buildTagsPayload_`　<sub>Router_Action.gs</sub>
...
### `actionUpdateRelTag`　<sub>Router_Action.gs</sub>
...
### `actionSetNickname`　<sub>Router_Action.gs</sub>
...
玩家：「御主不要上戰場好了 感覺好怪喔 都改成指揮從者對打就好」。查下去發現「上戰場」其實只有兩塊，
其餘早就是指揮制（斬首是號令從者去斬、出戰是從者交手）：①`STANCE_SHARE_` 讓御主替從者扛 0/5/10% 的傷，
②`master_melee` 把御主體術變成從者的傷害加成。兩塊一起拔。
- **拔的是戰鬥，不是資源**：電池燃血、補魔燒迴路、令咒反噬都留著——那是御主的代價，不是御主在打架。
  這條界線很重要，因為玩家同時提過「魔力條也給到從者身上」，那會把代價也拿掉，御主就只剩一個名字。
- **`master_magic` 刻意留**（限 Caster）：御主在後方引動魔術支援是原作常態，不是近身。拔了還會讓
  創角的「魔術階位」失去唯一的戰鬥消費端。
- **體術階位變成純演出**：這是這次改動唯一的損失，記在這裡——它現在只進 `masterCard_`。
  之後若想給它消費端，合理的方向是「被狙擊時逃不逃得掉」，而不是把它變回傷害。
- **`stance` 沒砍**：它還在決定接近敵人時對方察不察覺（`arriveStanceNotice_`），以及戰報裡御主站得多前。
  前端藥丸的 `share` 欄位已拿掉，別再加回來。
- **🫱 `_masterCatchLine_`（同輪加回）**：玩家「可以幫忙 但是不扣血! 之前有從者被打大傷害 御主去接住的畫面 那樣我很喜歡」。
  所以「御主幫忙」這件事玩家要的是**畫面**，不是數值——舊版把它綁在扣血上，才會兩個一起被拔掉。
  現在它是純敘事：條件用既有的 `dmgSeverityWord_`（最重一擊 ≥15%），一滴血都不動。
  ⚠ 它跟同一段的鐵律「不可寫替從者擋下攻擊/以身相代」**不衝突**，因為時序不同：擋是在攻擊【之前】、
  接住是在攻擊【之後】。這一點在提示詞裡逐字寫明，不然模型會把兩句讀成互相打架。
- **🐛→✅ 切字串切出孤兒運算式**：拔分擔那一刀我把 `_masterJoinLine` 的鏈式 `+` 切斷，留下
...
兩個問題——① 那是給人看的顯示字串，格式一改判定就靜靜失效；
② 沒回 `clock` 的動作完全不受檢查，而 `np_respond` 正是「會推進一小時卻不回 clock」的那一支，
於是在第 14 日深夜按下應對就能跨過時限繼續玩（實測第 15 日 00:00 仍 `defeat` 未設）。
改成從**資料**問日子（`getClock_`），並優先複用 handler 交棒的 `STATE_PRE_DATA_`——
...
### `SEED_SERVANTS`　<sub>Seed_Codex.gs</sub>

🔎 **2026-09-23 逐角色對官方設定（玩家「要確定是官方設定！」）**：只動「喜歡、討厭」兩格與一處外貌，
資料來源＝Fate/complete material／Fate/Zero material 的人物 profile（經 TYPE-MOON Wiki、ピクシブ百科、
アニヲタWiki 轉載；本機連不到那些站，是用搜尋摘要逐條核的）。查無官方資料的寫「無」（`QUAD_EMPTY_` 會讓卡片不印）。

| 種子 | 官方 好き／嫌い（苦手） | 改前 → 改後 |
|---|---|---|
| Saber | 細かい作業、ぬいぐるみ／大雜把な食事、裝飾過多 | 美食與獅子玩偶、恃強凌倪 → 獅子布偶與細緻的手工、過度的裝飾與馬虎的飯菜 |
| EMIYA | 家事全般／正義の味方 | 下廚與修東西、矯情場面話 → 一切家事、正義的夥伴 |
| 庫丘林 | 氣の強い女、子供達…／回りくどい方針、裡切り | 釣魚與湊熱鬧（釣魚是特技）→ 氣性強的女人與小孩、拐彎抹角的做法與背叛 |
| 美杜莎 | 酒、讀書、蛇、馬／鏡、身長測定、馬刺 | 閱讀與騎車、被過度注視 → 讀書與酒、鏡子與量身高 |
| 美狄亞 | 寡黙で誠實な人、可愛らしい服／筋肉ダルマ | 洋裁手作、輕率的承諾 → 可愛的衣服與寡言誠實的人、肌肉笨蛋 |
| 小次郎 | 花鳥風月／特になし | 討厭「喧鬧紛擾」→ 無 |
| 赫拉克勒斯 | −（狂化）／− | 搬重物與小孩、重要的人受委屈 → 無、無 |
| 吉爾伽美什 | 自分、権力／自分、蛇 | 重機與稀奇玩具、平庸無趣之物 → 自己與權力、蛇 |
| 迪盧木多 | 友情、仁義／戀する乙女、嫉妒深い男 | 堂堂正正的較量、趁人之危 → 友情與仁義、戀愛中的少女與善妒的男人；**外貌 墨綠髮→黑髮（右眼下淚痣）** |
| 伊斯坎達爾 | 世界征服／母親 | 酒宴與高談闊論、小家子氣的計較 → 征服世界、母親 |
| 吉爾德萊 | おてんばな少女、女性的な少年／政治と財政管理 | 厚重的舊書…→ 男孩子氣的少女與陰柔的少年、政治與理財 |
| 咒腕之哈桑 | 自分の闇、忠義、寝正月／自分以外の全て | 獨處的時光、多餘的閒談 → 忠義與正月睡到飽、自己以外的一切 |
| 蘭斯洛特 | （FGO 我的房間台詞）熱い戀愛と劍術／自分 | 安靜的角落、被張揚地感謝 → 熾烈的戀愛與劍術、自己 |
| 恩奇都 | なし（FGO）／− | 新奇事物、傷害他人之舉 → 無、無 |
| 遠坂凜 | 宝石／電化製品（極度の機械オンチ） | 寶石、被人看穿與電子產品 → 寶石、電子產品 |
| 伊莉雅 | 雪、バーサーカー、士郎のエプロン姿／寒い所、貓、納豆 | 被疼愛的感覺、獨自一人 → 雪、寒冷的地方、貓與納豆 |
| 間桐櫻 | 甘いもの、怪談／体育、体重計 | ✓ 原本就對 |
| 衛宮士郎 | 家庭料理／梅昆布茶 | 木工與張羅三餐、有人硬撐著不求助 → 家常菜、梅昆布茶 |
| 藤村大河 | 万物全て／ライオン | 蹭飯的機會、被說像小孩 → 萬物一切、獅子 |
| 百貌哈桑、斯卡哈×2 | **查無官方喜惡** | → 無、無（要補得等玩家給資料） |

外貌其餘 21 筆逐一對過髮色／瞳色，只有迪盧木多那筆錯。性格前兩格（性情）沒動——官方 profile 沒有這一項，那是我們自己的設計。
...
### `upgradeCodexPersonas_`　<sub>Seed_Codex.gs</sub>
...
### `ensureFateSheets_`　<sub>Setup_FateWorld.gs</sub>
...
### `AI_MODEL`　<sub>Core_Settings.gs</sub>

預設值直寫程式碼，指令碼屬性(MODEL / FALLBACK_MODEL)有設就優先。

### `plus`　<sub>Core_Settings.gs</sub>
...
### `dirtyPcRows`　<sub>Gallery.gs</sub>
...
### `advanceHours`　<sub>Gallery.gs</sub>
...
### `CHINESE_NAME_FIELDS`　<sub>Router_Action.gs</sub>
...
### `myGameId`　<sub>Router_Action.gs</sub>
...
### `hrows`　<sub>Router_Creation.gs</sub>
...
### `seedFateCodex_`　<sub>Seed_Codex.gs</sub>
...
**為什麼 `worldSame_` 只比對最近兩天、不掃全表**：第一版掃全表，實測把
...
寫入已經握著整張表了。所以：政策抽成 `worldEvictees_`（純函式，只回答「該砍哪幾列」，
...
讀與回填共用 `worldRow_` 一份欄位對應，避免兩處各寫一份 mapping 日久長歪。
...
### `WAR_`　<sub>War_Engine.gs／War_Router.gs／Script_War.html</sub>

**⚔️ 新聖杯戰爭（2026-09-24 從零重建 solo）。** 玩家回饋「SABER 一直拿寶具到處轟，休息就贏了，很無聊」「設定那麼多數值也沒甚麼用」；
玩家定調「重做吧 你決定方向 我要的核心 按鈕越簡單越好」「魔力系統你也可以拿掉」「你全部重新建構」「畫面簡單 但要有策略感」。

**舊版為什麼無聊（程式量出來的，不是感覺）**：寶具一發抵 4～8 下普攻、沒被對轟就必中、只花魔力不設冷卻；休息 6 小時自己回 60%、敵人同期只回 12%，
空格休息零風險；敵人不成長也不找人；14 天約 200 次行動、每次一小時，沒有一步重要；畫面三個分頁五六十顆鈕。

**新骨架：日與夜。** 白天三選一（打聽／休養／補魔），夜裡選去哪（突襲某位／巡邏／固守），戰鬥每回合四姿態（正面／試探／寶具／撤退）＋令咒。
一局約 50 個決定。任何時候畫面上最多幾顆大按鈕，一顆就是一個完整的決定。
- **真名是底牌**：敵人一開始不明；打聽或交手看出職階與位置，試探／看見對方寶具才知道真名，知道真名傷害 ×1.25。
  反過來，你一放寶具，全冬木都知道你是誰（之後敵人對你 ×1.25、更常來夜襲）。
- **寶具靠御主的魔力，但沒有魔力條**：放完要等 `NP_COOLDOWN` 個早晨魔力才回來；補魔讓魔力早回來 `SUPPLY_CD` 夜；令咒可以拿自己的魔力硬放（原作令咒本身就是魔力結晶）。魔力數值整組拿掉（玩家同意）。
- **戰爭不等你**：敵人每夜照職階個性（`WAR_CLASS_.aggr`）找人打、互相削弱；倒下的人越多，剩下的越急著找出彼此（`HUNT_LATE`/`FIND_LATE`）；知道你據點的會在你固守時夜襲。
- **最後一夜＝柳洞寺決戰**：時間到不是莫名其妙判輸，是剩下的從者一個接一個上來、不能撤退、打到有人倒下；上山前大家都養好了傷（`FINAL_REST`）。
- **數值還在、換成看得懂的話**：六圍照原作讀進來，攻擊取筋力與魔力高者；畫面只給「勝算大／勢均力敵／勝算小／凶險」與傷勢字。
  `STAT_SPREAD` 把階級差距打六折：原作強弱還在，但抽到誰不至於開局就定勝負（沒打折時 Saber 81%、小次郎 18%）。
- **技能＝一張表、固定幾個時機（`WAR_SKILL_`）**：玩家「你可以參考原本的技能寫法，但是不要太複雜，架構一個全新的、你能清楚明白的架構」。
  舊版 solo 的技能是 `SKILL_FX_`／`DEF_FX_`／`FX_TUNING_` 三張表＋散在 `resolveFateBattle_` 的 if；新版每個技能（鍵＝種子 fx）
...
- **技能要看得見**：戰鬥續行撐住、氣息遮斷先手本來只在數字裡生效，玩家看不出自己的技能做了什麼。現在這兩種觸發都會出一行事件、
  念出原作技能名（`warSkName_`）；說書也照這行寫。`warApply_` 只回「撐住了沒」，那一行由呼叫端在自己的句子之後補，順序才會是「先挨打、再撐住」。
- **世界書（玩家「SOLO 也可以世界書吧」）**：跟鑑賞同一支 `loreHits_`，掃的是【這一段發生的事】的文字——事件提到柳洞寺、遠坂宅、
  某個寶具名、令咒，才把那一條原作設定遞給說書（`warLoreStr_`）。寶具條目取種子 `np` 的原文描述，**只寫寶具、不寫持有者**：
  真名還沒看穿的對手放了寶具，說書也不該先替玩家認出是誰（看穿要靠規則，不靠說書洩題）。`WAR_WORLD_BOOK_` 與鑑賞的
  `KANSHOU_WORLD_BOOK_` 分開：鑑賞那份寫的是戰後日常（誰住哪、誰在學校教英文），放進第四次聖杯戰爭會錯。
- **對手情報照看穿程度攤開（`warFoeCard_`）**：知道職階＝位置、傷勢、勝算；看穿真名＝再加御主、寶具可不可以放、技能。
  「寶具可不可以放」是真名最實際的好處——對方魔力沒回的那幾夜，就是放心正面打的時機。
- **規則數字只有一份（`warRules_`）**：開局表單原本寫死「十四夜」，改 `NIGHTS` 就會說謊；現在表單與「怎麼玩」都照後端給的數字寫，探針擋前端寫死。
- **老虎道場＝GAS 先算、AI 只演（`warDebrief_`）**：舊道場讓前端從戰報猜敗因；新版引擎在 `warOver_` 當下記下致命那一場的對手、
...
- **最後一夜在哪要照原作（`WAR_FINAL_`）**：第一版不管哪一場都寫「前往柳洞寺」——第四次聖杯戰爭的聖杯是在新都的冬木市民會館降臨的。
  地點、進場句、下一位上場句一起放進表，按鈕、事件、講評、「怎麼玩」都讀同一格。
- **讀起來要對（逐行讀模擬器的一整局抓到的）**：奇襲那一回合原本照「寶具先放」排序，於是出現「對方解放寶具……趁對方還沒察覺先下手」；
  現在奇襲方永遠先手。早報原本每晚三四行「兩位不明的從者都掛了彩」，把真正要緊的消息埋掉；現在沒人倒下的交手併成一句、有人倒下的才單獨一行。
- **2026-09 整體稽核（另開一個審查者從頭讀）抓到、已修**：
  ① 重新召喚根本沒換人——`warAct_` 手上沒有名冊，只扣次數、印「回應你的是另一位英靈」配同一位；現在路由把 `warSeedCtx_` 傳進去，引擎沒拿到名冊就拒絕。
  ② 說書／道場寫錯列——兩者不取鎖、等 AI 好幾秒，拿開頭記的列號去寫；期間上面有人放棄（刪列）就寫進下一個帳號那一列，連同快取的說書一起送給別人。
  現在寫的當下用「帳號＋這一局的 gid」重找，找不到就不寫（也順手擋掉「舊局的說書寫進剛開的新局」）；前端說書進行中也鎖住「放棄這一局」。
  ③ 只認真的發生了的事——令咒撤退成功、對方的寶具根本沒放出來，照樣「看見那道寶具，認出了對方」；對方先撤走、我的寶具沒放出去，照樣曝光真名、扣令咒。
  `warStrike_` 現在在出手方標 `struck`／`fired`，揭露、曝光、寶具次數、令咒都只看這兩個旗子。
  ④ 敵人的技能卡寫著「出擊的第一擊必中」「守家受傷更少」，但這兩個時機只對玩家生效——看穿真名後攤給玩家看的是假資訊。現在兩邊對稱（`foeAmbush`、`lair`）；模擬器量過勝率沒有明顯移動。
  ⑤ 決戰打倒一位之後，`st.battle` 已經換成下一位，說書的【對手】寫成還沒交手的那位；現在 `actionWarAct` 記下這一段打的是誰（`narr.foe`）。
  ⑥ 世界書的「肯尼斯包下了整整一層」「韋伯用暗示……」在只知道職階時就把御主講給說書聽了，改成只寫地方。
  ⑦ 手機夜晚：已知的對手一多，名單換行成八排、技能表三行，把按鈕擠出畫面——名單改一排左右滑、技能表點從者卡才展開、按鈕區跟著縮自己捲（`uiplay.js` 在四種螢幕把整局點完量過）。
- **【此刻】是事情發生的那一刻（`WAR_WHEN_`）**：做真模型試跑包時把整局的提示詞印出來讀，才看到白天打聽的那一段寫著「第 1 天的夜晚」、
  固守到天亮的那一段寫著隔天——`warNarrPrompt_` 讀的是結算完的 `st.phase`／`st.day`。現在 `actionWarAct` 在結算前記下那一天與時段；
  召喚在開戰之前，寫「聖杯戰爭開始前的那一夜」。
...
- **新工房一張表單**：真名、職階、性別、六圍（E～EX，點數 ≤ 18，算法見下）、寶具名（階級＝六圍的寶具）、
...
- **點數只算引擎讀的格（2026-09 整體稽核）**：第一版是「六圍全加、≤ 22」，稽核抓到它算了引擎根本不讀的格——
  攻擊只取筋力／魔力較高者、幸運沒人讀、寶具有名字就至少當 C（`NP_FLOOR`），而 EX 在工房算 6、在引擎值 7。
  於是「筋耐敏三格 EX、魔力幸運壓到 E、寶具 D」只花 21 點，模擬器量出 79.9%，比 Saber（74.6%）還強。
  現在點數＝攻擊（筋魔取高）＋耐久＋敏捷＋寶具（至少 C），每格價錢直接用 `warRank_`——工房跟引擎是同一把尺，不會再各算各的。
  上限 18 是量出來的：18 點最強的做法（配最好的三個技能）約 54%，跟赫拉克勒斯同一級；19 點就摸到 68%、太貼近 Saber。
  原作 Saber 用這把尺算是 20 點，所以「做得出好用的、做不出比 Saber 還強的」這句現在真的成立。
- **舊作不被弄壞**：舊工房存過 A+、B- 這種階級，新表單只有六階——第一版一律退回 C（改一次就被降階）；現在取字頭。
  舊技能的 fx 換成同一列的工房代表（心眼／全知全能之星→直感、十二試煉→戰鬥續行），新規則沒有的列給玩家看（`lost`），不再無聲丟掉。
  改既有的一位時不重驗真名：舊作可能沒有中文字，真名本來就鎖住、照表上那一列。
