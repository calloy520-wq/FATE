...
- **技能讀種子、不看職階（`WAR_SKILL_`）**：每位從者只吃自己原作真的有的技能——直感／心眼／全知全能之星／氣息感知看得出寶具預兆，
  戰鬥續行／十二試煉／完全之形撐住第一次致命傷，騎乘撤退，氣息遮斷難被找到＋出擊第一擊，陣地作成守家＋闖進對方陣地會先挨魔術陣，
  狂化加傷不能試探，對魔力減 Caster 的傷，千里眼巡邏必遇，單獨行動打聽多問一條，避矢加護躲 Archer，軍略減寶具傷。從者卡上顯示「技能名：效果」。

**平衡是量出來的**：`scratchpad/war/sim.js` 載入真的引擎＋種子，四種策略各打幾百局（定案數字）：
...
| 一直固守＋休養 | 14% | 28% |
| 狂放寶具（天天補魔、令咒硬放） | 6% | 13% |
| 有想法（先打聽、挑勝算、試探後再放寶具、傷重就撤） | 48% | 71% |

每位從者（有想法的打法）落在 15%（吉爾·德·萊斯）～78%（阿爾托莉雅）：Caster 偏弱是原作（大家都有對魔力，Caster 本該窩在陣地），
Saber 最強也是原作。一局約 55 個決定、平均打 12～13 場。

**原作對照（2026-09-24 玩家：「有符合原作嗎...你不要加太多奇怪的設定喔...型月世界的話勉強還行 盡量 fate」）**：
第一版有三條是我自己加的，已經拿掉——①打倒別人的從者會變強、最後一夜不是你收下的魂灌給活著的敵人
（原作裡敗退從者的魂流進聖杯容器，不是被別的從者吃掉）；②把個人技能當職階通用（戰鬥續行套給所有 Lancer、
千里眼套給所有 Archer）；③令咒配試探可以「看穿真名、這回合不受傷」（原作沒有）。令咒只留強化、以魔力硬放寶具、強制撤離三種用法。
拿掉①之後固守一度衝到 77%（敵人互打完半殘、最後一夜撿便宜），改用原作說得通的兩條拉回來：
上柳洞寺前大家都養好傷、倒下的人越多剩下的越急著找人。
調的過程踩到一個坑：按鈕上「用令咒時的說明」欄位原本叫 `seal`，模擬器把它當成「要用令咒」，每回合都在燒——前端照樣會踩，
所以改名 `sealSub`，真正的旗標只在送出的 `act.seal`。

**GAS 端**：一局一格 JSON（聖杯戰局分頁），不再跟 36 欄、只能棄用不能刪的眾生表打架；每按一次讀一次、寫一次。
戰況與說書分兩格：`war_act` 取鎖寫戰況，`war_narrate` 不取鎖（AI 要跑數秒）只寫說書那格，互不覆蓋；同一個 `seq` 說過就回快取。
按鈕白名單在後端：`warAllowed_` 只認 `warButtons_` 列出來的，前端送什麼都照驗。

**舊版 solo**：選單上留著「舊版」入口，新版確認之後整組拆掉（Router_Battle／Router_Movement／Router_Bond／Engine_Fate／Time_World 大半、
Script.html 八成五），拆的時候照 CLAUDE.md「kanshou 有用到的留」逐一核對。
