...
📖 **檔案地圖已移到 `CODE_MAP.md`**（一頁式：每檔行數／函式數／職責／何時要動它＋全部 action 對照（數字以 §4 表頭為準）＋資料驅動表索引）。
...
- **平行世界設定(2026-06 玩家定案·核心原則)**：鑑賞世界「從未發生過聖杯戰爭」——角色仍是原本的英靈，但不背負戰爭/創傷造成的沉重反差，`persona.look/words`(戰時版，solo專用)一律要先過daily轉換管線(`translateLookToDaily_`/`translatePersonalityToDaily_`，AI原創英靈適用；種子英靈由人工手寫`dailyLook`等欄)才能進鑑賞，禁止任何戰時原始欄位不經轉換直接餵給鑑賞AI——這條原則歷經多輪稽核抓出的洩漏(`persona.back`身世／`persona.speech`+`tic`口吻小動作)已於2026-07修正，完整清單與逐函式track歸屬見 §12。
