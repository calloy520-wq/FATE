  ...
  const aiPrompt = ambushDispatchPrompt_(ambush,
    ...
    function () {
      ...
      return masterCard_(pcData[pIdx]) + servantCard_(pcData[svIdx]) +
        ...
        `${genderFactMana}★【800~1000 字】寫這場供魔：魔力流動只是成因，全篇寫在肉體這一側——接觸、溫度、反應。${LEWD_EXPLICIT_}這一場的節奏由「${svName}」掌控：英靈遠比常人強韌，何時攀頂由「${svName}」自己決定。御主剛燒過迴路，到最後是癱軟的那一方。「${svName}」在魔力上飽足了，肉體這一側還留著沒盡興的餘裕。挑一兩個關鍵瞬間深寫，篇幅全給實際發生的細節。收在餘韻猶存的溫柔。`;
    }
  );
