...
function buildDreamPrompt_(pcName, wish, servantName, cause) {
  ...
  return lead + `\n` +
    ...
    // 🐛→✅ 舊版寫「第二人稱」，跟這段敘事同樣要吃的 miniSystem 規則1(旁白第一人稱「我」，禁用
    //   「你」與上帝視角)直接矛盾——這裡改成一致的第一人稱。
...
function actionNarrateOnly(userData, pcId, sheets) {
  ...
  const miniSystem = `你是《命運停駐之夜》的說書人。用 Fate／TYPE-MOON 筆觸、台灣繁體中文，依指令寫一段劇情。【篇幅以下方指令指定的字數為準，節奏明快不灌水；無指定時 100~160 字】。從者廝殺寫關鍵攻防與寶具威能的張力即可，不必逐回合流水帳。
【鐵律】
1. 旁白第一人稱「我」（玩家＝御主），不用「你」、不用上帝視角。
2. ${dialogueFormatRule_()}
3. 每2~3句用 <br><br> 分段（字數少時 1 個就夠）。換行一律用 <br><br>，不用真實換行、不用其他 HTML 標籤。
4. ★這是純敘事補完，系統底層已結算完所有數值，你只負責寫字、不複述數字。
5. ★對話歷史是「已經發生並結束」的既定事實，只供語氣與情緒連貫；本回合真正發生的新事件，只有這句指令給的內容。
6. ★語氣依【當前狀態】(血量/魔力)與實際勝負決定，不可臆測勝敗；瀕死就是命懸一線，不會是輕鬆閒聊。移動/互動都接續前情。
