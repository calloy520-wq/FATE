...
function buildDreamPrompt_(pcName, wish, servantName, cause) {
  ...
  return lead + `\n` +
    ...
    `★旁白第二人稱「你」＝玩家，寫一段唯美而令人心碎的虛假美夢：讓「演出」暗示願望成真的幸福感，絕不可直接說出願望內容或「這是假的」。${servantName ? `從者依其性格自然相伴。` : ""}${flaw}。\n` +
    `★【鐵律】只輸出夢境敘事，禁選項或系統字樣。`;
}
...
function buildVictoryDreamPrompt_(pcName, wish, servantName) {
  return `【聖杯降臨·已裁定】御主『${pcName}』斬盡了聖杯戰爭中所有的敵對從者，聖杯已然屬於你。\n` +
    ...
    `★旁白第二人稱「你」＝玩家，寫一段真摯溫暖的勝利瞬間：讓「演出」暗示願望終於觸手可及的踏實感與如釋重負，絕不可直接說出願望內容。${servantName ? `從者依其性格自然相伴、給出這一刻該有的反應。` : ""}與敗北的虛假之夢不同，這次無需露出任何破綻——這是真實發生的結局。\n` +
    `★【鐵律】只輸出這段敘事，禁選項或系統字樣。`;
}
...
function actionNarrateOnly(userData, pcId, sheets) {
  ...
  const miniSystem = `你是《命運停駐之夜》的說書人。Fate／TYPE-MOON 筆觸、台灣繁體中文。篇幅依指令指定的字數，沒指定就 100~160 字。廝殺寫關鍵攻防與寶具威能，不寫逐回合流水帳。
【鐵律】
1. 旁白一律第二人稱：「你」＝玩家（御主）本人，只寫你看得見、感覺得到的。旁白【不可】用「我」——場上每個角色引號內的台詞才用得到「我」。
