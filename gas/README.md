...
| `Gallery.gs` | 鑑賞室（後日談）：奪杯解鎖英靈、自由新增、最多 3 名在場、約會/對話。全 `ms_id` 隔離 |
...
- **靈基出力（`outputMod_`）**：從者當前靈基魔力% 影響戰鬥發揮，以 80%（自然維持線）為基準——100%:+5 / 80%:0 / 40%:−10 / 0%:−20，套用在命中・閃避・傷害・寶具威力。供魔充足的御主（凜/櫻）能把從者頂到高出力＝更強；窮御主（慎二）養不起 80%＝從者衰弱。要超過 80% 需主動 `feed`/補魔/獵魔（自然回充只到 80%）。全滿對戰時雙方同 +5，故平衡中性，差異只在實戰供魔能力。
...
- **Archer 遠程開幕**：`cls==='Archer'` 者於近戰前先射一輪（`敏捷×0.4`，不吃寶具以免與寶具開幕雙重加成）——給射手職階遠程身份。模擬：吉爾 30→35%、僵持/頂端不變。
- **潛殺御主**（`act_assassinate_`，Assassin 限定）：嘗試繞過守護從者直取敵御主。氣息遮斷階級 vs 敵預判（直感/心眼）→ 偵測機率。成功＝重創/取首敵御主（取首則其從者失主：有單獨行動→`masterless` 候補再契約、否則消散）；敗露＝守護從者反擊我從者。建議分離・派駐潛入、御主留安全處。（御主級「切嗣式」master-vs-master 暫不做，避免失衡。）
- **遠距離支援／狙擊**（`act_snipe_`，Archer 限定）：御主**親自上前、與敵從者同地**（直面、身邊無護衛）為斥候指示目標，Archer 從**相鄰地（射程 1）**遠程開火：`寶具×0.7+敏捷×0.5`、**不挨近戰反擊**、可斬殺。代價＝敵從者反手揮向暴露的御主（`敵筋力×0.4+5` 扣御主 HP）。需先分離→派駐 Archer 至敵相鄰地，御主再移上敵所在地。
- **撤退/令咒 AI**：敵從者重傷→敵御主可能燃令咒瞬移脫離/治療；玩家重傷→敵御主可能燃令咒追擊。
- **瀕死令咒救援**：從者或御主將死且有令咒→彈窗抉擇（靈基修復 / 接受命運）。
- **分離／派駐／合體**：`分離`後從者脫離同行（御主失護衛）；分離中可用`派駐`（`act_dispatch_`）單獨指揮從者移動到其相鄰地——遠程作戰/站崗/攔截，御主留安全處；`召回`即時會合。前端「派駐」為地圖點擊模式（以從者所在為原點）。
- **鑑賞模式（後日談）**：奪取聖杯 → 該帳號解鎖此英靈（存 `GALLERY` 分頁，`ms_id` 隔離）。登入後選單進入；可自由新增角色（`generateServant_`）、最多 3 名「在場」、點地圖帶焦點英靈各地約會。**敘述走獨立路徑** `afterStorySystem_`／`narrateAfterStory_`（戰後日常、肢體具體可感、親密 fade-to-black）。左欄只顯示名稱＋體況，不顯示 HP。
- **玩家御主死亡**：直接結算（老虎道場）。玩家側不做無御主續戰。
- **NPC 再編組**：戰場上 NPC 之間會動態重組——①NPC 從者被斬、其御主仍有令咒 → 該列轉「失從者御主」(`solo_hours='seeking'`) 候補；②偶發 NPC 御主殞落，其從者若有單獨行動 → 淪「無主從者」(`solo_hours='masterless'`)、否則消散；③`npcRecontract_` 把無主從者＋失從者御主湊成一支**新隊伍**，化為新威脅。（玩家會在傳聞中看到，並可能遭遇。）`solo`/`soloRank_` 仍用於分離供給減免與此處的單獨行動判定。
