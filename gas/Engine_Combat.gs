...
4. physical_state欄位key固定用數字代碼(1=姿勢與動作 2=胸部 3=顏面 4=肉棒 5=蜜穴 6=服裝狀態)，禁用文字key，其餘進narration。1/2/3/4或5(擇一)每回合都要據實填最新狀態，不可偷懶沿用舊值；只有6(服裝狀態，描述玩家指定服裝【當下的凌亂/破損程度】，非更換服裝本身)可省略=維持原樣，脫離接觸改寫「鬆開/餘韻」。★此欄純為系統狀態記錄，narration敘事【絕對禁止】比照逐格謄寫每個部位狀態，敘事仍以第2條為準、聚焦留白。
