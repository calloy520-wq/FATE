  ...
  function attemptWithModel_(model) {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        // 🧠 這顆模型的思考關不掉（OpenRouter 400「Reasoning is mandatory」）：換最低檔再送一次，思考額度加回 max_tokens；只換一次、不算重試。
        if (/Reasoning is mandatory/i.test(e.message) && !reasoningRelaxed) {
          reasoningRelaxed = true;
          payload.reasoning = { effort: "low", exclude: true };
          payload.max_tokens = maxT + REASONING_ALLOWANCE_;
...
// 思考關不掉的模型：退到 low 時加回 max_tokens 的思考額度，敘事本體才不會被思考吃掉。
var REASONING_ALLOWANCE_ = 1500;
