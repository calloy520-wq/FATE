...
function buildDefaultSystemPrompt(isNsfwMode, backLocked) {
  ...
  const _physicalState = {
    "1": "姿勢與動作(≤20字)",
    "2": "胸部狀態(女性適用·男性不適用則不輸出此代碼，≤15字)",
    "3": "顏面狀態(表情與汗水，≤15字)",
    "4": "肉棒狀態(僅男性適用·女性不輸出此代碼，≤15字)",
    "5": "蜜穴狀態(僅女性適用·男性不輸出此代碼，≤15字)",
    "6": "服裝狀態(玩家指定服裝本身不變，只描述它當下的凌亂/破損程度，如領口散亂/半褪至肩/裙擺撕裂，≤15字)"
  };
  ...
  if (isNsfwMode) {
    finalJson = {
      ...
      "intimacy_feedback": {
        "_note": "★physical_state各欄皆角色「自身」當下姿態與肉體狀態，純肢體與感官、禁內心戲，第三人稱填寫，省略=維持原樣，絕對禁寫'自己'。★不適用的器官代碼(4肉棒/5蜜穴二選一，依角色實際性別)直接不要輸出這個代碼，禁止填「無」佔位。npcs每位與player共用此格式，依其實際狀態填寫對應欄位。",
        "player": {
...
3. 器官依實際性別只填一項：男性只填「肉棒」代碼、女性只填「蜜穴」代碼——不適用的那項【直接不輸出這個代碼】，禁止寫「無」這種佔位敷衍(一旦寫入就會永久留在角色資料上)。絕對禁止插入式陽具，以手指/舌/器物替代，嚴禁憑空生出男性器官。
4. physical_state欄位key固定用數字代碼(1=姿勢與動作 2=胸部 3=顏面 4=肉棒 5=蜜穴 6=服裝狀態)，禁用文字key，其餘進narration。服裝狀態(6)描述的是玩家指定服裝【當下的凌亂/破損程度】，不是更換服裝本身(換裝一律由系統按鈕管理)。據實填當下狀態，未被碰的代碼不填；脫離接觸改寫「鬆開/餘韻」。★此欄純為系統狀態記錄，narration敘事【絕對禁止】比照逐格謄寫每個部位狀態，敘事仍以第2條為準、聚焦留白。
