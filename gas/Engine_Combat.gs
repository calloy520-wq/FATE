...
function buildDefaultSystemPrompt(isNsfwMode, backLocked) {
  ...
  function dialogueFormatRule_(example) {
    return `對話格式：（角色動作或神情，例如：${example}）名字：「台詞，或（聲音，如低吟／輕笑）」（角色動作或神情，可省略）。同一段落【不限制】名字出現次數、也不限制括號(動作/聲音)的使用次數與位置——可依演出彈性重複、交錯多輪對話與動作。引號全文僅用一層「」，【絕對禁止】「」內再嵌『』或再嵌一層「」。純背景／環境描述(非角色動作、非聲音)【不使用括號】，直接以敘事文字呈現，且【盡量精簡】——把篇幅留給互動本身(動作與對話)，少花筆墨鋪陳場景氛圍。開頭禁代名詞(他/她)，必指名道姓。`;
  }
