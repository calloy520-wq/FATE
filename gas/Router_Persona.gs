...
function servantCard_(row) {
  ...
  try {
    ...
    var card = `〈${name}·${cls}·演出依據(僅供內化，禁複述設定字面)〉此角色台詞內自稱「${fp}」(僅限她/他自己的引號台詞，敘事旁白的「我」永遠是玩家本人、與此無關)｜對自己御主的態度：${toM || '依真名'}` +
      ...
      (moe ? `｜萌點(不必每回合硬塞，情境對了才自然浮現一次，避免每次都用同一個具體動作重複——若牽涉隨身物品，別每次都靠「摸/看一眼該物品」交差)：${moe}` : "") +
      (tic ? `｜小動作：${tic}` : "") +
...
function masterCard_(row) {
  ...
  try {
    ...
    return `〈御主「${name}」·演出依據(僅內化、禁複述)〉` + (sex ? `性別${sex}` : "") +
      ...
      (moe && moe !== "（待揭曉）" ? `｜萌點(反差·僅供內化，不必每回合硬塞，情境對了才自然浮現一次，避免每次都用同一個具體動作重複——尤其若此萌點牽涉隨身物品，別每次都靠「摸/看一眼該物品」這個最省力的動作交差，多用神情/語氣/其他行為表現這份反差)：${moe}` : "") +
      (back ? `｜身世：${back}` : "") +
      (origin ? `｜出身：${origin}` : "") +
      ...
      `。御主＝玩家所扮演的角色：【可】依其性格/身世自然開口、有神態反應與台詞，讓角色鮮活有聲(別只當沉默旁觀者，show, don't tell：禁把性格詞/特徵/萌點當台詞或由旁白點破)；從者可開口問御主接下來怎麼辦，御主(我)也可以自問該如何是好，也可以不開口、單純用動作/神態/沉默停在那個當下——但【不可】替御主拍板下一步戰略抉擇(是否出戰/結盟/移動/補魔由玩家按鍵定奪)。收尾形式不限於問句：停在思索、一個準備出手的姿態、屏息的對峙都行，同一場戰鬥連續好幾回合時尤其別每次都用同一種問句收尾(如反覆問「接下來怎麼辦」)，換個角度換個形式；不可自己接著演出答案，也不可把劇情快轉越過決策點。` +
