...
// ⚠ 2026-09 玩家「全面語意再確認」：舊標籤是「日常表象／真實內裡」。「表象」的語意是
//    【裝出來的】——但種子資料裡那一格是真的（SABER 就是一絲不苟，不是裝的），
//    22 筆逐筆看過沒有一個是假面。標籤在說謊，資料沒有。順帶新標籤還比舊的短。
var PREF_LABELS_ = ['平常', '熟了之後', '喜歡', '討厭'];
// 🧹 標籤已經講了「討厭的事物」，值再寫一次「厭惡…」就是疊字（種子 16 處，AI 生成的也會這樣寫）。
//    剝在【唯一的渲染出口】而不是去改每一筆資料：舊試算表的列不會因為改種子而更新，而 AI 隨時能再產一個。
// ⚠ 鍵必須跟 PREF_LABELS_／TRAIT_LABELS_ 的字面【逐字相同】——這裡是用標籤名去查表的。
//    2026-09 改標籤時就是漏了這裡：查不到 → 疊字不剝 → 卡上出現「喜歡：喜歡甜食」，零錯誤訊息。
var QUAD_REDUNDANT_ = { '喜歡': /^(熱衷|喜歡)[於的]?/, '討厭': /^(厭惡|討厭)[於的]?/ };
