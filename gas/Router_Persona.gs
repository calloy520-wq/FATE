...
// 🧭 2026-07 玩家實測反映：明明御主是女性，補魔/令咒的親密敘述卻寫成男性插入視角(「當我進入她時」)
//   ——masterCard_ 雖已內嵌御主性別(見上方「性別${sex}」)，但只是一個孤立事實標籤，沒教AI該怎麼據此
//   裁定肢體互動，小模型便預設男性視角。獨立小函式算好配對事實直接餵給AI，技法借鏡kanshou
//   Gallery.gs的genderHintStr分派邏輯(女女配對特殊處理、其餘依實際性別)，但完全獨立成新代碼、
//   不呼叫也不共用那支——solo/kanshou機制須徹底隔離(CLAUDE.md紅線①)。
