...
function narratorSystem_(){
  return [
    ...
    '【篇幅】約 250～420 字，分 3～5 段（段間空一行）：把場景、肢體、神情、對白寫得有畫面有層次，但仍要克制、不流水帳、不硬湊字、不重複先前演過的內容。',
    '【務必扣緊現況】開頭提供的「現況快照」是此刻的真實狀態（時間/地點/在場者/雙方傷勢出力）——敘述必須與它一致：人在哪、誰在場、誰受傷、出力高低都要對上，嚴禁虛構地點、在場者或無中生有的事件。',
...
function narrateJSON_(sys, user){
  var r = callLLM(sys, user, { json:true, temperature:0.9, maxTokens:1400 });
...
function narrateAndExtract_(prompt, memory){
  // 對話專用風格：覆蓋旁白那套「固定 2~3 段小作文」，改成台詞為主、長短自然——避免每句都硬湊字而僵硬出戲。
  var sys = narratorSystem_()
    + '\n【這是自由對話，不是旁白】此刻是「我」與從者面對面交談。請以「從者的台詞」為主軸真正地『對話』——'
    + '依其個性、一人稱、對我的態度與當下好感，自然地回應我剛說的話（可反問、吐槽、沉默以對、岔開話題、調侃或動情）。'
    + '\n【長短隨情境】不要硬湊字數、不要固定段落：該簡短就一兩句俐落台詞＋一個小動作；該深聊才展開。重點是「像真人對話」而非交代劇情。'
    + '對白格式——從者（神態或動作）：「台詞。」；「我」的反應與內心可穿插，但別把對話寫成獨白旁白。嚴禁答非所問、嚴禁忽略我說的內容。'
    + '\n輸出 JSON：{"narration":"以對話為主的演出","condition":"我方從者此刻的外觀/姿勢動作/神情，純外顯、第三人稱、≤24字",'
