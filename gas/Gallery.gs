...
function translateLookToDaily_(name, cls, rawLook, firstP, speech, sex) {
  ...
  var sys = KANSHOU_DAILY_TRANSLATE_SYS_PREFIX_ + "玩家提供一段用「、」或「・」分隔的角色戰時外貌描述" +
    ...
    "[氣質(" + AURA_SPEC_ + "依和平日常情境自然轉化，但性格底色不變；【不可與下方口氣段用相同字眼重複】)]、" +
...
★【設定怎麼用】以下設定是【給你內化的素材】，禁複述字面：靠言行與神態流露，不必每回合硬塞。
★【格式鐵律】traits 【恰好2段】、personality 【恰好4段】，只用頓號「、」分隔，每段是一個【簡短詞組】(不是完整句子)，每段內部也【不要】再用頓號列舉多項；禁數字標籤。
...
★speech：${pron_(finalSex)}講話的調調，限16字，寫成短詞組、不要寫成完整句子。這是給 AI 演這個人的依據，不是給玩家看的。
...
var KANSHOU_STYLE_MODULES_ = [
  ...
  { key: 'enact',      name: '演玩家這一步', hint: '你寫的那一步要被演多細——撐成完整一拍，還是直接跳到對方的反應。', slot: 'sys', cat: 'me', def: 'narration【從玩家這一步演起】，別跳過它、別只當成已經發生的前提：把那一步撐成完整的一拍(具體的動作、距離、觸感、開口時的語氣與視線)，再往下接對方的反應。但【只擴寫玩家真的寫的那一步】——不補沒做的動作、沒說的話，也不改原意與語氣。' },
  ...
  { key: 'length',     name: '篇幅',     hint: '一回合大概寫多長、寫不滿的時候怎麼補。', slot: 'user', cat: 'pen', def: '★【篇幅】：narration 寫 {篇幅} 字，【不可少於下限】——寫不滿就加互動與真實反應，別靠環境描寫充數。' },
...
function actionPlay_(userData, pcId, sheets) {
  ...
  (function () {
    ...
    if (_kind === 'visit') {
      ...
      kanshouInitStr = `\n★【對方開口約你】：『${String(_r[COL.PC.NAME])}』說了${_today ? '今天' : '明天'}${_band.label}在「${_loc.name}」等你——這句話【已經說出口】，由對方依自己的個性演出對方是怎麼提的(慎重／裝作隨口／彆扭地繞一圈才講皆可)。★這是對方單方面的邀約，你答不答應都行，narration 停在等你回答的那一刻，去或不去留給玩家決定。`;
    } else {
      const _r = _pick(_wantPool);
      _stamp();
      kanshouInitStr = `\n★【對方此刻的心思】：『${String(_r[COL.PC.NAME])}』心裡此刻有一件自己的事——是什麼，由對方的個性、你們的關係、這個時間與地點決定(不必是大事，一個念頭、一個想問的問題、一件想給你看的東西都行)。這一回合讓它自然浮出來一次(要不要說破、怎麼說，依對方的個性決定)。★只是一個起頭，怎麼回應留給玩家決定。`;
    }
  })();
...
🛑【角色一致性】：情慾裡生理反應可以有，但說話做事仍照各自的個性，劇情推進不軟化誰。標「事實：」的是系統裁定的結果、不是演法——怎麼表現依那個人的個性。`;
