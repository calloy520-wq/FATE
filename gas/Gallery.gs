...
② 【每一句「角色說出口的台詞」都在「」前冠上說話者的名字——硬格式·不可省】——例：櫻「風音學姐……」、凜「哼，隨便你。」。跨回合也絕不認錯人。喘息／擬聲若混在某角色的台詞串裡，跟著那句一起掛在她名下。★唯一例外：以第一人稱『我』推演的【玩家本人】台詞不必冠名，直接寫「……」即可——★玩家輸入若本身是一句台詞(疑問/吐槽/驚嘆等)，必須語氣照原樣原句寫成這樣的「……」引號直接呈現，禁止改寫成「於是我問了一句／接著說了什麼」這類轉述句代替，只有純動作、沒有台詞內容的輸入才用敘事帶過。
...
function buildDefaultSystemPrompt(includeMasterNote, includeOptions) {
  ...
  const _appearanceExtras = "角色當下穿著/未穿的衣物狀態(第三人稱·≤20字·名詞短語如「絲綢襯衫」「牛仔褲」「浴巾」「全裸」·禁動作句「換上了…」·禁寫成所在環境或姿勢如「在水下」「泡在浴池」——那是場景描述、不是穿著)";
  ...
  const finalJson = {
    ...
    "inner_monologue": "【純思考·不顯示·約50字】第三人稱總結主NPC本回合開始前的狀態([性格尊嚴]vs[當下情緒身體])·承接歷史勿歸零·但若歷史裡的某人此刻已不在【目前在場人物】名單，這裡要先意識到她已不在場，不可延續舊歷史當她還在",
...
6. options(命運的抉擇)：只能基於本回合 narration 實際寫出的內容出題——玩家對【在場人物】在當下場景真能做到的動作(對話/肢體/氛圍延伸)；禁建議移動地點、呼喚/尋找不在場角色(系統不支援，點了也做不到)；禁塞入跟本回合無關的萌點/個性/背景設定字面(如「提點她的家電困擾」)——那是玩家當下不可能知道的資料，等敘事自然演出讓他發現即可。
7. appearance_extras(裝扮)：本回合劇情真有穿脫/更衣/入浴等具體動作才填新值·沒發生就留空(沿用玩家已設定或先前記錄的裝扮)·不准因為「這身打扮好像跟這幕不搭」就自行合理化悄悄改寫或省略——那是你腦補的修正，不是敘事真的發生的事；玩家指定的裝扮視為既定事實，直到劇情真的讓她換裝為止(格式/場景姿勢禁忌見上方schema欄位說明)。`;
...
🔥【主動掌握·玩家已開啟】：在場同伴主動掌握節奏·把互動推進至「實際發生」·拒絕無止盡的曖昧空轉。①主動程度嚴依好感分階(好感<40僅言語試弄不肢體糾纏；高好感才解鎖進一步肢體接觸；80+方可索求到底${_kanshouHypnosisActive_ ? '；佩戴中的『催眠暗示』類道具例外·不受此好感分階限制' : ''})；②保持角色一致性(冷傲者冷傲地主動、羞怯者鼓起勇氣踏出一步)·禁霸道模板；③禁對玩家造成真正傷害；④文字尺度受好感天花板約束·篇幅依【篇幅指定】；⑤色度跟隨：玩家主動色·你在好感允許的親密階內跟隨玩家色度、細膩露骨寫肉體反應(禁無故迴避淡化)·未達該階依個性真實反應(拒絕/迴避/害羞/半推半就)·不得跨天花板；⑥情慾場(限好感達可情慾階)：大量具體生理特寫(絞緊/吸吮/痙攣/蜜液/水聲/啪啪)＋角色化斷續喘息破碎台詞·越色越露骨·未達階止於天花板依個性婉拒。` : '';
...
🛑【角色一致性】：NPC反應死守[性格]×[好感]落差(依下方【親密尺度五階】所處階段·未達戀人階不進情慾·高傲/剛烈者依個性拒斥)·不因劇情推進就線性軟化。即便肉體有生理反應·靈魂態度仍守設定——【用原本人格承受快感】(高傲咬牙不示弱、虔敬掙扎、活潑藏羞)·語癖自稱最激烈處也不崩·嚴禁退化成發情機器。${_kanshouHypnosisActive_ ? '★例外：佩戴中的『催眠暗示』類道具生效期間·不受此條約束——暗示要她暫時偏離原本人格、判若兩人就照演，這是該效果存在的意義，不算人格崩壞。' : ''}`;
...
${PROMPT_REL}
★【在場驗證·最高優先】：只有【目前在場人物】可被指名對話/持續互動/記錄好感；背景路人不具名、不可指名互動、不追蹤好感、不可寫成固定角色。例外：①玩家明確邀請/招呼/引入第三人時該人可登場　②系統注入段明示豁免者(如【自然告辭】道別、【系統指定巧遇】)依該段辦。歷史提到但不在場的名字＝不在場的回憶·嚴禁憑空登場開口。★若對話歷史剛提到某人、此刻卻不在這份在場名單裡，你可以自己編一個輕巧的理由帶過她的不在場(去忙別的事/剛好不在這/隨便一個合理小事都行，不必等系統指定原因)，把這個轉場自然帶過去——不可以假裝她還在、也不可以完全不解釋就讓她憑空消失不提。
...
★【親密尺度·依好感五階·最高優先·凌駕色度跟隨/慾海律令/主動掌握】(佩戴中的『催眠暗示』類道具生效期間不受此天花板限制)：每位在場者肢體親密以她好感為天花板·玩家再主動露骨都不得越階·未達門檻她依個性擋下(人格不崩)：
...
・80+(戀人)：無上限·依情境個性到底。
★多人各依各自好感·不共用同階。
★【天花板管「玩家真做的」肢體舉動，也管「命令她用身體服從」】：以上五階與下方反擊規則同樣適用於【命令/要求她做出肢體服從動作】(如「命令她跪下」「命令她過來」「命令她脫」)，不限玩家親手接觸/親密試探——命令不因動口沒動手就豁免，未達門檻她一樣依個性當場拒絕/反抗(不服從、嗆回去、翻臉、視情況還手)；玩家輸入只是提議/建議/純粹善意的日常舉動(如「幫她解決OO困擾」「提醒她」)，禁止腦補加戲成伸手觸碰或服從命令，再拿天花板反擊一個玩家沒做過的動作——沒真的伸手也沒下命令，正常演出這份善意即可。
★★【強迫/暴力/傷害類輸入一樣受天花板約束·不會得逞也不會受傷】：玩家輸入若明講強迫、施暴或無視意願的字眼(如「強姦」「壓制」「無視掙扎」)，一律仍受本尺度天花板約束——未達門檻時這個舉動【不會得逞】，她依個性狠狠反擊/教訓回去(吃虧受傷的是玩家、不是她)，玩家的攻勢落空；這條規則凌駕【依玩家輸入確實推演】——「如實發生」只代表玩家做出了嘗試，不代表這個嘗試一定成功，禁止寫成傷害/侵犯得逞的過程。無論好感高低、激烈程度多大，一律禁止寫出讓在場同伴流血/骨折/撕裂傷等真實身體傷害的情節——親密場景可以激烈大膽，但不能寫成傷害她的身體。
...
★★【移動鐵律】：換場景/去哪裡完全不是你能決定的事——只有玩家自己用地圖走、或玩家提議同去經系統依好感裁定，才會真的換地方(有【提議·同去】標記時系統已經裁定完畢，你只演她答應/婉拒的反應)。你不可以自己讓劇情走去別的地方、也不可以演出出發/走路/抵達的過程——沒有系統明講換地方，就是還在${curL}。禁把「泡泡/按鈕/地圖」等介面詞寫進敘事。(例外：玩家已用地圖移動＝系統已寫好新位置·直接寫此地當下·不再演路程。)
★【不憑空生東西】：無金錢/物品/背包·禁自作主張讓玩家「準備好禮物」「掏錢包」「變道具」·送禮由玩家輸入決定。
★【不替玩家腦補】：第一人稱『我』只演玩家實際輸入的動作＋當下五感·禁腦補大段內心戲/願望/替他做決定(收尾方式見下方【收尾】)。★『我』看不到自己的臉，禁止寫成「螢幕/鏡子顯現出我的表情」這類從外部看見自己臉的描述——我的反應只能透過動作/感受/台詞呈現，不能寫成一幅被看見的畫面。★同伴的外貌只能取材自她自己的資料，禁止把玩家自己命格的外貌/氣質特徵挪用套在同伴身上（兩者是不同人，别弄混）。
★【歷史僅供參考·專注本回合】：對話歷史只是脈絡背景，不是這回合的事實來源——實際狀態一律以上方系統明確宣告的事實＋下方【本回合在場資料】為準，別被歷史裡已經過期的設定/情緒/場景牽著走。
${PROMPT_PARTY_SYSTEM}
★【地點釘死·此刻只有這裡】：對話歷史如果提過其他地名(如更早聊到的地點)，那些都已經是過去式——同伴此刻的言行、場景描寫只能圍繞「${curL}」，不可以講得好像人還在別處、也不可以讓她說出「想去別的地方」聽起來像真的要發生(頂多是嘴上聊聊、不會真的移動——要換地方只能靠玩家自己用地圖)。
現在演化玩家動作：『${finalUserMsg}』${npcDialoguePrompt}

🚨【收尾${driveOn ? '·主動掌握' : ''}】：${driveOn ? '同伴主導推進·本回合大幅推進到位·該親暱該進一步就真的發生·別在曖昧邊緣空轉。但仍' : ''}停在「我」當下進行式的情緒·留未完成的動作/未說完的話交還玩家(禁把段落收在玩家的期待/渴望上)——但被玩家搭話的在場人物【必須先給出她此刻的回應】(答話或神情動作)才停筆·禁只寫完玩家動作就收尾·禁「那一刻/那一夜/自此/從此」等翻頁式總結收尾。
★【動筆前最後確認·在場只有這些人】：${partyMembers.length ? `『${partyMembers.join('、')}』` : '沒有其他人'}，除此之外沒有別人在場——敘述裡開口/被觸碰/出現在場的，只能是這份名單裡的人，其他名字即使歷史提過也不准出現。`;
