...
function translateLookToDaily_(name, cls, rawLook, firstP, speech, dailyMoeHint, sex) {
  ...
  try {
    ...
    var figureHint = (sex === "女") ? "，若角色是成年女性、務必包含身形/胸部具體描寫(如巨乳、貧乳、嬌小玲瓏、高挑纖細等，「豐滿」這類籠統形容詞不夠明確，須明講胸部大小)，不要只寫髮色瞳色就交差" : "";
...
- traits：外貌、氣質舉止、自稱與口氣(第一人稱·如 我/俺/吾＋說話語氣)、卸下心防的私密一面。${finalSex === '女' ? '外貌段務必包含身形/胸部具體描寫(如巨乳、貧乳、嬌小玲瓏、高挑纖細等，「豐滿」這類籠統形容詞不夠明確，須明講胸部大小)，不要只寫髮色瞳色就交差。' : ''}範例：「黑長直髮琥珀瞳、氣質溫婉恬靜、自稱「我」語氣輕柔、私下愛對植物自言自語」
