...
function actionBackfillKanshouAi(userData, pcId, sheets) {
  ...
  const promptStr = `【主角】：名號『${finalName}』，性別『${finalSex}』\n【外貌】：${appearance || "隨機"}\n【身世】：${standing || "隨機"}\n【個性方向】：${persona || "隨機"}`;
  ...
  //   過聖杯戰爭這回事」原則正面矛盾，等於教AI把玩家當成戰爭倖存者、用「御主」(聖杯戰爭裡令咒契約
  //   的戰爭專屬身分)稱呼——這正是稍早修過的「從者」REL_TAG洩漏的同一類問題，只是這次換成玩家自己
  //   的persona生成入口沒被同步修到。已全段改寫成中性的「這個平行世界的日常主角」框架，拿掉所有
  //   「聖杯戰爭已結束/已落幕」的斷言。
  const KANSHOU_MASTER_GEN_SYS = `你是《命運停駐之夜》後日談(鑑賞)的角色生成核心，為玩家建立一位生活在平行世界(這裡從來沒有發生過聖杯戰爭這回事)、與身邊英靈共度和平日常的主角形象。請依玩家提供的姓名、性別、外貌、身世、個性方向，生成合理且溫暖自然的設定。
...
★npc_intent：一句【簡短】萌點（可愛反差，≤18字，系統會在30字處硬性截斷、務必精簡），結合此人身分性格，要反差、可愛、獨特。務必寫完整一句話，不可斷在句意未完處。【禁】誤用聖杯戰爭機制專有詞(令咒/寶具/魔術迴路/從者/御主/職階等)當裝飾性魔法元素湊萌點——這個平行世界從未發生過聖杯戰爭，這些詞在這裡沒有來由，請改用生活化情境(手作/習慣/小癖好等)。★這個萌點必須是單看了會覺得溫馨、正面、會心一笑的日常小反差(如生活小習慣、意外的手藝、小小的害羞反應等)，【禁】靠創傷/自卑/孤獨/悲劇宿命撐出反差感——那是戰時角色才需要的沉重寫法，這裡是輕鬆的日常後日談。
...
function buildDefaultSystemPrompt() {
  ...
  const _physicalState = "本回合角色當下的顏面神情與衣裝狀態(第三人稱填寫，依情境自然帶到即可，不必逐項列舉，≤15字)";
  ...
  const finalJson = {
    ...
    "location": "本回合結束時玩家所在地點——若narration有實際敘述移動/抵達，填新地點名稱(可自創、不限於冬木既有地名)；沒有移動則原樣填目前地點",
    ...
    "intimacy_feedback": {
      "_note": "★physical_state是角色「自身」當下的顏面神情與衣裝狀態，禁內心戲，第三人稱填寫，絕對禁寫'自己'，≤15字。★每回合都要據實反映最新狀態，不可偷懶沿用舊值；不必逐項列舉，依當下情境自然帶到即可。npcs每位與player共用此格式，依其實際狀態填寫。",
      "player": {
...
💕【鑑賞·後日談模式·最高優先級覆寫】：${partyRows.length === 0
    ? `這裡是平行世界的和平都市日常——聖杯戰爭這回事從未在這個世界發生過，眼下沒有同行的英靈在場，就是玩家一人的尋常時光。`
...
★世界觀＝和平的現代都市日常：【絕對禁止】任何戰鬥、廝殺、敵人、聖杯爭奪、靈基受損、血量／生命變化、寶具對轟、死亡或威脅，世界是安全的；但節奏與親密程度依劇情、好感與玩家/同伴當下意圖自然發展，可以是散步閒聊的尋常時光，也可以是更靠近、更熱烈的相處，不強制鎖在「悠閒」基調(尤其🔥主動掌握模式開啟或情慾已自然升溫時)，讓從者貼近其官方性格自然地與玩家相處互動。
...
${driveOn ? `🚨【敘事終極警告·主動掌握模式】：同伴主導推進，本回合可以確實大幅向前推展——不必像平日矜持模式那樣每次都停在剛起步的瞬間，讓「步步進逼」的壓迫感真的往前走、玩家打少少字也能推進不少。但仍【絕對禁止】把這整段相處寫成「那一夜／自此／就這樣／從此」等總結收尾句，不可讓這回合讀起來像已經翻頁的完結篇章——停在「我」當下進行式的心境與情緒中，留一點空間給玩家插入反應、喊停或喘息，而非停在原地一動也不動。`
  ...
  try {
    ...
    if (aiData.intimacy_feedback) {
      ...
      //   範圍與字數，這裡補上後端強制截斷防呆，不完全依賴AI自律守住15字上限。
