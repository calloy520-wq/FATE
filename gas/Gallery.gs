...
🕰️現在是${curDateObj_.year}年${curDateObj_.month}月${curDateObj_.day}日・${kanshouFmtHM_(curHour)}・${timeBand_(curHour)}，僅供揣摩場景氛圍與時段感(如深夜靜謐、清晨慵懶、應景節氣)，不必刻意報時或提及具體數字。★【時間尺度】：narration 只寫此刻當下的片段、順著目前時段的光線氛圍；時間感靠光線、氣氛、動作的節奏自然流露，【絕對禁止】自行跳時段(「過了好幾個鐘頭」「到了傍晚」)或把「半小時」「三十分鐘」「過了一段時間」這類時間長度字眼寫進敘述——唯一例外：系統本回合已明確宣告時間推進(如「過了N個小時」的換幕)時，據實承接那次跳轉即可。
...
🚨【敘事終極警告${driveOn ? '·主動掌握模式' : ''}】：${driveOn ? '同伴主導推進，本回合可以確實大幅向前推展——不必像平日矜持模式那樣每次都停在剛起步的瞬間，讓「步步進逼」的壓迫感真的往前走、玩家打少少字也能推進不少。但仍' : '結果後'}必須停在「我」當下進行式的心境與情緒中，留一個未完成的動作、未說完的話或懸而未決的情緒把下一步交還玩家——【絕對禁止】寫出「那一刻／那一夜／自此／就這樣／從此」等總結收尾句，讓這回合讀起來像已經翻頁的完結篇章！`;
