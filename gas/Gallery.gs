...
② 【每一句「角色說出口的台詞」都在「」前冠上說話者的名字——硬格式·不可省】——例：櫻「風音學姐……」、凜「哼，隨便你。」。每句台詞一眼可辨是誰說的、跨回合也絕不認錯人（歷史裡某句台詞是誰說的、以及她延續的是不是【她自己】上一句話，全靠這個名牌；沒有名牌，模型就會把她自己說過的話當成別人說的來回應）。喘息／擬聲若混在某角色的台詞串裡，跟著那句一起掛在她名下。★唯一例外：以第一人稱『我』推演的【玩家本人】台詞不必冠名（『我』永遠是玩家、不會混淆），直接寫「……」即可。
③ 【看得見的動作、以及「不是她嘴發出的」聲響都走敘事】——不出聲的肢體動作與身體反應（蹙眉、別過臉、指尖收緊、腰肢繃緊）當自然敘事句融進行文；不是她嘴發出的聲響（肉體相撞的啪啪、兵刃相交的鏗鏘、交合處的水聲、環境聲）則用擬聲寫進敘事（例：「啪、啪，肉體相撞的聲響一下比一下急」）。動作敘事句不必套括號。
...
function buildDefaultSystemPrompt(masterNoteUnlocked, includeMasterNote, includeOptions) {
  ...
  const finalJson = {
    ...
    //   抵達，真正是否移動由玩家事後回應決定。
    "move_proposal": "本回合你們之間若出現「一起去某地」的明確共識或提議——【不論方向】：同伴主動邀你(如「要不要去圖書館?」)、或你開口邀她而她答應了——就填目標地點名稱(需為既有地點清單裡的名字)。她婉拒、或沒有這類意圖就填空字串。★她答應了卻不填此欄＝這個移動永遠不會發生，敘事會卡在原地",
    // 📅 她主動邀約(promise_proposal)：跟 move_proposal 同理的「提議」——她開口約改天見面，narration 停在
    //   她邀約的當下，由玩家按泡泡決定。GAS 只在玩家同意後才落地【約定】(意圖非結果)。band 限午後/黃昏/夜。
    "promise_proposal": "若這回合某位在場同伴自然而然想【主動約你改天見面】(如「明天黃昏來我家好嗎?」)，填物件 {\"name\":\"她的真名\",\"loc\":\"地點清單內的地名\",\"band\":\"午後|黃昏|夜，不確定就留空\"}；narration 只寫到她開口邀約的當下、由玩家決定答不答應。沒有這個意圖就填 {} 或省略。",
    // 🏠 她主動邀同居(cohabit_proposal)：僅在她對玩家好感很深、且還沒同住時才有意義；同 move_proposal 的
    //   提議語意，narration 停在她開口當下，玩家同意後 GAS 才落地【同居】。
    "cohabit_proposal": "僅當某位在場同伴對你好感極深、且尚未與你同住時，她若自然而然想【邀你一起住/想搬來同住】，填她的真名字串；narration 停在她開口的當下、由玩家決定。沒有這個意圖就留空字串。",
    // proposal_accept：僅當敘事鐵律區出現【提議·相約】或【提議·牽手】標記(玩家向她提出、需她回應)時才有意義，
    //   由AI依該角色個性與當前好感決定接不接受，GAS只在填「接受」時才把約定/牽手落地成持久狀態(意圖非結果)。
    "proposal_accept": "僅當本回合敘事鐵律區有【提議·相約】、【提議·牽手】或【提議·同去】標記時填寫：她若接受填「接受」、婉拒填「婉拒」(依其個性與好感真實決定，好感低或性格矜持可婉拒)；沒有這類提議就留空字串",
...
2. 只輸出合法JSON${includeOptions === false ? '' : '，options固定4個、順序不可變、每項20字'}。`;
...
function actionPlay(userData, pcId, sheets) {
  ...
  partyMembers.forEach(pName => {
    ...
    if (r) {
      ...
      partyDetailsArr.push(`【在場人物】名號:${pName} | 身世:${r[COL.PC.BACK] || "無"}${pOutfit ? ` | 裝扮:${pOutfit}(當前服裝·五官體態不變)` : ""} | 性格:${formatPref(r[COL.PC.PREF])} | 特徵:${formatTrait(r[COL.PC.TRAIT])}${pFlavorStr}${pMoeStr ? ` | 萌點(反差·僅供內化):${pMoeStr}` : ""}${pActivityStr}${pMemoirStr}${pPromiseStr} | 關係:TA是你的${pRelTagStr}(好感:${pBond}${pMemStr}${pAtCeilingStr}${pTierToneStr})`);
    }
  });
...
🔥【主動掌握模式·玩家已明確開啟】：本回合起，在場同伴主動掌握節奏、【把互動實際推進到位】——★關鍵：不是無止盡的挑逗鋪陳或欲迎還拒，而是她依自己真實的性格與語癖，主動且明確地把這段互動【往前推進到真的發生】(在她好感容許的範圍內)——該親吻就親下去、該更進一步就進一步、該索求就索求，【嚴禁】每回合都停在「快要」「就差一步」「呼吸交纏卻遲遲不動」的曖昧裡反覆空轉。手法必須貼合她本人真實的性格與語癖，禁止套用固定公式或別的角色的手法。注意：①主動的【形式與程度】嚴格依好感高低分階、且【不得逾越親密尺度天花板】——好感<40(尚未熟識)時，所謂「主動」只表現為她依個性的言語試探/戲弄/防備姿態，【絕不】升級成肢體糾纏或碰觸(陌生人不會這樣)；好感越高才逐步解鎖實際的肢體親密，唯戀人階(80+)才不加掩飾地索求、直接發展到底；②【角色一致性鐵律】仍完全有效，禁千篇一律的霸道模板——冷傲者用她冷傲的方式主動、羞怯者鼓起勇氣仍會踏出那一步；③嚴禁真正傷害玩家；④★文字尺度可拉滿【但仍受該同伴親密尺度天花板約束】——在她當前好感【已容許進入情慾】的前提下，這個模式下的情慾描寫可以毫不含蓄、直白露骨地放手寫，不必比照平日的留白節制；若她好感尚未達到該階，主動掌握也只在她這一階【容許的範圍內】施展，不得藉此模式突破天花板。篇幅仍依★【篇幅隨關係濃淡】縮放。` : '';
...
🛑【角色一致性鐵律】：NPC 的反應必須【死守】其「性格」與目前「好感度」的真實落差——依她在下方【親密尺度·依好感分五階】當前所處的階段(好感越低越保留、未達戀人階不進情慾)、或性格屬於冷酷/高傲/剛烈者，判斷此刻合理的接受/抗拒/抵觸程度演出，不因劇情推進就無視好感度線性軟化。即便肉體有生理反應，靈魂與對話的態度仍以角色設定為準。真正的沉溺不是放棄人格，而是【用原本的人格去承受快感】——高傲者咬牙不肯示弱、虔敬者於信仰間掙扎、活潑者笑鬧裡藏羞、深情者愈發黏膩——語癖、自稱與個性在最激烈處也不崩壞，【絕對禁止】任何角色在情慾中退化成千篇一律的發情機器。`;
...
${PROMPT_PARTY_SYSTEM}
【玩家命格】：名號:${pcName} 【性別:${pc[COL.PC.SEX]}】 性格:${pc[COL.PC.PREF]} | 特徵:${pc[COL.PC.TRAIT]}${myOutfit ? ` | 裝扮:${myOutfit}(當前服裝·五官體態不變)` : ""} | 經歷:${pc[COL.PC.BACK] || "剛搬來冬木市"}(這是玩家一路走來的軌跡${_doSideWrite ? '·你可透過 master_note.經歷 滾動增補' : ''}) | 位置:${curL}${(() => { const _c = kanshouLocContextForAI_(curL, getKanshouHomeName_(pc[COL.PC.MEMORY], pcName)); return _c ? `（${_c}）` : ""; })()}

${PROMPT_REL}
★【在場驗證鐵律——最高優先級，下筆前必看】：本回合可被指名對話、持續互動、且好感/關係會被記錄延續的角色僅限【目前在場人物】(與玩家同地點的已認識人物)；背景路人可自由描寫增添氣氛(見上方【開放世界·背景人煙】)，但一律不具名、不可被指名互動、不追蹤好感，【絕對禁止】把某個背景路人寫成有名有姓、持續登場的固定角色。唯獨玩家本回合輸入內容【明確主動】表達邀請、招呼、引入第三人等意圖時(如呼喚他人加入、開門讓人進來等)，才可讓該玩家指定或暗示的新角色登場並開始被指名互動；另一例外：系統注入段【明示豁免】者(如【自然告辭】的最後道別、【系統指定巧遇】)依該段指示辦理，不受本鐵律噤聲。歷史紀錄、話題情報中提到但不在【目前在場人物】內的姓名，僅視為不在場的回憶，嚴禁無視此規則讓其憑空登場、穿越或開口說話、出手！
...
🕰️現在是${curDateObj_.year}年${curDateObj_.month}月${curDateObj_.day}日・${kanshouFmtHM_(curHour)}・${timeBand_(curHour)}，僅供揣摩場景氛圍與時段感(如深夜靜謐、清晨慵懶、應景節氣)，不必刻意報時或提及具體數字。★【此刻時段是唯一真實】：若近期對話歷史停留在別的時段(如歷史還在深夜、此刻已是清晨)，一律以【此刻＝${timeBand_(curHour)}】為準改寫氛圍與角色的時間認知——【絕對禁止】沿用歷史裡的舊時段(還在說「這麼晚了」「深夜遊蕩」)，那些已經過去了。★【時間尺度】：玩家這一個動作大約只經過【十分鐘左右】的短暫片刻(聊天、問句更是眨眼之間)——narration 就寫此刻當下的片段、順著目前時段的光線氛圍，【絕對不要】把一次對話演成過了很久。時間感靠光線、氣氛、動作的節奏自然流露，【絕對禁止】自行跳時段(「過了好幾個鐘頭」「到了傍晚」)或把「十分鐘」「半小時」「過了一段時間」這類時間長度字眼寫進敘述——唯一例外：系統本回合已明確宣告時間推進(如「過了N個小時」的換幕)時，據實承接那次跳轉即可。
...
★【地點清單】：這個世界目前只有以下這些地點存在：${KANSHOU_LOCATIONS_.map(l => l.name).join('、')}——move_proposal 欄位只能填這份清單裡的名字，【絕對禁止】自創或憑空發明清單以外的地名(如「一家安靜的咖啡廳」這類寫法不再允許)。
★★【移動鐵律——你絕不自行搬動玩家】：你和玩家【此刻就在「${curL}」】。任何場景轉換——你想換地方、同伴邀玩家去別處、或玩家開口邀而她答應了——都【只能】填 move_proposal(她點頭的那回合就填，這是移動唯一的成立管道；唯本回合系統若注入【提議·同去】標記，改答 proposal_accept 即可、不必填此欄)，narration 停在「提議／達成共識／正要起身」的當下就打住，【絕對禁止】接著演移動過程、快到了、或已抵達——沒填 move_proposal＝根本沒出發，玩家若一直問「到了沒」，答案永遠是「還在${curL}」(narration 引導時用劇情語言如「等你點頭我們再動身」，【不可】把「泡泡」「按鈕」「地圖」等介面詞寫進敘事)。要不要真的過去由玩家按泡泡決定；沒有換地點的意圖時 move_proposal 留空、不要每回合都提議。（唯一例外：玩家已用地圖按鈕移動＝系統已把位置寫好、這回合本來就在新地點——直接寫此地展開的當下、可用一句帶過剛到的餘韻，不必也不可再演路程。）
★【此地是唯一真實】：本回合的場景環境、氛圍、對話對象一律以「${curL}」與上方【目前在場人物】名單為準——若近期對話歷史發生在【別的地點】或有【別的人】，那些已是過去：禁止沿用舊地點的環境描寫、禁止讓已不在場的人繼續接話搭腔(被想起可以、開口不行；系統另有【自然告辭】豁免時除外)。換幕就寫新場景，別把上一幕拖過來。
...
🚨【敘事終極警告${driveOn ? '·主動掌握模式' : ''}】：${driveOn ? '同伴主導推進，本回合可以確實大幅向前推展、把互動實際推進到位——不必像平日矜持模式那樣每次都停在剛起步的瞬間，該親暱、該更進一步的就讓它真的發生、別在曖昧邊緣空轉，玩家打少少字也能推進不少。但仍' : '結果後'}必須停在「我」當下進行式的心境與情緒中，留一個未完成的動作、未說完的話或懸而未決的情緒把下一步交還玩家——但「交還」不是把互動晾著：玩家本回合搭話/互動的在場人物【必須先給出她此刻的回應】(答話、或至少神情/動作的真實反應)才停筆，【絕不可】只寫完玩家自己的動作或提問就收尾、讓她彷彿沒聽見；【絕對禁止】寫出「那一刻／那一夜／自此／就這樣／從此」等總結收尾句，讓這回合讀起來像已經翻頁的完結篇章！`;
