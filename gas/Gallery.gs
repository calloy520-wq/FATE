...
function actionPlay_(userData, pcId, sheets) {
  ...
  if (kanshouBandCrossed_) {
    ...
    if (_lvNames.length) kanshouNpcLeaveStr_ = `\n★【自然告辭·作息——本回合特別豁免，不受【在場驗證鐵律】限制】：時段來到${timeBand_(curHour)}，『${_lvNames.join('、')}』到了該走的時間——她(們)雖已不在【目前在場人物】名單，本回合【唯獨允許】她(們)開口說這最後一句道別(依你記憶中她的個性，有事要辦/該回去了皆可，去向不必交代)，narration 要把道別演出來、不可無聲消失；★若剛才的敘事有肢體接觸/牽制/擁抱等仍在進行中的互動，這裡要明確演出接觸中斷(掙脫/放開/退開皆可)才道別，不可假裝那段接觸還在繼續；道別之後她(們)就真的不在場了——這是她(們)在這個場景的最後一次出現，之後的回合絕對不可以再讓她(們)開口、被觸碰、或被寫成仍在場，即使剛才的互動聽起來還沒收尾也要當作已經結束。`;
  }
