...
function dialogueFormatRule_() {
  return `對話格式：單層「」只收嘴巴發得出的聲音(話語/笑聲/嘆息/悶哼)，每句前冠說話者的名字，只用卡片開頭句號前那一個名字（性別與真名留在卡上），同一個人跨回合都用同一個；★玩家的台詞免冠名，可以擴寫成完整的一句、補上說這句話當下的動作與神態，語意跟原句一樣；擴寫的範圍就是這一句話。肢體動作與環境聲響留在引號外，台詞跟說話的人那一刻的動作放在同一段。`;
}
