...
var KANSHOU_STYLE_MODULES_ = [
  ...
  { key: 'agency',     fixed: true, slot: 'sys',  def: '玩家這一步做什麼、說什麼，由玩家的輸入決定：把它化成「我」的動作與台詞融進場景（口氣可以改、句子可以拆、可以接在動作裡），再從它落地的那一刻往下演。' },
...
接著往下演，玩家這一步（化成我的動作與台詞融進去）：『${finalUserMsg}』`.replace(/\n{3,}/g, '\n\n');
