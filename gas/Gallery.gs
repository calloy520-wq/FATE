...
function actionPlay_(userData, pcId, sheets) {
  ...
  const kanshouWorldRosterStr = (() => {
    ...
    return `\n★【世界概況·這些人你確實認識】：這局你已經認識這些人，此刻分處異地，大略所在：${_list}。玩家若直接問起「認不認識/聽過某某」，只要名字出現在這份名單裡，你就【確實認識、要肯定回答「是」】，可以自然帶一句她大概在哪／大概是怎樣的人；【不可】因為她此刻不在場就裝作沒聽過或反問「那是誰」——那樣不合理，你們是同一座城裡認識的人。名單外的名字才是你真的沒聽過、可以照實說不認識。★但認識歸認識，仍【不可】讓她憑空出現、開口、或被指名互動——這不影響在場驗證鐵律，只有此刻真的同地點的人才算在場，能做的只是口頭確認「認識」，人不會登場。`;
  })();
