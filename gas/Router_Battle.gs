...
function actionFateBattle(userData, pcId, sheets) {
  ...
  //   實際上可能是我方從者自己的名字(敵方回擊/NP對轟回震/敵盟協防致死時)。此戰若還有其他從者存活，
  //   defeat 不會是 true(見 fateStrike_ 的「雙從者」判定)，於是走進這支 finalLine／終局指令／收尾指令，
  //   卻把「我方死了」誤講成「defC死了」，AI 收到自相矛盾的事實只能各自表述。
  // 🛡️ 穩健：不能只比對 destroyedName===atkC.name——雙從者出戰時，敵方回擊/敵盟協防的目標(ctgt/ctgt2)
  //   在 atkC 已陣亡時會改打另一名在世從者(見上方「alt/alt2」邏輯)，那種情況死的是「我方」但不是 atkC。
  //   改直接查 destroyedName 那一列在 pcData 裡的真實 FACTION 是否為「從者」，涵蓋所有我方陣亡路徑，
  //   而非只堵已回報的那一種。
  ...
  if (defeat) {
    ...
    aiPrompt = ourMasterCardStr + servantCard_(pcData[atkIdx]) + foeServantCardStr + enemyMasterCardStr + allyAssistCardStr + pactDefCardStr +
      ...
      ((destroyedName && !sealEscaped && !godRevived) ? (ourSideDestroyed
        ? `★【本戰已於第 ${rounds.length} 回合終結】『${destroyedName}』已當場靈基崩潰、化作光點消散——這是我方的死局，「${defC.name}」仍存活於場上、無需跟著消散。【嚴禁】讓『${destroyedName}』在此之後繼續出手、反擊或存在於場上，也【嚴禁】讓御主詢問「接下來怎麼辦／要不要繼續」這類彷彿戰鬥仍未分曉的台詞。演出應收在「我方從者殞落的這一擊」與御主的震動反應，不可延伸出新的交鋒回合。\n`
      ...
      // 🐛→✅ 玩家實測抓到：「值得乘勝追擊還是該見好就收」這句範例文字太具體，同一場戰鬥拖好幾回合
      //   時，模型每回合都套用近乎同一種「要不要撤退/繼續」問句收尾，讀起來像跳針——況且這句本身
      //   也踩到 masterCard_ 那條「不可替御主拍板下一步」的線(見上方戰局實況、僅供角色觀察不是戰略
      //   建議的立場沒問題，但字面給的範例太窄)。範例改給更多樣的角度、並點名連續回合別重複同一種。
      ((!destroyedName && !sealEscaped && !godRevived) ? (hasFx_(atkC, 'mad')
        ? `★戰後讓「${atkC.name}」以其已狂化的方式(低吼／肢體動作／神情)透出對這場交手的直覺判斷，不成篇整句台詞。\n`
        : `★戰後讓「${atkC.name}」依其性格與口吻，對這場交鋒給出簡短的主觀判斷或反應(如看出的破綻、對方寶具是否已現底牌、自己的傷勢與魔力還撐不撐得住、對敵手的評價或情緒——不必每次都框在「該追擊還是該撤退」這個角度)——是角色的觀察，不是戰略指令，下一步仍由御主按鍵定奪；若同一場戰鬥已連續好幾回合，這句反應換個角度講，別每回合都用同一種問法收尾。\n`) : "") +
