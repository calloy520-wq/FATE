...
function actionFateBattle(userData, pcId, sheets) {
  ...
  if (defeat) {
    ...
    aiPrompt = ourMasterCardStr + servantCard_(pcData[atkIdx], { skipClose: true }) + foeServantCardStr + enemyMasterCardStr + allyAssistCardStr + pactDefCardStr + performanceNote_(_perfNames) +
      ...
      //   時，模型每回合都套用近乎同一種「要不要撤退/繼續」問句收尾，讀起來像跳針。範例改給更多樣
      //   的角度、並點名連續回合別重複同一種。「不可替御主拍板下一步」這條規則 `ourMasterCardStr`
      //   (=masterCard_，見下方 aiPrompt 組裝已固定排在最前面)已經講過一次，這裡不重複，省字數。
      ((!destroyedName && !sealEscaped && !godRevived) ? (hasFx_(atkC, 'mad')
        ? `★戰後讓「${atkC.name}」以其已狂化的方式(低吼／肢體動作／神情)透出對這場交手的直覺判斷，不成篇整句台詞。\n`
        : `★戰後讓「${atkC.name}」依其性格與口吻，對這場交鋒給出簡短的主觀判斷或反應(如看出的破綻、對方寶具是否已現底牌、自己的傷勢與魔力還撐不撐得住、對敵手的評價或情緒——不必每次都框在「該追擊還是該撤退」這個角度)；若同一場戰鬥已連續好幾回合，這句反應換個角度講，別每回合都用同一種問法收尾。\n`) : "") +
